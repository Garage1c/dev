...
Процедура ОбработатьОтображениеИнфОКонтрагенте() Экспорт
	ОперативнаяИнформация.ОбработатьОтображениеИнфОПартнере(ЭтаФорма, "ПохожиеКонтрагенты", "ПохожиеКонтрагенты", партнер);//?(Контрагент <> Неопределено, Контрагент, ПохожиеКонтрагенты.Получить(Элементы.ПохожиеКонтрагенты.ТекущаяСтрока).Значение ));
КонецПроцедуры
&НаКлиенте
