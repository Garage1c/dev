...
///****  конец изменения 
	...
	Элементы.КнопкаОбработкаЗавершена.Видимость	    = ЛОжь;//МожноРедактировать И Склад.ПередачаТовараМВЗ;
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//видимость доступна
	//для автора, ответственного, основного менеджера контрагента и для того, у кого главные права в области.
	...
	//ПользователиУправляющиеРезервом.Добавить(Объект.заказ.МенеджерЗаказа);
	...
	//Элементы.СуммаНДС.Видимость 		= УчитыватьНДС;
	//Элементы.ЗаполнитьСтавкуНДС.Доступность = УчитыватьНДС;
	
	Элементы.КнопкаСоздатьКонтрагента.Видимость = МожноРедактировать И Контрагент.Пустая();
...
Функция ПолучитьДолгКонтрагента(СсылкаКонтрагент)
...
Процедура ПрочитатьРеквизиты()
	...
	ДолгПарнера = ?(Объект.Ссылка.Пустая(), 0, ПолучитьДолгКонтрагента(Контрагент));
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПустойКонтрагент = Справочники[?(ПолучитьФункциональнуюОпцию("НемецкийУчет"), "Контрагенты", "Грузополучатели")].ПустаяСсылка();
...
Процедура Разместить(Команда)
	...
	ФункцииФормДокументов.ОбновитьСтруктуруКолонокТовары(ЭтаФорма, СтруктураКолонокТовары, КэшируемыеФункции.ПолучитьРеквизитыДокумента("ИнтернетЗаказПокупателя"));
...
Функция ПолучитьПользователяИнтернет()
	...
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1 ПользовательИнтернет ИЗ Документ.ИнтернетЗаказПокупателя ГДЕ Ссылка = &Заказ");
	Запрос.УстановитьПараметр("Заказ", Объект.Заказ);
...
Функция ПроверитьРеквизитыЗаказа()
	...
	Если НЕ ЗначениеЗаполнено(ВариантДоставкиНов) Тогда
		ОбщиеФункции.СообщитьТекст("Не заполнен вариант доставки","ВариантДоставкиНов"); Отказ = Истина; 
	Иначе
		...
		Если ВариантДоставкиНов=ПредопределенноеЗначение("Перечисление.ВариантДоставки.ДоставкаДоКлиента") Тогда
			...
			Если НЕ ЗначениеЗаполнено(ЗаЧейСчетДоставка) Тогда
				ОбщиеФункции.СообщитьТекст("Не заполнено за чей счет доставка","ЗаЧейСчетДоставка"); Отказ = Истина; КонецЕсли; 
			
			Если НЕ ЗначениеЗаполнено(Грузоперевозчик) Тогда
				ОбщиеФункции.СообщитьТекст("Не заполнен грузоперевозчик","Грузоперевозчик"); Отказ = Истина; КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(АдресДоставкиНов) Тогда
				ОбщиеФункции.СообщитьТекст("Не заполнен адрес доставки","АдресДоставкиНов"); Отказ = Истина; КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(КонтактноеЛицоДоставки) Тогда
				ОбщиеФункции.СообщитьТекст("Не заполнено контактное лицо","КонтактноеЛицоДоставки"); Отказ = Истина; КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(ТелефонКонтактногоЛицаДоставки) Тогда
				ОбщиеФункции.СообщитьТекст("Не заполнен телефон контактного лица","ТелефонКонтактногоЛицаДоставки"); Отказ = Истина; КонецЕсли;
		...
		ИначеЕсли ВариантДоставкиНов=ПредопределенноеЗначение("Перечисление.ВариантДоставки.ДоставкаДоКлиентаНашимиСилами") Тогда
			...
			Если НЕ ЗначениеЗаполнено(ДатаДоставки) Тогда
				ОбщиеФункции.СообщитьТекст("Не заполнена дата доставки","ДатаДоставки"); Отказ = Истина; КонецЕсли;
...
Процедура Отказ(Команда)
	...
	//Если 	Вопрос("Отменить заказ?", РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да
...
Функция СохранитьИзмененияРеквизитовЗаказа(ЕстьТранзакция = Ложь)
	...
	МассивРеквизитов = КэшируемыеФункции.ПолучитьРеквизитыДокумента("ИнтернетЗаказПокупателя");
	
	// Таблица доставки
		
	времДок = Документы.ИнтернетЗаказПокупателя.СоздатьДокумент();	
...
Процедура ПриИзмененииРеквизитаВлияющегоНаТабличнуюЧасть(СтруктураКолонокТовары, ОбновитьСтруктуру = Истина, ПересчитатьЦену = Ложь)
	...
	Если ОбновитьСтруктуру Тогда
		ФункцииФормДокументов.ОбновитьСтруктуруКолонокТовары(
				ЭтаФорма, 
				СтруктураКолонокТовары, 
				КэшируемыеФункции.ПолучитьРеквизитыДокумента("ИнтернетЗаказПокупателя"));
	КонецЕсли;
	/////////временный кусок//////////////////
...
Процедура КонтрагентПриИзменении(Элемент)
	...
	Если КонтрагентРаботаетПоПередаче(Контрагент) Тогда	
...
Процедура ЗаполнитьРеквизитыДоставки()
	...
	Грузоперевозчик	   = ?(ЗначениеЗаполнено(Контрагент.ОсновнойГрузоперевозчик), Контрагент.ОсновнойГрузоперевозчик, Грузоперевозчик );
 	ЧастичнаяОтгрузка	   = ?(ЗначениеЗаполнено(Контрагент.ЧастичнаяОтгрузка), Контрагент.ЧастичнаяОтгрузка, ЧастичнаяОтгрузка );
...
&НаСервере
Функция КонтрагентРаботаетПоПередаче(Контрагент)
...
Процедура ЗаполнитьПоЗаказу(Команда)
	...
	ВыбранноеЗначение = ОткрытьФорму("Документ.ИнтернетЗаказПокупателя.ФормаВыбора",,,,,,Новый ОписаниеОповещения("ОбработкаЗаполненияПоЗаказу",ЭтаФорма,));
КонецПроцедуры
&НаКлиенте
...
Процедура ОтправитьПисьмоНаСервере()
	...
	//ТекстПисьма = Объект_.ВернутьТекстАвтоматическогоПисьма();
	//БизнесПроцессы.ИнтернетЗаявка.СформироватьПисьмо_Нов(Объект.Заказ, """Гараж Тулс"". Готов заказ", ТекстПисьма,,Истина,,, "Готов к отгрузке");
	//БизнесПроцессы.ИнтернетЗаявка.СформироватьПисьмо_Нов(Объект.Заказ, "", ТекстПисьма,,Истина,,, "");
	Объект_.ПисьмоПодтверждениеЗаказаПредоплата(Неопределено);
	//БизнесПроцессы.ИнтернетЗаявка.ПисьмоПодтверждениеЗаказаПредоплата(ТочкаМаршрутаБизнесПроцесса);
