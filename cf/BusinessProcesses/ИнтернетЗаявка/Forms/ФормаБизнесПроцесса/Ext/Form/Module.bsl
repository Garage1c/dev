...
Процедура ПрочитатьРеквизиты()
	...
	ДолгПарнера = ?(Объект.Ссылка.Пустая(), 0, ПолучитьДолгПартнера(Контрагент));
...
&НаСервере
Функция ПолучитьЗаполненныйЗаказ_уд()
	...
	Заказ.Партнер 		= Объект.Партнер;
	Заказ.Контрагент 	= Объект.Контрагент;
	Заказ.Склад 		= Объект.Склад;
	Заказ.ТипЦен 		= Объект.ТипЦен;
	Заказ.Валюта 		= Объект.ТипЦен.Валюта;
...
КонецФункции
&НаСервере
Функция ПровестиЗаказ_уд(ТекущийОбъект)
	
	НовДокумент = ПолучитьЗаполненныйЗаказ_уд();
	НовДокумент.Дата = ТекущаяДата();
	
	Если Не ПроведенияДокументов.ПровестиДокумент(НовДокумент) Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПартнерПриИзмененииНаСервере(СтруктураКолонокТовары = Неопределено) 
	...
	ДолгПарнера = ДенежныеСредства.ПолучитьДолгПартнера(Контрагент);
...
Процедура ПартнерПриИзменении(Элемент)
	...
	//ДолгПарнера = ?(Объект.Ссылка.Пустая(), 0, ПолучитьДолгПартнера(Партнер));
