...
Функция ПолучитьТаблицуТоваров(СсылкаПроцесс, СсылкаЗадача = Неопределено) Экспорт
	...
	//|	СуммаПриход		КАК Сумма,
	//|	Номенклатура.Вес 	Вес,
	//|	Номенклатура.Объем 	Объем
	//|
	//|ИЗ
	//|	РегистрНакопления.ИнтернетЗаказПокупателя.ОстаткиИОбороты(," + ?(ОперативнаяДата, "&ДатаВыполнения","") + ",,,ИнтернетЗаказ = &Заказ)
	//|
	//|ОБЪЕДИНИТЬ ВСЕ
	//|
	//|ВЫБРАТЬ
...
Процедура ЗаполнитьФормуПоБП(Форма, СсылкаПроцесс, СсылкаЗадача = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СсылкаПроцесс) Тогда
		...
		|ИЗ
		|	БизнесПроцесс.ИнтернетЗаявка КАК Процесс
		|ГДЕ
		|	Ссылка = &СсылкаБП
		|;
		|");
...
Функция ПолучитьЗаголовокБП(СсылкаПроцесс) Экспорт
	...
	Возврат ?(СсылкаПроцесс.Пустая(),
	                    "Создание",
						"Интернет заказ " + СсылкаПроцесс.Заказ.Номер + " от " + СсылкаПроцесс.Заказ.Дата + " (БП №" + СсылкаПроцесс.Номер + ")");
...
Функция ПолучитьМассивКомментариев(СсылкаПроцесс) Экспорт
	...
	|
	|	ВЫБРАТЬ
	|		Комментарий, ""Интрнет заказ"", Дата, Ответственный
	|	ИЗ
...
Функция ПолучитьДеталиЗаказа_Ст(ЭтотЗаказ) Экспорт
	
	ТекстДеталиЗаказа = "<table>";
	
	АдресИнтернетМагазин = Константы.ПутьИнтернетМагазин.Получить();
	
	Запрос = Новый Запрос("	ВЫБРАТЬ
							|	Заказ.Номенклатура Товар,
							|	Заказ.КоличествоОстаток КолВо,
							|	Заказ.СуммаОстаток Сумма
							|ИЗ
							|	РегистрНакопления.ИнтернетЗаказПокупателя.Остатки(,ИнтернетЗаказ = &ЭтотЗаказ) Заказ
							|");
	...
	Пока Выборка.Следующий() Цикл
		Товар  = Выборка.Товар;
		ТекстДеталиЗаказа = ТекстДеталиЗаказа + "<tr><td>" + Товар.Артикул + "</td><td><A style=""COLOR: rgb(0,0,204)"" href=" + АдресИнтернетМагазин + НРег(Товар.alies) + "> "+ Товар +"</A></td><td> " + Строка(Выборка.КолВо) + "</td><td> на сумму " + Строка(Выборка.Сумма) + " руб.</td></tr>";
	КонецЦикла; 
	
	Возврат ТекстДеталиЗаказа + "</table><br><br>";
...
Функция ПолучитьДеталиЗаказа(ЭтотЗаказ, ДопРасходы, ОткрытиеЗаказа, СоСрокомПоставки = Ложь) Экспорт
	...
	//belova 07.06.2016 убираю срок поставки, им никто не пользуется
	//|" + ?(СоСрокомПоставки, "<th style=""background-color:#333333; text-align:center;color:#fff;font-weight:400;line-height:12px;font-family: Verdana,Geneva,sans-serif;font-size:11px;border-bottom:1px solid #bbbbbb;"">Срок поставки</th>", "") + "	
	...
	//Запрос = Новый Запрос("	ВЫБРАТЬ
	//|	Заказ.Номенклатура.Артикул 		Артикул,
	//|	Заказ.Номенклатура.alies		alies,
	//|	Заказ.Номенклатура.Наименование Наименование,
	//|	ВЫБОР 	КОГДА Заказ.Размещение = Заказ.ИнтернетЗаказ.Склад 	ТОГДА ""2 рабочих дня""
	//|			КОГДА Заказ.Размещение ССЫЛКА Справочник.Склады 	ТОГДА ""1 неделя""
	//|			ИНАЧЕ ""6 недель"" КОНЕЦ  СрокПоставки,
	//|	ВЫБОР КОГДА Заказ.КоличествоОстаток = 0 ТОГДА 0 ИНАЧЕ Заказ.СуммаОстаток / Заказ.КоличествоОстаток КОНЕЦ Цена,
	//|	Заказ.КоличествоОстаток 		Количество,
	//|	Заказ.СуммаОстаток 				Стоимость
	//|ИЗ
	//|	РегистрНакопления.ИнтернетЗаказПокупателя.Остатки(,ИнтернетЗаказ = &ЭтотЗаказ) Заказ
	//|");
							
	//belova 07.06.2016 переписываю запрос, для упрощения убираю СрокПоставки - им никто не пользуется	
	...
	|ИЗ
	|	Документ.ИнтернетЗаказПокупателя.Товары Заказ ГДЕ Ссылка = &ЭтотЗаказ
	|");
	...
	Пока Выборка.Следующий() Цикл СуммаИтого = СуммаИтого + Выборка.Стоимость;
		...
		//belova 07.06.2016 убираю срок поставки, им никто не пользуется
		//|" + ?(СоСрокомПоставки, "<td align=""center"" valign=""top"" style=""padding: 10px 5px;font-family: Verdana,Geneva,sans-serif;font-size:12px;color:#333333;border-bottom:1px solid #bbbbbb;"">" + Выборка.СрокПоставки + "</td>", "") + "
...
//	|ICQ:  " + АСК + "<br>
	...
	ОбщиеФункции.ОповеститьПоПочте(АдресПочты , ТемаПисьма, 	ЗаголовокПисьма + 
																	БизнесПроцессы.ИнтернетЗаявка.ПолучитьДеталиЗаказа(Заказ, ДопРасходы, ОткрытиеЗаказа, СоСрокомПоставки) + 
