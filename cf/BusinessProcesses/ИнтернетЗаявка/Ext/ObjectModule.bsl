...
//	|				<td align=""left"" colspan=""3"" style=""width:640px""><img src=""http://spbmain.dlinkddns.com/orders_pdf/images/site2_verstka_0490_07.png"" width=""639"" height=""8"" /></td>
	...
	ИначеЕсли ПустаяСтрока(АдресПочты) Тогда
		...
		АдресПочты = ?( АдресПочты = "",Справочники.Контрагенты.ПолучитьЭлектроннуюПочтуПартнера(Заказ.Контрагент),АдресПочты);  //теперь почта из контрагента на крайний случай поищем и в партнере
	КонецЕсли;							
...
Процедура ПисьмоПодтверждениеЗаказаПредоплата(ТочкаМаршрутаБизнесПроцесса)
...
Процедура ОбработатьЗаказПриСозданииЗадач(ТочкаМаршрутаБизнесПроцесса, ФормируемыеЗадачи, Отказ)
	...
	Если Задача.ЭтоНовый() Тогда
		...
		//Если ЗначениеЗаполнено(Заказ.МВЗ) Тогда
		//	Задача.Склад = Заказ.МВЗ.Владелец;
		//ИначеЕсли 
		//	ЗначениеЗаполнено(Заказ.Партнер) Тогда
		//	Задача.Пользователь = Заказ.Партнер.ОсновнойМенеджер;
		//	Задача.Роль 		= Справочники.Роли.Менеджер;
		//КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаВариантовДоставки(ТочкаМаршрутаБизнесПроцесса)
	...
	ИначеЕсли Заказ.ВариантДоставки = "dellin_to_door" Тогда	
		...
		мГрузоперевозчик = Справочники.Партнеры.НайтиПоКоду("000000098"); //Деловые Линии
	...
	ИначеЕсли Заказ.ВариантДоставки = "dpd_to_door" Тогда
		ЗаказОбъект.ВариантДоставкиНов = Перечисления.ВариантДоставки.ДоставкаДоКлиента;
		мГрузоперевозчик = Справочники.Партнеры.НайтиПоКоду("000000853"); //DPD
		Если НЕ мГрузоперевозчик.Пустая() Тогда
			ЗаказОбъект.Грузоперевозчик = мГрузоперевозчик;	
		КонецЕсли;
	ИначеЕсли Заказ.ВариантДоставки = "ems_to_door" Тогда
		ЗаказОбъект.ВариантДоставкиНов = Перечисления.ВариантДоставки.ДоставкаДоКлиента;
		мГрузоперевозчик = Справочники.Партнеры.НайтиПоКоду("000043877"); //EMS
		Если НЕ мГрузоперевозчик.Пустая() Тогда
			ЗаказОбъект.Грузоперевозчик = мГрузоперевозчик;	
		КонецЕсли;
