...
&НаКлиенте
Процедура КоммерческиеУсловияСогласованы(Команда)
	
	Если 	КоммерческиеУсловияСогласоватьНаСервере(Истина, Ложь, Ложь) И
...
&НаКлиенте
Процедура КоммерческиеУсловияНеСогласованы(Команда)
	
	Если 	КоммерческиеУсловияСогласоватьНаСервере(Ложь, Ложь, Ложь) И
		ВыполнитьЗадачуНаСервере() Тогда
...
&НаСервере
Функция КоммерческиеУсловияСогласоватьНаСервере(РезультатСогласования, ВернутьИнициатору, ВернутьНаШагНазад)
	
	Процесс = Объект.БизнесПроцесс.ПолучитьОбъект();
	Процесс.СогласованоСИсполнительнымДиректором = РезультатСогласования;
...
Процедура ВернутьНаШагНазад(Команда)
	...
	Если 	КоммерческиеУсловияСогласоватьНаСервере(Ложь, Ложь, Истина) И
		ВыполнитьЗадачуНаСервере() Тогда
...
Процедура ВернутьИнициатору(Команда)
	...
	Если 	КоммерческиеУсловияСогласоватьНаСервере(Ложь, Истина, Ложь) И
		ВыполнитьЗадачуНаСервере() Тогда
