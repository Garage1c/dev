...
&НаКлиенте
Процедура КонтрагентПрошелПроверку(Команда)
	
	Если 	ПроверкаКонтрагентаНаСервере(Истина, Ложь) И
...
&НаКлиенте
Процедура КонтрагентНеПрошелПроверку(Команда)
	
	Если 	ПроверкаКонтрагентаНаСервере(Ложь, Ложь) И
		ВыполнитьЗадачуНаСервере() Тогда
...
&НаСервере
Функция ПроверкаКонтрагентаНаСервере(РезультатПроверки, ВернутьИнициатору)
	
	Процесс = Объект.БизнесПроцесс.ПолучитьОбъект();
	Процесс.КлиентПрошелПроверку = РезультатПроверки;
...
Процедура ВернутьИнициатору(Команда)
	...
	Если 	ПроверкаКонтрагентаНаСервере(Ложь, Истина) И
		ВыполнитьЗадачуНаСервере() Тогда
