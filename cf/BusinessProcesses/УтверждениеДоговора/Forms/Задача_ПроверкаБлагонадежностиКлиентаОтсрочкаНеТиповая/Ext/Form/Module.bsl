...
Процедура КонтрагентПрошелПроверку(Команда)
	...
	Если 	ПроверкаКонтрагентаНаСервере(Истина) И
...
Процедура КонтрагентНеПрошелПроверку(Команда)
	...
	Если 	ПроверкаКонтрагентаНаСервере(Ложь) И
		ВыполнитьЗадачуНаСервере() Тогда
...
&НаСервере
Функция ПроверкаКонтрагентаНаСервере(РезультатПроверки)
	
	Процесс = Объект.БизнесПроцесс.ПолучитьОбъект();
	Процесс.КлиентПрошелПроверку = РезультатПроверки;
	Процесс.СогласованоСИсполнительнымДиректором = РезультатПроверки;
