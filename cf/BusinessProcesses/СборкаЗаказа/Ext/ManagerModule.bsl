...
Процедура ЗаполнитьФормуПоБП(Форма, СсылкаПроцесс, СсылкаЗадача = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СсылкаПроцесс) Тогда
		...
		|ВЫБРАТЬ 	
		|	Заказ, Заказ.Партнер Партнер, Заказ.Контрагент Контрагент, Склад, РеализацияТоваров, ЯчеистаяСборка, ПродолжатьСобирать
		|ПОМЕСТИТЬ
		|	Шапка
		|ИЗ
		...
		|ВЫБРАТЬ 	
		|	Заказ, Партнер, Контрагент, Склад, РеализацияТоваров, ЯчеистаяСборка, ПродолжатьСобирать
		|ИЗ
		|	Шапка
		|");
