...
Функция СформироватьПровестиСборкуИВыполнитьЗадачу()
	...
	Если ЗначениеЗаполнено(Заказ) И (ТипЗнч(Заказ) = Тип("ДокументСсылка.ИнтернетЗаказПокупателя") Или ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказПокупателя")) Тогда 
		Если Не ЗначениеЗаполнено(Заказ.МВЗ) Тогда 						
			Если ЗначениеЗаполнено(Заказ.МенеджерЗаказа.Почта) Или ЗначениеЗаполнено(Заказ.Контрагент.ОсновнойМенеджер.Почта) Тогда 
				ОтправитьПисьмоОНенайденномТоваре();		
			КонецЕсли;                          		
		КонецЕсли;
	КонецЕсли;
	//Нэти-Конец
...
//Нэти-Начало Гашников Александр 24.05.2017, заявка 73	
Процедура ОтправитьПисьмоОНенайденномТоваре()
			
	Кому = "";
	Кому = Заказ.МенеджерЗаказа.Почта;
	МенеджерЗаказа = Заказ.МенеджерЗаказа;
	
	Если Не ЗначениеЗаполнено(Кому) Тогда 
		Кому = Заказ.Контрагент.ОсновнойМенеджер.Почта;
		МенеджерЗаказа = Заказ.Контрагент.ОсновнойМенеджер; 
	КонецЕсли;	
	...
	ТекстПисьмаЗаголовок = "Уважаемый(ая) " + МенеджерЗаказа + "!" + "<BR><BR> Спешим сообщить Вам, что следующие позиции были ненайдены в заказе № " + Заказ.Номер + ":<BR><BR>";	
	ТекстПисьмаТело = "";
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Объект.Выполнена И Не Объект.Ссылка.БизнесПроцесс.СборочныйЛист.Проведен Тогда
		...
		//Собран = Перечисления.ДействияПоСборкеЗаказа.Собрать;
		
		Для Каждого Строка Из Товары Цикл 
			//Если Склад = Строка.Размещение И СтрНачинаетсяС(Строка.Состояние, "Собирается") Тогда 
			...
			Иначе	Строка.Количество 	= 0; КонецЕсли; КонецЦикла; КонецЕсли;
...
Процедура ПроверитьVIPКонтрагента()
	...
	//Если у сборочного листа есть контролер, то "Проверить контролеру" ИСТИНА
	Если НЕ Объект.БизнесПроцесс.СборочныйЛист.Контролер.Пустая() и НЕ ПроверитьКонтролеру Тогда 
		ПроверитьКонтролеру = Истина;
	КонецЕсли;	
	
	//Если статус ИСТИНА ПроверитьКонтролеру уже установлен
	Если ПроверитьКонтролеру Тогда 
		Элементы.Контролер.Доступность = Истина;
		Возврат;
	КонецЕсли;
	
	//Вычисляем статус контрагента (если контрагент не VIP) согласно условиям и устанавливаем признак "Проверить контролеру"
	СтатусVIP = Ложь;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	// Отправим в буфер таблицу
	
	//Таблица = Товары.Выгрузить();
	
	// Если Выполняется тогда очистим буфер ячеек, если нет тогда запомним ячейки
	
	//Если ТекущийОбъект.БизнесПроцесс.Склад.Ячеестый Тогда
	//	
	//	ВыполнитьЗадачу = Ложь;
	//	ПараметрыЗаписи.Свойство("ВыполнитьЗадачу", ВыполнитьЗадачу);
	//	ВыполнитьЗадачу = ВыполнитьЗадачу = Истина;
	//	
	//	Если 	Не ВыполнитьЗадачу И 
	//			Не ФункцииФормДокументов.ЗапомнитьЯчейкиВЗадаче(Объект.Ссылка, Товары) Тогда
	//		Отказ = Истина; КонецЕсли; КонецЕсли;
	
	// Запишем сборочный лист
	
	//Если Не (ПараметрыЗаписи.Свойство("НеСоздаватьСборку") И ПараметрыЗаписи.НеСоздаватьСборку) Тогда
		//ФункцииФормЗадач.ЗаписатьШапкуСборочногоЛиста(ЭтаФорма, ТекущийОбъект, Отказ); КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПроверитьКонтролеру и ПолучитьСборкуЧерезКонтролера(Склад) и НЕ ЗначениеЗаполнено(Контролер) Тогда
...
&НаСервере
Функция ПолучитьСборкуЧерезКонтролера(СкладСсылка)
	 Возврат СкладСсылка.СборкаЧерезКонтролера;
КонецФункции	
