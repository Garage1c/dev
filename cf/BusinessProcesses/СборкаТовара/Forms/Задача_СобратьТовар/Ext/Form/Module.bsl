...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	//Если Не (ПараметрыЗаписи.Свойство("НеСоздаватьСборку") И ПараметрыЗаписи.НеСоздаватьСборку) Тогда
		...
		Сборка.Заполнить(Новый Структура("Шапка, Товары, ТоварыУсловие, ТоварыАлгоритм", 
				Новый Структура("Заказ, Склад, Вес, Объем, КоличествоПозиций, ТипСборочногоЛиста, ЯчейкаСобранногоТовара, КоличествоМест, ДатаСборки", Заказ, Склад, Вес, Объем, КоличествоПозиций, ТипСборочногоЛиста, ЯчейкаСобранногоТовара, КоличествоМест, ДатаСборки),
				Товары,
				" Строка.Состояние <> Перечисления.СостояниеСборкиЗаказа.Собрано И Строка.Количество > 0",
				"НовСтрока.Количество = Строка.Собрано; НовСтрока.Собрать = Строка.Количество;")); 
...
Процедура ПечатьНовогоСборочногоЛиста(Команда)
	Если Модифицированность Или Объект.Ссылка.Пустая() Тогда
		...
		Возврат;
	КонецЕсли;
	ПечатьНаСервере.ПолучитьСборочныйЛист(Объект.БизнесПроцесс).Показать();
