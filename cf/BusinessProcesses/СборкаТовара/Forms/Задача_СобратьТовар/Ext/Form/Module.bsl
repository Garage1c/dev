...
Процедура ПрочитатьРеквизиты()
	...
	ПроставитьДопРеквизитыВТаблице();
...
&НаСервере
Процедура ПроставитьДопРеквизитыВТаблице()
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Объединение
	ЗаказыДляОбъединенияПриОткрытии = Параметры.ЗаказыДляОбъединения;
...
Процедура ПриОткрытии(Отказ)
	...
	Если Не Отказ И ЗаказыДляОбъединенияПриОткрытии.Количество() Тогда
		ПодключитьОбработчикОжидания("ОткрытьФормуОбъекдиненияСопределеннымиЗаказами", 0.1, Истина); КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	СборочныйЛист = Сборка.Ссылка;
	
	Если Объект.БизнесПроцесс.СборочныйЛист.Пустая() Тогда
		БПОбъект = Объект.БизнесПроцесс.ПолучитьОбъект();
		БПОбъект.СборочныйЛист = СборочныйЛист;
		Если ОбщиеФункции.ЗаписатьОбъектИСообщитьЕслиОшибка(БПОбъект) Тогда
			Отказ = Истина; КонецЕсли; КонецЕсли;
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Не ЗаписатьОбъединение() Тогда Отказ = Истина КонецЕсли;
...
Функция Заказы() Экспорт
	...
	Для Каждого Строка Из Товары Цикл 
		//смЗаказ = ?(ЗначениеЗаполнено(Строка.Заказ), Строка.Заказ, Заказ);
		смЗаказ = Строка.Заказ;
		Если текЗаказы.Найти(смЗаказ) = Неопределено Тогда текЗаказы.Добавить(смЗаказ) КонецЕсли; КонецЦикла;
...
&НаСервере
Функция ЗаписатьОбъединение()
	
	// Установим всем БП сборочный лист у которых нет
	
	БПСборок = ЗаказыКоллекцияСервер.ПолучитьЗадачиБизнесПроцессыСОбщимСборочникомВЗаказах(Заказ, Склад, СборочныйЛист);
	
	Для Каждого Строка Из БПСборок Цикл
		
		Если Строка.Задача <> Объект.Ссылка И Не Строка.Задача.Параллельная Тогда
			ЗадОб = Строка.Задача.ПолучитьОбъект();
			ЗадОб.Параллельная = Истина;
			Если Не ОбщиеФункции.ЗаписатьОбъектИСообщитьЕслиОшибка(ЗадОб) Тогда
				ОбщиеФункции.СообщитьТекст("Ошибка при записи параллельности задачи");
				Возврат Ложь; КонецЕсли; КонецЕсли;
		
		Если Строка.СборочныйЛист.пустая() Тогда
			БПОб = Строка.БизнесПроцесс.ПолучитьОбъект();
			БПОб.СборочныйЛист = СборочныйЛист;
			Если Не ОбщиеФункции.ЗаписатьОбъектИСообщитьЕслиОшибка(БПОб) Тогда
				ОбщиеФункции.СообщитьТекст("Ошибка при присвоении сборочного листа бизнес процессу");
...
Процедура ОбновитьДанныеПоТоварамНаСервере()
	...
	БизнесПроцессы.СборкаТовара.ОбновитьТаблицуТоваров(Товары, Объект.БизнесПроцесс, Заказы());
	ПодготовитьТаблицу();
	ПроставитьДопРеквизитыВТаблице();
	ОбновитьПредставлениеРеквизитовОбъединенныхЭлементов();
...
&НаКлиенте
Процедура ОткрытьФормуОбъекдиненияСопределеннымиЗаказами() Экспорт
	...
	ЗаказыКоллекция.ВыбратьЗаказыДляОбъединения(ЭтаФорма, Новый Структура("ТекстФильтраЗароса, Параметры",
		...
		ЗаказыДляОбъединенияПриОткрытии);
