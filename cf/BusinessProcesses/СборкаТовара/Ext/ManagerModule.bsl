...
Функция ПолучитьТоварыИзПроводокСборочногоЛиста(СборочныйЛист)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ Ячейка, Номенклатура, Собрать Количество, Количество Собрано, НеНайдено, Сборщик ИЗ Документ.СборочныйЛист.Товары ГДЕ Ссылка = &СборочныйЛист");
...
Процедура ЗаполнитьФормуПоБП(Форма, СсылкаПроцесс, СсылкаЗадача = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СсылкаПроцесс) Тогда
		...
		|ВЫБРАТЬ 	Заказ, Заказ.Контрагент Контрагент, Склад
		...
		ЗаполнитьЗначенияСвойств(Форма, КонвертацияТипов.ПолучитьСтруктуруИзСТрокиТаблицыЗначений(Запрос.Выполнить().Выгрузить()));

		// Заполним таблицу
		
		ТабТов = ?(СсылкаПроцесс.СборочныйЛист.Проведен, 
					ПолучитьТоварыИзПроводокСборочногоЛиста(СсылкаПроцесс.СборочныйЛист),
					Заказы.ПолучитьСостояниеТоваров(СсылкаПроцесс, ?(СсылкаЗадача <> Неопределено И СсылкаЗадача.Выполнена, СсылкаЗадача.ДатаВыполнения, ТекущаяДата())));
