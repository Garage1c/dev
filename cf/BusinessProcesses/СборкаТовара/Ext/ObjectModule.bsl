...
Процедура СобратьТоварУОбъединенныхСборокОбработка(ТочкаМаршрутаБизнесПроцесса)
	...
	БПБезСборки = ЗаказыКоллекцияСервер.ПолучитьЗадачиБизнесПроцессыСОбщимСборочникомВЗаказах(Заказ, Склад, СборочныйЛист);
	Для Каждого Строка Из БПБезСборки Цикл
		
	    Если Строка.СборочныйЛист.Пустая() Тогда // если у БП нет сборочника тогда устанавливаем его
			БПОбъект 				= Строка.БизнесПроцесс.ПолучитьОбъект();
			БПОбъект.СборочныйЛист 	= СборочныйЛист;
			БПОбъект.Записать(); КонецЕсли;
		
		Строка.Задача.ПолучитьОбъект().ВыполнитьЗадачу() КонецЦикла;
