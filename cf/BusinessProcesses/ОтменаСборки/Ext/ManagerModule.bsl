...
Процедура ЗаполнитьФормуПоБП(Форма, СсылкаПроцесс, СсылкаЗадача = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СсылкаПроцесс) Тогда
		...
		|ВЫБРАТЬ 	
		|	Заказ, Заказ.Контрагент Контрагент, Склад
		|ПОМЕСТИТЬ
		|	Шапка
		|ИЗ
		...
		Запрос.Текст = "Выбрать Номенклатура, СборочныйЛист, СборочныйЛист.ЯчейкаСобранногоТовара КАК ЯчейкаСобранногоТовара, КоличествоОстаток Количество из РегистрНакопления.ЗаказыНаРазборку.Остатки(, Заказ = &Заказ и Склад = &Склад )";
