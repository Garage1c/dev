...
Процедура ЗаполнитьФормуПоБП(Форма, СсылкаПроцесс, СсылкаЗадача = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СсылкаПроцесс) Тогда
		...
		|ВЫБРАТЬ 	
		|	Заказ, Заказ.Контрагент Контрагент, СборочныйЛист, СборочныйЛист.ЯчейкаСобранногоТовара ЯчейкаСобранногоТовара, Склад
		|ПОМЕСТИТЬ
		|	Шапка
		|ИЗ
		...
		|ВЫБРАТЬ 	
		|	Заказ, Контрагент, СборочныйЛист, ЯчейкаСобранногоТовара, Склад
		|ИЗ
		|	Шапка;
		...
		// Заполним таблицу
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("Заказ",ВыборкаШапки.Заказ);
		Запрос.УстановитьПараметр("СборочныйЛист",ВыборкаШапки.СборочныйЛист);
		Запрос.УстановитьПараметр("Склад",ВыборкаШапки.Склад);
		
		Запрос.Текст = "Выбрать Номенклатура, Упаковка, КоличествоОстаток Количество из РегистрНакопления.ТоварыСобранные.Остатки(, Заказ = &Заказ и СборочныйЛист = &СборочныйЛист и Склад = &Склад )";
		ТаблТоваров = Запрос.Выполнить().Выгрузить();
		
		Форма.Товары.Загрузить(ТаблТоваров);
