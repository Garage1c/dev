...
Процедура ЗаполнитьФормуПоБП(Форма, СсылкаПроцесс, СсылкаЗадача = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СсылкаПроцесс) Тогда
		...
		|	Заказ.Партнер Партнер, Заказ.Организация Организация, Заказ.БанковскийСчетОрганизации БанковскийСчетОрганизации,
		...
		|ВЫБРАТЬ 	
		|	ДатаПоставки, ДатаАктуальности, ДатаЗаказаКлиента, НомерЗаказаКлиента, Автор, ТипЦен, Заказ, Склад, КонтактноеЛицо, Контрагент, СуммаВключаетНДС, Валюта,  Партнер, Организация, БанковскийСчетОрганизации, 
