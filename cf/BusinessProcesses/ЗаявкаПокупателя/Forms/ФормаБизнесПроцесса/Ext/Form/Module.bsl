...
Процедура УправлениеВидимостьюДоступностью() Экспорт
	...
	Элементы.Партнер.		ТолькоПросмотр = Не РазрешеноРедактировать ИЛИ ЕстьОтгруженные;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Если Не Параметры.Контрагент.Пустая() Тогда
			...
			ПартнерПриИзмененииНаСервере(); КонецЕсли;
		
		Если Не Параметры.Контрагент.Пустая() Тогда
			
			Контрагент = Параметры.Контрагент;
			КонтрагентПриИзмененииНаСервере(); КонецЕсли;
...
#КонецОбласти
	...
	ДолгПарнера = ?(Объект.Ссылка.Пустая(), 0, ДенежныеСредства.ПолучитьДолгПартнера(Контрагент));
...
Процедура ОбработкаВыбораВариантаПроведения(Результат, ПараметрыОповещения) Экспорт
	...
	Если Открыта() И ЗакрытьЗаявку Тогда Если Вопрос("Закрыть форму заявки?", РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда Закрыть();КонецЕсли;КонецЕсли;
...
//&НаКлиенте
//Процедура БыстраяПродажа_уд(Команда)
//	
...
&НаСервере
Процедура ПартнерПриИзмененииНаСервере(СтруктураКолонокТовары = Неопределено, СуммаВключаетНДС_ПоУмолчанию = Ложь)
	...
	Если СтруктураКолонокТовары <> Неопределено Тогда 
		...
		Если Товары.Количество() Тогда
			РаботаСНоменклатурой.ПересчитатьАкциюДляТоваров(Контрагент, ТипЦен, Товары, СтруктураКолонокТовары, Валюта, Ложь); КонецЕсли;
...
Процедура ПартнерПриИзменении(Элемент)
	...
	ПартнерПриИзмененииНаСервере(СтруктураКолонокТовары, СуммаВключаетНДС_ПоУмолчанию);	
	СохранитьТекущиеЗначенияПараметров();
