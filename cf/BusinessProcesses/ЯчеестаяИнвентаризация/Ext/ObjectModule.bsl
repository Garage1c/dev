...
Процедура ОбработатьНедостачи()
	...
	|;
	|
	|Выбрать Рез.Номенклатура, Рез.ДокументРезерва Заказ, Рез.ДокументРезерва.Дата Дата, Рез.КоличествоОстаток - isnull(Всбо.КоличествоОстаток,0) - isnull(Соб.КоличествоОстаток,0) КоличествоОстаток 
	|из
	|РегистрНакопления.ТоварыВРезерве.Остатки(,Номенклатура в (Выбрать Номенклатура из НовТаб) и Размещение = &Склад) Рез
	|
	|Левое соединение
	|РегистрНакопления.ТоварыВСборке.Остатки(,Номенклатура в (Выбрать Номенклатура из НовТаб) и Склад = &Склад) Всбо
	|По Рез.Номенклатура = Всбо.Номенклатура и Рез.ДокументРезерва = Всбо.Заказ
	|
	|Левое соединение
	|РегистрНакопления.ТоварыСобранные.Остатки(,Номенклатура в (Выбрать Номенклатура из НовТаб) и Склад = &Склад) Соб
	|По Рез.Номенклатура = Соб.Номенклатура и Рез.ДокументРезерва = Соб.Заказ
	|
	|ГДЕ Рез.КоличествоОстаток - isnull(Всбо.КоличествоОстаток,0) - isnull(Соб.КоличествоОстаток,0) > 0
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Списать>0 Тогда
			//это значит что товар собирается либо собран. Снимать резерв нельзя. 
			//Транзакция в этом случае не выполнится, так как не проведется перемещение из за отсутствия резервов.
			//Пока не придумал, что с этим делать.
			Сообщить("Не удалось сделать снятие резерва из за того, что товар уже собран или собирается. "+Выборка.Номенклатура.Артикул+" : "+Выборка.Номенклатура+" : требуется снять "+Выборка.Списать+", возможно снять "+(Выборка.Списать-Списать)+ " шт");
	...
	Если ТабСн.Количество() Тогда
		...
		//ТекстСообщения="";
		//Для Каждого Стр Из ТабСн Цикл
		//	ТекстСообщения = ТекстСообщения + Стр.Заказ.Автор + "  " + Стр.Заказ + "  " + Стр.Размещение  + "  "+Стр.Номенклатура.Артикул+ "  " + Стр.Номенклатура + "   " + Стр.Количество + Символы.ПС;
		//КонецЦикла;
		//
		//ТекстСообщения = "Было произведено снятие резервов. Пожалуйста уведомите ответственных лиц"+Символы.ПС+ТекстСообщения;
...
Процедура ОбработатьИзлишки()
	...
	Для Каждого Стр Из Товары Цикл
		Если Стр.КоличествоРасхождение > 0 Тогда
			НовСтр = НовТаб.Добавить();
			ЗаполнитьЗначенияСвойств(НовСтр,Стр);
			НовСтр.ЯчейкаОтправитель = ?(ЗначениеЗаполнено(Стр.ЯчейкаКорреспондент),Стр.ЯчейкаКорреспондент, Склад.ЯчейкаНеНайдено);
