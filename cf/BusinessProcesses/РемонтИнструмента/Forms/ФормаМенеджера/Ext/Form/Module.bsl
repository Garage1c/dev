...
Функция ЗаполнитьДанныеТаблицы()
    ...
    |	Зад.БизнесПроцесс.Партнер КАК Партнер,
    |	Зад.Пользователь,
	|	Зад.Заказ.Сумма	Сумма,
	|	ВЫБОР КОГДА Зад.БизнесПроцесс.Партнер.ТипЦен В (ВЫБРАТЬ Значение ИЗ Константа.ТипЦенРозница) ТОГДА ""Ждать оплату"" ИНАЧЕ ""Отсрочка"" КОНЕЦ Действие,
	|	ВЫБОР КОГДА Зад.БизнесПроцесс.Партнер.ТипЦен В (ВЫБРАТЬ Значение ИЗ Константа.ТипЦенРозница) ТОГДА 0 ИНАЧЕ 1 КОНЕЦ Отсрочка
    |ИЗ
...
Процедура ИнформацияПоОплатеНажатие(Элемент)
	...
	ОткрытьФорму("Обработка.ДолгиПоОплате_Управление.Форма.Управление", Новый Структура("Организация, Партнер", ТекущиеДанные.Организация, ТекущиеДанные.Партнер));
КонецПроцедуры
