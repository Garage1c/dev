...
Функция СоздатьДокумент(ДокументСтрока, СписокСообщений)
		...
		НовыйДокумент.Партнер = ДокументСтрока.Контрагент.Партнер;
		...
		//отдел заполняем только, если основной менеджер партнера не является свободным лидом. Если свободный лид, то отдел заполняется из автора заказа соответственно, после разнесения.
		Если ТипЗнч(НовыйДокумент) = Тип("ДокументОбъект.ПлатежноеПоручениеВходящее") тогда
			Если ЗначениеЗаполнено(НовыйДокумент.Партнер) Тогда
				ОснМен=НовыйДокумент.Партнер.ОсновнойМенеджер;
...
Функция ЗаполнитьСсылочныеДанные(СтрокаДокумента, СписокСообщений)
	...
	Если НЕ ПустаяСтрока(СтрокаДокумента.ИННКонтрагента) Тогда
		Запрос = Новый Запрос("
		|ВЫБРАТЬ Ссылка, Партнер.СтатьяДвиженияДенежныхСредств СтатьяДДС ИЗ Справочник.Контрагенты ГДЕ НЕ ПометкаУдаления И ИНН = &ИНН" + ?(СтрокаДокумента.КППКонтрагента = "0" ИЛИ СтрокаДокумента.КППКонтрагента = "", "", " И КПП = &КПП") + "; " + " 
		|ВЫБРАТЬ Ссылка, Партнер.СтатьяДвиженияДенежныхСредств СтатьяДДС ИЗ Справочник.Контрагенты ГДЕ НЕ ПометкаУдаления И ИНН = &ИНН");
	...
	Если БанковскийОрдер и 	НЕ ЗначениеЗаполнено(СтрокаДокумента.ПолучательИНН) Тогда
		...
		СтрокаДокумента.Контрагент=Справочники.Контрагенты.НайтиПоНаименованию(СтрокаДокумента.Получатель1);
		Если  СтрокаДокумента.Контрагент="" Тогда
			СтрокаДокумента.ОписаниеОшибок="Не найден контрагент " +  СтрокаДокумента.Получатель1;
		Иначе
	...
	Иначе
		СтрокаДокумента.ОписаниеОшибок = СтрокаДокумента.ОписаниеОшибок + "Банковский счет " + Строка(СтрокаДокумента.СчетКонтрагентаДляПоиска) + " партнера не найден. Будет создан новый.";
		СтрокаДокумента.Предупреждение = Истина;
		//Возврат Ложь;
	КонецЕсли;
