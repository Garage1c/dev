...
Функция ЗаполнитьСсылочныеДанные(СтрокаДокумента, СписокСообщений)
	...
	Если НЕ ПустаяСтрока(СтрокаДокумента.ИННКонтрагента) Тогда
		Запрос = Новый Запрос("
		|ВЫБРАТЬ Ссылка, Контрагент.СтатьяДвиженияДенежныхСредств СтатьяДДС ИЗ Справочник.Контрагенты ГДЕ НЕ ПометкаУдаления И ИНН = &ИНН" + ?(СтрокаДокумента.КППКонтрагента = "0" ИЛИ СтрокаДокумента.КППКонтрагента = "", "", " И КПП = &КПП") + "; " + " 
		|ВЫБРАТЬ Ссылка, Контрагент.СтатьяДвиженияДенежныхСредств СтатьяДДС ИЗ Справочник.Контрагенты ГДЕ НЕ ПометкаУдаления И ИНН = &ИНН");
