...
Функция ПолучитьПартнераКонтакта(Контакт)
	...
	//	 Возврат ?(ЗначениеЗаполнено(Контакт.Партнер),Контакт.Партнер,Контакт);
	//ИначеЕсли
	//	 ТипКонтакта = Тип("СправочникСсылка.КонтактныеЛица") Тогда
	//	  
	//	 Возврат ?(ТипЗнч(Контакт.Владелец) = Тип("СправочникСсылка.Партнеры"),Контакт.Владелец,?(ЗначениеЗаполнено(Контакт.Владелец.Партнер),Контакт.Владелец.Партнер,Контакт.Владелец));//Контакт.Владелец.Партнер;
	// КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Партнер = Неопределено ИЛИ Объект.Партнер.Пустая() Тогда Отказ = Истина;
