...
&НаСервере
Функция ЗагрузитьДоларКЕвроНаСервере(стрОшибки)
	
	Возврат РеквизитФормыВЗначение("Объект").ЗагрузитьКурсыДолараКЕвро(стрОшибки);
	
КонецФункции
&НаКлиенте
Процедура ЗагрузитьДоларКЕвро(Команда)
	
	СтрОшибки = "";
	Если ЗагрузитьДоларКЕвроНаСервере(СтрОшибки) Тогда
		ПоказатьПредупреждение(,"Загружено"); 
	Иначе
