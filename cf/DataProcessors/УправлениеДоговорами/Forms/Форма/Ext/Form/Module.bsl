...
Функция ПолучитьСтруктуруРасшифровки(Выборка, Действие)
	...
	Возврат Новый Структура("Действие, Организация, НомерДоговора, Контрагент, ДокументВорд, ДатаНачала, ДатаОкончания, НомерСтроки, НомерДокумента, Основной", 
			Действие,
			Выборка.Организация, 
...
Процедура ОбновитьТаблицу()
	...
	|" + ?(Менеджер.Пустая(), "",					" И Контр.ОсновнойМенеджер = &Менеджер") + "
	...
	|" + ?(ПустаяСтрока(Наименование), "",			" И Контр.Наименование ПОДОБНО ""%" + Наименование + "%"")") + "
	|
	|УПОРЯДОЧИТЬ ПО
	|	Контрагент.Наименование, НомерСтроки, НомерДокумента
	|
	...
	Иначе
		стИмя = Элементы.Таблица.ТекущаяОбласть.Имя; 
	КонецЕсли;
	...
	Пока ВыборкаПартнеров.Следующий() Цикл 
		Если ПустаяСТрока(ВыборкаПартнеров.Контрагент.Код) Тогда 
			Продолжить; 
		КонецЕсли;
		...
		Если ТекущаяДата() - ВремяНачала > НеБольшеСекунд Тогда
			ОбщиеФункции.СообщитьТекст("Выведены не все Контрагенты, так как список оказался слишком большим,
			|рекомендуется установить фильтры, чтобы сократить список."); 
			Прервать; 
		КонецЕсли;
		...
		Если Не ЕстьДоговор И ОдинКонтрагент Тогда
			...
			Если ЕстьДоговор Тогда Таблица.НачатьАвтогруппировкуСтрок(); текГруппа = текГруппа + 1;
			КонецЕсли;
			Таблица.Вывести(ОбластьПартнерБезДокументов, текГруппа, "Контрагент");
		...
		Если Не ПропуститьВыводПартнера Тогда
			...
			Таблица.Вывести(Область, текГруппа, "Контрагент"); 
		КонецЕсли;
		...
		//	КонецЕсли;
			...
			Выборка 		= ВыборкаПартнеров.Выбрать();
			ОдинДокумент 	= Выборка.Количество() = 1;
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Не ЗначениеЗаполнено(Выборка.ДокументВорд) Тогда Область.Рисунки.Удалить(РисунокОткрыть);
				КонецЕсли; // Спрячем картинку открытия
				...
				Иначе
					Область.Рисунки["Удалить" + Приставка + ПриставкаКартинки].Расшифровка = ПолучитьСтруктуруРасшифровки(Выборка, "Удалить");
				КонецЕсли;
				
				Таблица.Вывести(Область, текГруппа + 1, "Документ");
			КонецЦикла; 
	...
	Иначе
		Элементы.Таблица.ТекущаяОбласть = Таблица.НайтиТекст(стКонтра);
	КонецЕсли;
...
Процедура ОбновитьТаблицу_Нов()
	...
	Пока ВыборкаПартнеров.Следующий() Цикл Если ПустаяСТрока(ВыборкаПартнеров.Контрагент.Код) Тогда Продолжить; КонецЕсли;
		...
		Иначе
			...
			Область = ?(ОдинКонтрагент, ОбластьПартнерОрганизация, ОбластьПартнер);
			Область.Параметры.Заполнить(ВыборкаПартнеров);
			Таблица.Вывести(Область, текГруппа, "Контрагент"); КонецЕсли;
	...
	Если ТипЗнч(Расшифровка) = Тип("Структура") Тогда СтандартнаяОбработка = Ложь;
		...
		Если Расшифровка.Действие = "Открыть" Тогда
			...
			ДиалогиСПользователем.ОткрытьДокументВордДляПросмотра(
						...
						Новый Структура("Организация, Контрагент, ИндексДоговора", 
											Расшифровка.Организация, 
											Расшифровка.Контрагент, 
