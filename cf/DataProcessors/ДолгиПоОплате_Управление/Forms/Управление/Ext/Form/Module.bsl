...
//	|	Заказы.Заказ,
//	|	Заказы.Сумма,
//	|	ЕСТЬNULL(Долг.СуммаОплаты, 0)		СуммаОплаты,
//	//|	ЕСТЬNULL(Авансы.СуммаАванса, 0)		СуммаАванса,
//	|	0 СуммаАванса,
//	|	ЕСТЬNULL(Отгрузки.СуммаОборот, 0) 	СуммаОтгрузки,
//	|	ВЫБОР КОГДА Заказы.Сумма = 0 ТОГДА 0 КОГДА ЕСТЬNULL(Долг.СуммаОплаты, 0) = 0 ТОГДА 1 КОГДА ЕСТЬNULL(Долг.СуммаОплаты, 0) = Заказы.Сумма ТОГДА 3 ИНАЧЕ 2 КОНЕЦ 	КАК КартинкаОплаты,
//	|	ВЫБОР КОГДА ЕСТЬNULL(Отгрузки.СуммаОборот, 0) = 0 ТОГДА 0 КОГДА ЕСТЬNULL(Отгрузки.СуммаОборот, 0) = Заказы.Сумма ТОГДА 2 ИНАЧЕ 1 КОНЕЦ 	КАК КартинкаОтгрузки
//	|ИЗ
//	|(
//	|	ВЫБРАТЬ
//	|		Жур.Ссылка 			Заказ,
//	//|		ВЫБОР КОГДА Жур.Ссылка ССЫЛКА Документ.ЗаказНаряд ТОГДА Жур.Ссылка.Сумма ИНАЧЕ ЕСТЬNULL(ЕСТЬNULL(РегПок.СуммаПриход, РегИнт.СуммаПриход), 0) КОНЕЦ Сумма, //belova{ Жур.Ссылка.Сумма) 	Сумма,}
//	|		ВЫБОР КОГДА Жур.Ссылка ССЫЛКА Документ.ЗаказНаряд ТОГДА Жур.Ссылка.Сумма ИНАЧЕ ЕСТЬNULL(ЕСТЬNULL(РегПок.СуммаПриход, РегИнт.СуммаПриход), 0) + 
//	| ВЫБОР КОГДА Жур.Ссылка.СуммаВключаетНДС ТОГДА 0 ИНАЧЕ " + Заказы.ПолучитьСуммуНДСВЗапросе("РегПок",,"Жур.Ссылка", Истина) + " КОНЕЦ КОНЕЦ Сумма,
//	|
//	|		Жур.Дата
//	|	ИЗ
...
//	|
//	|ЛЕВОЕ СОЕДИНЕНИЕ
//	//|	(	ВЫБРАТЬ Заказ, СУММА(СуммаРасход) СуммаОплаты
//	//|		ИЗ		РегистрНакопления.ДолгиПоОтгрузкам.ОстаткиИОбороты(,,РЕГИСТРАТОР,,Организация = &Организация И Партнер = &Партнер) Долг
//	//|		ГДЕ		Регистратор ССЫЛКА Документ.ПлатежноеПоручениеВходящее ИЛИ
//	//|				Регистратор ССЫЛКА Документ.ПриходныйКассовыйОрдер
//	//|		СГРУППИРОВАТЬ ПО Заказ
//	//|	) Долг
//	|	(	ВЫБРАТЬ Заказ, СуммаРасход СуммаОплаты
//	|		ИЗ		РегистрНакопления.ДолгиПоЗаказам.Обороты(,,,Организация = &Организация И Партнер = &Партнер) Долг
//	|	) Долг
//	|ПО
//	|	Заказы.Заказ = Долг.Заказ
//	|
//	//авансы не имеют смысла в связи с заказами
//	//|ЛЕВОЕ СОЕДИНЕНИЕ
//	//|	(	ВЫБРАТЬ Регистратор Заказ, СУММА(СуммаПриход) СуммаАванса
//	//|		ИЗ		РегистрНакопления.ДолгиПоОтгрузкам.ОстаткиИОбороты(,,РЕГИСТРАТОР,,Организация = &Организация И Партнер = &Партнер) Долг
//	//|		ГДЕ		(	Регистратор ССЫЛКА Документ.ЗаказПокупателя ИЛИ
//	//|					Регистратор ССЫЛКА Документ.ИнтернетЗаказПокупателя ИЛИ 
//	//|					Регистратор ССЫЛКА Документ.ЗаказНаряд
//	//|				) И
//	//|				Заказ = Неопределено И ДокументОтгрузки = Неопределено
//	//|		СГРУППИРОВАТЬ ПО Регистратор
//	//|	) Авансы
//	//|ПО
//	//|	Заказы.Заказ = Авансы.Заказ
//	|
//	|ЛЕВОЕ СОЕДИНЕНИЕ
//	|	РегистрНакопления.Продажи.Обороты(,,,Партнер = &Партнер) Отгрузки
//	|ПО
//	|	Заказы.Заказ = Отгрузки.Заказ
//	|
//	|УПОРЯДОЧИТЬ ПО Дата
//	|;
//	
...
//	//|ИЗ
//	//|	РегистрНакопления.Продажи.Обороты(,,Регистратор,,Организация = &Организация И Партнер = &Партнер)
//	////|ГДЕ
...
//	|	ДокументОтгрузки.Дата Дата,
//	|	СуммаПриход Сумма
//	|ИЗ
//	|	РегистрНакопления.ДолгиПоОтгрузкам.Обороты(,,,Организация = &Организация И Партнер = &Партнер)
//	|
//	|УПОРЯДОЧИТЬ ПО Дата
//	|;
//	
...
//	
//	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
//	|	ДокПолн.Регистратор ДокументОплаты,
//	|	ДокПолн.Период Дата,
//	|	СУММА(ДокПолн.СуммаОборот) 		Сумма,
//	|	СУММА(ДокАвансов.СуммаАванса)	СуммаАванса
//	|ИЗ
//	|	РегистрНакопления.ДенежныеСредства.Обороты(,,Регистратор,) ДокПолн
//	|
//	|ЛЕВОЕ СОЕДИНЕНИЕ
//	//
//	|	(
//	|		ВЫБРАТЬ Регистратор ДокументОплаты, Сумма(Сумма) СуммаАванса
//	|		ИЗ		РегистрНакопления.ДолгиПоЗаказам
//	|		ГДЕ		Организация = &Организация И Партнер = &Партнер И Заказ = НЕОПРЕДЕЛЕНО И ВидДвижения = Значение(ВидДвиженияНакопления.Расход) И Сумма > 0
//	|       СГРУППИРОВАТЬ ПО Регистратор
//	|
//	|	) ДокАвансов
//	//
//	|ПО
//	|	ДокПолн.Регистратор = ДокАвансов.ДокументОплаты
//	|
//	|ГДЕ
//	|	СуммаОборот > 0 И
//	|	Регистратор.Организация 	= &Организация И
...
////	|;
//	|ВЫБРАТЬ ""Это таблица просто так, чтобы не сбивать счетчик выборки пакета""
//	|;
//	
//	
//	
...
//	|	ЕСТЬNULL(ЕСТЬNULL(Запр.Дата, ЗакПок.Дата),ИнЗак.Дата) Дата,
//	|	ДокументОплаты,
//	|	Заказ,
//	|	ДокументОтгрузки,
//	|	ЕСТЬNULL(ЕСТЬNULL(ЗакПок.КорректировкаЗаказа, ИнЗак.КорректировкаЗаказа), НЕОПРЕДЕЛЕНО) КорректировкаЗаказа
//	|ИЗ
//	|(
//	//что это???
//	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
//	|		ЕСТЬNULL(Долг.Заказ.Дата, ЕСТЬNULL(ЕСТЬNULL(Прод.Регистратор.Дата, Долг.ДокументОтгрузки.Дата), Долг.Регистратор.Дата)) Дата,
//	|		Долг.Регистратор ДокументОплаты,
//	|		Долг.Заказ,
//	|		ЕСТЬNULL(Прод.Регистратор, Долг.ДокументОтгрузки) ДокументОтгрузки
//	|	ИЗ 	
//	|		РегистрНакопления.ДолгиПоОтгрузкам.Обороты(,,Регистратор,Организация = &Организация И Партнер = &Партнер) Долг
//	//что это???
//	|
//	|	ЛЕВОЕ СОЕДИНЕНИЕ
//	|		РегистрНакопления.Продажи.Обороты(,,Регистратор,Партнер = &Партнер) Прод
//	|	ПО
//	|		Долг.Заказ = Прод.Заказ И
//	|		Прод.Регистратор.Организация = &Организация И
//	|
//	|	ОБЪЕДИНИТЬ
//	|
//	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
//	|		ЕСТЬNULL(Заказ.Дата, Регистратор.Дата),
//	|		НЕОПРЕДЕЛЕНО,
//	|		Заказ,
//	|		Регистратор
//	|	ИЗ
//	|		РегистрНакопления.Продажи.Обороты(,,Регистратор,Партнер = &Партнер)
//	|	ГДЕ
//	|		Регистратор.Организация 	= &Организация И
//	|) Запр
//	|
//	|ЛЕВОЕ СОЕДИНЕНИЕ
//	|	(	ВЫБРАТЬ РАЗЛИЧНЫЕ Регистратор КорректировкаЗаказа, ЗаказПокупателя, Период Дата
//	|		ИЗ 		РегистрНакопления.ЗаказыПокупателей
//	|		ГДЕ		Регистратор ССЫЛКА Документ.КорректировкаЗаказаПокупателя И
//	|				ЗаказПокупателя.Организация = &Организация И 
//	|				ЗаказПокупателя.Партнер = &Партнер
//	|	) ЗакПок
//	|ПО
//	|	Запр.Заказ = ЗакПок.ЗаказПокупателя
//	|
//	|ЛЕВОЕ СОЕДИНЕНИЕ
//	|	(	ВЫБРАТЬ РАЗЛИЧНЫЕ Регистратор КорректировкаЗаказа, ИнтернетЗаказ, Период Дата
//	|		ИЗ 		РегистрНакопления.ИнтернетЗаказПокупателя
//	|		ГДЕ		Регистратор ССЫЛКА Документ.КорректировкаИнтернетЗаказа И
//	|				ИнтернетЗаказ.Организация = &Организация И 
//	|				ИнтернетЗаказ.Партнер = &Партнер
//	|	) ИнЗак
//	|ПО
//	|	Запр.Заказ = ИнЗак.ИнтернетЗаказ
//	|
//	|УПОРЯДОЧИТЬ ПО Дата
//	|");
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//	//Получаем организацию для партнера. Если их много, то в приоритете та, что с галками "по умолчанию"
	//	Запрос=Новый Запрос;
	//	Запрос.УстановитьПараметр("Партнер",Партнер);
	//	Запрос.Текст="ВЫБРАТЬ
	//	             |	КонтрагентыОрганизации.Организация,
	//	             |	КонтрагентыОрганизации.ЗначениеПоУмолчанию КАК ОрганизацияПоУмолчанию,
	//	             |	КонтрагентыОрганизации.Ссылка.ЗначениеПоУмолчанию КАК КонтрагентПоУмолчанию
	//	             |ИЗ
	//	             |	Справочник.Контрагенты.Организации КАК КонтрагентыОрганизации
	//	             |ГДЕ
	//	             |	КонтрагентыОрганизации.Ссылка.Партнер = &Партнер
	//	             |
	//	             |УПОРЯДОЧИТЬ ПО
	//	             |	КонтрагентПоУмолчанию УБЫВ,
	//	             |	ОрганизацияПоУмолчанию УБЫВ";
	//	Рез=Запрос.Выполнить();
	//	Если Не Рез.Пустой() Тогда
	//		Организация=Рез.Выгрузить()[0].Организация;
	//	КонецЕсли;
	//КонецЕсли;	
...
Процедура ДолгПарнераНажатие(Элемент, СтандартнаяОбработка)
	...
	//ПользовательскиеНастройки = ОбщегоНазначения.ПолучитьПользовательскиеНастройкиСКД("ЗадолженностьПоОтгрузкамПоСрокам",Новый Структура("Партнер",Партнер));
