...
Функция СохранитьИзменения(Изменения)
	...
	Если Изменения.Свойство("Организация") ИЛИ Изменения.Свойство("Партнер") Тогда
...
Функция ПартнерПриИзмененииНаСервере()
	 Возврат ФункцииФормДокументов.ПартнерПриИзменении(ЭтаФорма);
КонецФункции
...
Функция ПолучитьОсновногоКонтрагентаПартнера()
	Возврат Справочники.Партнеры.ПолучитьОсновногоКонтрагентаПартнера(Партнер);
КонецФункции
&НаСервере
...
&НаКлиенте
Функция ПроверитьНаСоответствиеОсновнойОрганизацииПартнера()
	НовыйКонтрагент = ПолучитьОсновногоКонтрагентаПартнера();
	Возврат ПроверитьНаСоответствиеОсновнойОрганизации(НовыйКонтрагент);	
КонецФункции
&НаКлиенте
...
КонецПроцедуры
&НаКлиенте
Процедура ВернутьПредыдущееЗначениеПартнера()
	
	Перем стПартнер;
	
	Если Изменения.Свойство("Партнер", стПартнер) И стПартнер <> Неопределено Тогда
		Партнер = стПартнер.НовоеЗначение;
	Иначе
		Партнер = ВладелецФормы.Объект.Партнер;
	КонецЕсли;
	
	ПартнерПриИзмененииНаКлиенте();
	
КонецПроцедуры

&НаКлиенте
Процедура ПартнерПриИзмененииНаКлиенте()
	
	СтруктураРеквизитов = ПартнерПриИзмененииНаСервере();
	ЭлементПриИзменении("Партнер");
...
Процедура ПартнерПриИзменении(Элемент)
	...
	Если НЕ ПроверитьНаСоответствиеОсновнойОрганизацииПартнера() Тогда
		ВернутьПредыдущееЗначениеПартнера();
		Возврат;
	КонецЕсли;
...
Процедура ДокументыОплатыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ВыбПлатежка = ОткрытьФорму("Документ.ПлатежноеПоручениеВходящее.ФормаВыбора", 
						...
						ВладелецФормы.Объект.Организация, Контрагент, Партнер),
