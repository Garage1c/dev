...
Процедура ОбновитьНадписьПартнеры()
	...
	Для Каждого Строка Из ДополнительныеПараметры Цикл
		...
		Если ЗначениеЗаполнено(Строка.Партнер) Тогда
			ФормСтроки.Добавить(Новый ФорматированнаяСтрока(Строка(Строка.Партнер),,,,ПолучитьНавигационнуюСсылку(Строка.Партнер))); КонецЕсли; КонецЦикла;
...
КонецПроцедуры
Процедура ОтправитьНаЭлектроннуюПочту_ст(Команда)
	...
	//	Если Найти(Строка.Почта, "@") Тогда Структура.Вставить("Почта", Строка.Почта); КонецЕсли; 
	//	
	//	Вложения.Добавить(Структура); КонецЦикла;
	//
	//ОткрытьФорму("Документ.Письмо.Форма.Письмо2", 
	//		Новый Структура("Вложения, ИмяТабличногоДокумента, ТабличныйДокумент, УдалитьФайлыПослеОтправки, ТипФайлаВложения, РасширениеФайлаВложения", 
	//					Вложения, 
	//					ИмяФайла, 
	//					ТабличныйДокумент, 
	//					Истина,
	//					ТипФайлаТабличногоДокумента[ФорматФайла],
	//					ПолучитьРасширениеДляФайлаПоТипу(ФорматФайла)));
	//
КонецПроцедуры
