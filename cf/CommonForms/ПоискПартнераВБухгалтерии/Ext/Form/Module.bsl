...
//&НаСервере
//Функция ИницилизироватьСоединение(стрОшибки)
//	 
//	Перем База;                           
//	
	...
	Если Connector <> Неопределено Тогда
		...
		Если РезультатЗапроса <> Неопределено Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				СпрСсылка = Справочники.Партнеры.ПолучитьСсылку(Новый УникальныйИдентификатор(ГУИД));
...
Процедура СоздатьПартнера(Команда)
	...
	Если НЕ текПартнер.ЕстьВТорговле И Вопрос("Партнер """ + текПартнер.Имя + """ отсутствует в торговле.
				|Создать нового, копированием из бухгалтерии?", РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
				
		СпрСсылка = ПолучитьНовогоПартнера(текПартнер.ГУИД);
		Если СпрСсылка <> Неопределено Тогда
			Сообщить("Новый партнер успешно создан", СтатусСообщения.Информация);	
		КонецЕсли;

	ИначеЕсли текПартнер.ЕстьВТорговле И Вопрос("Партнер """ + текПартнер.Имя + """ уже создан в тоговле.
				|Обновить существующего?", РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
		ПерезаписыватьНайденные = Истина;		
		СпрСсылка = ПолучитьНовогоПартнера(текПартнер.ГУИД);
		Если СпрСсылка <> Неопределено Тогда
			Сообщить("Партнер успешно обновлен", СтатусСообщения.Информация);		
		КонецЕсли;
	КонецЕсли;
...
Процедура СписокПохожихПриАктивизацииСтроки(Элемент)
	...
	Если ТекДанные <> Неопределено Тогда
		текПартнер = Новый Структура("Имя, ГУИД, ЕстьВТорговле",  ТекДанные.Представление, ТекДанные.Значение, ТекДанные.Пометка);
	КонецЕсли;
КонецПроцедуры
&НаСервере
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	текПартнер = Неопределено;
	ЧтоИщем = Параметры.Партнер;
...
&НаСервере
Процедура ПолучитьСправочникКонтактныеЛица(Ссылка81, Connector, Партнер)
...
Функция СоздатьНовогоПартнера(ГУИД, Connector = Неопределено)
	...
	Если НЕ СпрСсылка.Пустая() И (СпрСсылка.ПолучитьОбъект() = Неопределено ИЛИ ПерезаписыватьНайденные) Тогда
		...
		Если Connector <> Неопределено Тогда
			...
			ТекстЗапроса = "ВЫБРАТЬ
				|	Контр.Ссылка	 			Партнер,
				|	Контр.ЭтоГруппа,
				|	Контр.Наименование,
				|	Контр.Родитель				ГруппаПартнер,
				...
				|ГДЕ
				|	Контр.Ссылка = &Ссылка							
				|УПОРЯДОЧИТЬ ПО Партнер" + ?(Ссылка81.ЭтоГруппа, "" , ", Организация, ОрганизацияПоУмолчанию Убыв");
			...
			Если РезультатЗапроса <> Неопределено Тогда
				...
				Пока Выборка.СледующийПоЗначениюПоля("Партнер") Цикл
					...
					КонецЕсли;
					ГруппаГУИД = Connector.String(Выборка.ГруппаПартнер.УникальныйИдентификатор());
					...
					Если НЕ ПерезаписыватьНайденные Тогда 
						COMServer.ВыполнитьСинхронизациюКонтактнойИнформации("Справочник.Контрагенты", Выборка.Партнер, стрОшибки, Connector);
					КонецЕсли;
					
					// СОЗДАДИМ КОНТРАГЕНТА ДЛЯ ПАРТНЕРА
					
					КонтрСсылка = Справочники.Контрагенты.ПолучитьСсылку(СпрСсылка.УникальныйИдентификатор());
					
					Если КонтрСсылка.ПолучитьОбъект() = Неопределено И НЕ КонтрСсылка.Пустая() Тогда
						СпрКонтр = Справочники.Контрагенты.СоздатьЭлемент();  
			      		СпрКонтр.УстановитьСсылкуНового(КонтрСсылка);
  					ИначеЕсли ПерезаписыватьНайденные И НЕ КонтрСсылка.Пустая() Тогда
						СпрКонтр = КонтрСсылка.ПолучитьОбъект();
					Иначе
						Возврат Спр.Ссылка;
					КонецЕсли;	
					
					ЗаполнитьЗначенияСвойств(СпрКонтр, Выборка, "Наименование, Комментарий, НаименованиеПолное, ИНН, КПП, КодПоОКПО");
                    СпрКонтр.Партнер = Спр.Ссылка;
					
					Если НЕ Выборка.ЮрФизЛицо.Пустая() Тогда
						СпрКонтр.ЮрФизЛицо = Перечисления.ЮрФизЛицо.Получить(Connector.Перечисления.ЮрФизЛицо.Индекс(Выборка.ЮрФизЛицо));
					КонецЕсли;
										
					Пока Выборка.СледующийПоЗначениюПоля("Организация") Цикл
                        
			        	ОсновнаяОрганизация = Справочники.Организации.ПолучитьСсылку(
								Новый УникальныйИдентификатор(
									Connector.String(Выборка.Организация.УникальныйИдентификатор())
								));
					
		               	Если НЕ ОсновнаяОрганизация.Пустая() И ОсновнаяОрганизация.ПолучитьОбъект() <> Неопределено Тогда
							
							Строки = СпрКонтр.Организации.НайтиСтроки(Новый Структура("Организация", ОсновнаяОрганизация));
						
							НоваяСтрока = ?(Строки.Количество(), Строки[0], СпрКонтр.Организации.Добавить());
							
							НоваяСтрока.Организация = ОсновнаяОрганизация;
							НоваяСтрока.НомерДоговора = Выборка.НомерДоговора;
							НоваяСтрока.ЗначениеПоУмолчанию = Выборка.ОрганизацияПоУмолчанию;
						КонецЕсли;
					...
					Попытка
						СпрКонтр.Записать();
					Исключение
						Сообщить("Не удалось записать объект """ + СпрКонтр + """!
								  |" + ОписаниеОшибки());
					КонецПопытки;
							  
					СпрКонтр.УстановитьПометкуУдаления(Выборка.ПометкаУдаления);
					
					// КОНТАКТНЫЕ ЛИЦА
					ПолучитьСправочникКонтактныеЛица(Ссылка81, Connector, Спр.Ссылка);
                    					             	
	            	// БАНКОВСКИЕ СЧЕТА
					ПолучитьСправочникБанковскиеСчета(Ссылка81, Connector, СпрКонтр.Ссылка);
