...
КонецПроцедуры
Процедура УстановитьПометкиВСпискеЗначений(СписокЗначений, Помеченные, СниматьПредыдущие = Истина) Экспорт
	
	// Устанавливает пометки в списке значений
	// Помеченные 			- или массив значений которых нужно пометить или одно значение которое пометить
	// СниматьПредыдущие 	-  если установить то все пометки будут сняты кроме устанавливаемых
	
	ЭтоМассив = ТипЗнч(Помеченные) = Тип("Массив");
	
	Для Каждого Элемент Из СписокЗначений Цикл
		Есть = ?(ЭтоМассив, Помеченные.Найти(Элемент.Значение) <> Неопределено, Помеченные = Элемент.Значение);
		Элемент.Пометка = Есть ИЛИ ?(СниматьПредыдущие, Ложь, Элемент.Пометка); КонецЦикла;
...
КонецФункции
Функция ПолучитьТаблицуИзНайденныхСтрокТаблицыЗначений(Таблица, СтруктураПоиска) Экспорт
	
	Строки 		= Таблица.НайтиСтроки(СтруктураПоиска);
	НовТаблица 	= Таблица.СкопироватьКолонки();
...
КонецФункции
Функция ПолучитьСуммыКолонкПоУсловию(Знач Таблица, АкуммулКолонки, Отбор) Экспорт
	
	Структура 	= Новый Структура(АкуммулКолонки); Для Каждого Элемент Из Структура Цикл Элемент.Значение = 0 КонецЦикла;
	Строки 		= Таблица.НайтиСтроки(Отбор);
	Для Каждого Строка Из Строки Цикл Для Каждого Элемент Из Структура Цикл Элемент.Значение = Элемент.Значение + Строка[Элемент.Ключ] КонецЦикла; КонецЦикла;
...
Процедура УдалитьПохожиеСтрокиВтаблицеПоТаблицеНеБолееМинимальноогоПоказателя(ТаблицаУдаляемая, ТаблицаПоиска, КолонкиПоиска, ИмяКолонкиМинимум, ИменаПоказателейУменьшения = "") Экспорт
	
	// Удаляет из таблицы строки которые подходят по условию
	// при удалении не удаляет меньше чем есть в показателе
	//
	// ТаблицаУдаляемая - таблица из которой удаляем строки
	// ТаблицаПоиска - таблица по которой ищем строки 
...
Функция ПолучитьСтруктуруПравилОбменаСЭкселем_Ст(ПравилоСсылка, стрОшибки = "") Экспорт
	
	// Возвращает структуру правил для правил загрузки данных с экселя
	
	// - ВидЗагрузки
	// - ВидОбъекта
	// - Загружаемые строки (массив) элементами является структура
	// 		- Номер колонки
	// 		- Имя поля
	//		- Тип объекта
	//		- Это реквизит поиска
	...
	|ВЫБРАТЬ
	|	Спр.НачальнаяСтрока,
	|	Спр.Ссылка.ВидВыгрузки,
	|	Спр.Ссылка.ВидОбъекта,
	|	Спр.Ссылка,
	|	Спр.НомерСтроки,
	|	Спр.НомерКолонки,
	|	Спр.ИмяРеквизита,
	|	Спр.ТипОбъекта,
	|	Спр.ЭтоРеквизитПоиска,
	|	Спр.ТочноеСоответствие,
	|	Спр.Выражение
	|ИЗ
	|	Справочник.ПравилаОбменаСЭкселем.ЗагружаемыеСтроки КАК Спр
	|");
	
	Запрос.УстановитьПараметр("Ссылка", ПравилоСсылка);
	
	Выборка 				= Запрос.Выполнить().Выбрать();
	Структура 				= Новый Структура("ВидЗагрузки, ВидОбъекта, ЗагружаемыеСтроки",
											ПравилоСсылка.ВидЗагрузки, ПравилоСсылка.ВидОбъекта, ПравилоСсылка.ЗагружаемыеСтроки);
	ПустаяСтруктураТаблицы 	= ПолучитьПустуюСтруктуруИзПолейТабличнойЧасти(ПравилоСсылка.Метаданные().ЗагружаемыеСтроки);
	...
	Пока Выборка.Следующий() Цикл Ном = Ном + 1;
		...
		СтруктураТаблицы = ПустаяСтруктураТаблицы;
		ЗаполнитьЗначенияСвойств(СтруктураТаблицы, Выборка);
		
		Если ПустаяСтрока(Выборка.Выражение) Тогда
			
			// Примитивный объект тупа приравниваем
			
			Если КэшируемыеФункции.стрПримитивныйТип(Выборка.ТипОбъекта) Тогда
				
				СтруктураТаблицы.Выражение = "Результат = ТекстВЯчейке";
				
			Иначе
				
				Метаданное = Метаданные.НайтиПоПолномуИмени(Выборка.ТипОбъекта);
				Если Метаданное = Неопределено Тогда
					стрОшибки = "Не определен тип загрузки из экселя """ + Выборка.ТипОбъекта + """";
					Возврат Неопределено;
				КонецЕсли;
				
				Если Метаданные.Справочники.Содержит(Метаданное) Тогда
					
					ИмяРеквизита = ВРЕГ(СокрЛП(Выборка.ИмяРеквизита));
					
					Если ИмяРеквизита = "КОД" Тогда
						СтруктураТаблицы.Выражение = "Результат = Справочники." + Метаданное.Имя + ".НайтиПоКоду(ТекстВЯчейке)";
					ИначеЕсли ИмяРеквизита = "НАИМЕНОВАНИЕ" Тогда
						СтруктураТаблицы.Выражение = "Результат = Справочники." + Метаданное.Имя + ".НайтиПоНаименованию(ТекстВЯчейке," + ?(Выборка.ТочноеСоответствие,"ИСТИНА","ЛОЖЬ") + ")";
					Иначе
						СтруктураТаблицы.Выражение = "Результат = Справочники." + Метаданное.Имя + ".НайтиПоРеквизиту(""" + ИмяРеквизита + """, ТекстВЯчейке)";
					КонецЕсли;
					
				Иначе
					
					стрОшибки = "Не задано правило поиска для типа загрузки из экселя """ + Выборка.ТипОбъекта + """, на сегодня это только ""Справочники.xxx""";
					Возврат Неопределено;
		...
		КоллекцияДанных.Добавить(СтруктураТаблицы);
		
	КонецЦикла;
	
	Структура.Вставить("ЗагружаемыеСтроки", КоллекцияДанных);
	
	Возврат Структура;
	
КонецФункции
Функция ПолучитьСтруктуруПравилОбменаСЭкселем(ПравилоСсылка, стрОшибки = "") Экспорт
	...
	Структура = Новый Структура("ЛистЭкселя, НачальнаяСтрока, ВидЗагрузки, ВидОбъекта, ЭтоГруппа",
											ПравилоСсылка.ЛистЭкселя,
											ПравилоСсылка.НачальнаяСтрока, 
											ПравилоСсылка.ВидЗагрузки, 
											ПравилоСсылка.ВидОбъекта, 
											ПравилоСсылка.Группа);
	Запрос = Новый Запрос("
	|ВЫБРАТЬ
	|	*
	|ИЗ
	|	Справочник.ПравилаОбменаСЭкселем.ЗагружаемыеСтроки
	|ГДЕ
	|	Ссылка = &Ссылка
	|");
	...
	Выборка 				= Запрос.Выполнить().Выбрать();
	ПустаяСтруктураТаблицы 	= ПолучитьПустуюСтруктуруИзПолейТабличнойЧасти(ПравилоСсылка.Метаданные().табличныеЧасти.ЗагружаемыеСтроки);
	КоллекцияДанных 		= Новый Массив;
	...
	Пока Выборка.Следующий() Цикл Ном = Ном + 1;
		...
		Для Каждого Элемент Из ПустаяСтруктураТаблицы Цикл
			СтруктураТаблицы.Вставить(Элемент.Ключ, Выборка[Элемент.Ключ]);
		КонецЦикла;
...
Функция ПолучитьСтруктуруПравилОбменаСMySQL_Ст(ПравилоСсылка, стрОшибки = "") Экспорт
	
	// Возвращает структуру правил для правил загрузки данных с экселя
	
	Структура = Новый Структура("ИмяТаблицы,  ВидОбъекта, НеУдалятьПриСинхронизации",
											ПравилоСсылка.ИмяТаблицы,
											ПравилоСсылка.ВидОбъекта, 
											ПравилоСсылка.НеУдалятьПриСинхронизации);
	Запрос = Новый Запрос("
	|ВЫБРАТЬ
	|	*
	|ИЗ
	|	Справочник.ПравилаОбменаMySQL.ЗагружаемыеСтроки
	|ГДЕ
	|	Ссылка = &Ссылка
	|;
	|ВЫБРАТЬ
	|	*
	|ИЗ
	|	Справочник.ПравилаОбменаMySQL.Отборы1С
	|ГДЕ
	|	Ссылка = &Ссылка
	|;
	|ВЫБРАТЬ
	|	*
	|ИЗ
	|	Справочник.ПравилаОбменаMySQL.ОтборыMySQL
	|ГДЕ
	|	Ссылка = &Ссылка
	|");
	...
	МетаТаблОб 					= ПравилоСсылка.Метаданные().табличныеЧасти;
	
	ПустаяСтруктураТаблицы 		= ПолучитьПустуюСтруктуруИзПолейТабличнойЧасти(МетаТаблОб.ЗагружаемыеСтроки);
	ПустаяСтруктураОтбора1С		= ПолучитьПустуюСтруктуруИзПолейТабличнойЧасти(МетаТаблОб.Отборы1С);
	ПустаяСтруктураОтбораMySQL	= ПолучитьПустуюСтруктуруИзПолейТабличнойЧасти(МетаТаблОб.ОтборыMySQL);
	
	КоллекцияДанных 			= Новый Массив;
	КоллекцияДанныхОтбора1С		= Новый Массив;
	КоллекцияДанныхОтбораMySQL	= Новый Массив;
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого Элемент Из ПустаяСтруктураОтбора1С Цикл
			СтруктураТаблицы.Вставить(Элемент.Ключ, Выборка[Элемент.Ключ]);
		КонецЦикла;
		
		КоллекцияДанныхОтбора1С.Добавить(СтруктураТаблицы);
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого Элемент Из ПустаяСтруктураОтбораMySQL Цикл
			СтруктураТаблицы.Вставить(Элемент.Ключ, Выборка[Элемент.Ключ]);
		КонецЦикла;
		
		КоллекцияДанныхОтбораMySQL.Добавить(СтруктураТаблицы);
		
	КонецЦикла;
	
	Структура.Вставить("ЗагружаемыеСтроки", КоллекцияДанных);
	Структура.Вставить("Отборы1С", 			КоллекцияДанныхОтбора1С);
	Структура.Вставить("ОтборыMySQL", 		КоллекцияДанныхОтбораMySQL);
	
	Возврат Структура;
	
КонецФункции
Функция ПолучитьСтруктуруПравилОбменаСMySQL_откл(ПравилоСсылка, стрОшибки = "") Экспорт
	
	// Возвращает структуру правил для правил загрузки данных с экселя
	
	Возврат ПолучитьСтруктуруИзСправочника(ПравилоСсылка, стрОшибки);
	
КонецФункции

Функция ПолучитьСтруктуруИзСправочника(СправочникСсылка, стрОшибки = "") Экспорт
	
	// Возвращает структуру правил для правил загрузки данных с экселя
	
	Структура 		= Новый Структура;
	метаОбъект 		= СправочникСсылка.Метаданные();
	...
	Для Каждого ТабличнаяЧасть Из метаОбъект.ТабличныеЧасти Цикл
		...
		именаРеквизитовТаблиц = Новый Массив;
		
		Для Каждого Реквизит Из ТабличнаяЧасть.Реквизиты Цикл
			именаРеквизитовТаблиц.Добавить(Реквизит.Имя);
		КонецЦикла;
		
		именаТаблиц.Вставить(ТабличнаяЧасть.Имя, именаРеквизитовТаблиц);
	...
	Если Выгрузка.Количество() Тогда
		...
		Для Каждого Реквизит Из именаРеквизитов Цикл
			Структура[Реквизит] = Строка[Реквизит];
		КонецЦикла;
		
		Для Каждого ИмяТаблицы Из именаТаблиц Цикл
			
			МассивСтрок = Новый массив;
			Таблица 	= Строка[ИмяТаблицы.Ключ];
			
			Для Каждого СтрокаТаблицы Из Таблица Цикл
				
				Структуратаблицы = новый Структура;
				Для Каждого РеквизитТаблицы Из ИмяТаблицы.Значение Цикл
				
					Структуратаблицы.Вставить(РеквизитТаблицы,  СтрокаТаблицы[РеквизитТаблицы]);
					
				КонецЦикла;
				
				МассивСтрок.Добавить(Структуратаблицы);
				
			КонецЦикла;
			
			Структура.Вставить(ИмяТаблицы.Ключ, МассивСтрок);
...
КонецФункции
Функция ПолучитьСтруктуруИзЗаписиРегистраСведений(Запись, стрОшибки = "") Экспорт
	
	Тип = ТипЗнч(Запись);
	МетаОб = Метаданные.НайтиПоТипу(Тип);
	Структура = Новый Структура;
	Для Каждого Элемент ИЗ МетаОб.Измерения Цикл
		Структура.Вставить(Элемент.Имя, Запись[Элемент.Имя]); КонецЦикла;
	Возврат Структура;
КонецФункции
Функция ПолучитьСоответствиеСкладовККодамВнешнихПрограмм(ВидПриложения = Неопределено) Экспорт
	
	Соответствия = Новый Соответствие;
	
	Запрос = Новый Запрос("
	|ВЫБРАТЬ 	Склад, КодСклада
	|ИЗ 		Справочник.БазыДанных.СоответствияСкладов	
	|ГДЕ 		НЕ Ссылка.ПометкаУдаления
	|" + ?(ВидПриложения = Неопределено, ""," И Ссылка.ВидПриложения = &ВидПриложения") + "
	|");
	
	Запрос.УстановитьПараметр("ВидПриложения", ВидПриложения);
	
	Выборка = Запрос.Выполнить().Выбрать();
	ПОка Выборка.Следующий() Цикл
		
		Соответствия.Вставить(Выборка.Склад, Выборка.КодСклада);
