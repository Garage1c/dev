...
Функция ПредставлениеЗадачиПоЗаказу(Задача, Заказ) Экспорт
	...
	Иначе
		...
		Возврат Задача.Наименование + " № "  + НомерЗаказа + " (" + Формат(Заказ.Сумма, "Ч15.2,") + ") " + ?(ТипЗнч(Заказ) = Тип("ДокументСсылка.ИнтернетЗаказПокупателя"), ?(ЗначениеЗаполнено(Заказ.ПользовательИнтернет), Заказ.ПользовательИнтернет, Заказ.Контрагент), ?(Заказ.ОтветственноеЛицо.Пустая(), Заказ.Контрагент, Заказ.ОтветственноеЛицо));
...
Функция Перемещение_ЗадачаОтправить_ПолучитьДанныеЗаказНаСервере(ПроцессСсылка) Экспорт
...
Функция Перемещение_ЗадачаОтправить_ЕстьПодчиненныеНезавершенныеПроцессы(БП) Экспорт
...
Функция Перемещение_ЗадачаОтправить_СформироватьИПровестиОтправкуТовараИПроверитьБП(Объект,ЭтаФорма) Экспорт
	
	ЗаказСобран = Объект.БизнесПроцесс.ЗаказСобран;
	
	ДанныеЗаказа = Перемещение_ЗадачаОтправить_ПолучитьДанныеЗаказНаСервере(Объект.БизнесПроцесс);
	
	НачатьТранзакцию();
	
	// Запишем галку процесса если изменилась
	
	Если 	ЭтаФорма.БыстроеПеремещение И
			ЭтаФорма.БыстроеПеремещение <> Объект.Ссылка.БизнесПроцесс.БыстроеПеремещение Тогда
			
		Процесс = Объект.Ссылка.БизнесПроцесс.ПолучитьОбъект();
		Процесс.БыстроеПеремещение = ЭтаФорма.БыстроеПеремещение;
	...
	Если ЭтаФорма.Товары.Количество() Тогда
		
		// Вытащим товары
		//+Андриянов 16.06.2017 делаем универсальный модуль, может передаваться структура с ТЗ, а не форма и ТЧ
		Если ТипЗнч(ЭтаФорма.Товары) = Тип("ТаблицаЗначений") Тогда
			ТоварыТЗ = ЭтаФорма.Товары;
		Иначе
		//-Андриянов
			ТоварыТЗ = ЭтаФорма.Товары.Выгрузить();
		КонецЕсли;
		КонвертацияТипов.УдалитьСтрокиТЗ(ТоварыТЗ, Новый Структура("Количество", 0));
		
		// Заполним документы по разному
		
		Если ЭтаФорма.БыстроеПеремещение Тогда
			
			НовДок = Документы.ПеремещениеТоваров.СоздатьДокумент();
			НовДок.Основание 		= Объект.Ссылка.БизнесПроцесс;
			НовДок.ОтправитьВСборку = ?(БизнесПроцессы.ПеремещениеТоваров.ПолучитьПроцессРодитель(Объект.БизнесПроцесс) = Неопределено, ЭтаФорма.ОтправитьВСборку, Ложь);
			НовДок.СпособРазмещенияБезЗаказа = ДанныеЗаказа.СпособРазмещенияБезЗаказа;
			НовДок.СборочныйЛист = ЭтаФорма.СборочныйЛист;
			...
			Если Не ЗаказСобран Тогда  // сборка уже ячейки списала
				...
				ТоварыТЗ.ЗаполнитьЗначения(ЭтаФорма.ЯчейкаПолучатель, "ЯчейкаПолучатель"); КонецЕсли;
		...
		Иначе
			...
			НовДок.Маршрут 	= ЭтаФорма.Маршрут;
			НовДок.СборочныйЛист = ЭтаФорма.СборочныйЛист;
		...
		НовДок.СкладОтправитель = ЭтаФорма.СкладОтправитель;
		НовДок.СкладПолучатель 	= ЭтаФорма.СкладПолучатель;
		...
		Если ТоварыТЗ.Количество() Тогда
			...
			Если ЭтаФорма.БыстроеПеремещение И ЭтаФорма.ОтправитьВСборку Тогда // Отправим сразу в сборку
				
				ТоварыТЗ.Колонки.Добавить("Ячейка", Новый ОписаниеТипов("СправочникСсылка.Ячейки"));
				ТоварыТЗ.ЗаполнитьЗначения(ЭтаФорма.ЯчейкаПолучатель, "Ячейка"); 
			
				Если Не БизнесПроцессы.ПеремещениеТоваров.ОтправитьВСборку(ЭтаФорма.Заказчик.Заказ, ЭтаФорма.СкладПолучатель, ТоварыТЗ) Тогда
					ОтменитьТранзакцию();
...
Функция Перемещение_ЗадачаОтправить_ЗаполнитьСтруктуру(Объект) Экспорт
	стрПеремещение = Новый Структура;
	стрПеремещение.Вставить("СборочныйЛист",Перемещение_ЗадачаОтправить_ПолучитьСборочныйЛист(Объект));
	стрПеремещение.Вставить("ЯчейкаПолучатель",Справочники.Ячейки.ПустаяСсылка());
	стрПеремещение.Вставить("БыстроеПеремещение",Неопределено);
	стрПеремещение.Вставить("Маршрут",Неопределено);
	стрПеремещение.Вставить("СкладОтправитель",Неопределено);
	стрПеремещение.Вставить("СкладПолучатель",Неопределено);
	стрПеремещение.Вставить("ОтправитьВСборку",Неопределено);
	стрПеремещение.Вставить("Заказчик",Неопределено);
	стрПеремещение.Вставить("Товары",Неопределено);
	
	стрПеремещение.Вставить("ИмяФормы",Неопределено);
	
	ЗаполнитьДанные(стрПеремещение, Объект.Ссылка.БизнесПроцесс, Объект.Ссылка);
	
	Возврат стрПеремещение;
КонецФункции

Функция Перемещение_ЗадачаОтправить_ПолучитьСборочныйЛист(Объект) Экспорт
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда Возврат Выборка.СборочныйЛист
