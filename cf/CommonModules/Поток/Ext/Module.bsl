...
//Функция ИнформацияОДолгеКлиента(Организация, Контрагент, ФормаОтношений = Неопределено)
//	
...
//			
//		Возврат Новый Структура("Инф1, Инф2, Инф3, Документ, Дата, Сумма", 
//			?(ЗначениеЗаполнено(Организация), "", Новый ФорматированнаяСтрока("организация не выбрана")),
//			?(ЗначениеЗаполнено(Контрагент), "", Новый ФорматированнаяСтрока("контрагент не выбран")),
//			ТекстОтгрузкаЗапрещена,,'00010101',0); КонецЕсли;
//		
//	// Вычислим дни
//		
//	ДнейОтсрочки = ?(Контрагент.Партнер.Пустая(), 0, Контрагент.Партнер.ДнейОтсрочки);
//	
//	Запрос = Новый Запрос("
//	
//	// Посмотрим что за птица такая
//	
//	|ВЫБРАТЬ ПЕРВЫЕ 1 Документ, Дата, Сумма
//	|ИЗ РегистрСведений.ДолгиКонтрагентов.СрезПоследних(,Организация = &Организация И Контрагент = &Контрагент И ФормаОтношений = &ФормаОтношений)
//	|УПОРЯДОЧИТЬ ПО Дата Убыв
//	|;
//	
//	// Определим когда образовался долг
//	
//	|ВЫБРАТЬ ПЕРВЫЕ 1 ДокументОтгрузки ДокДолга, ДокументОтгрузки.Дата ПериодДолга ИЗ РегистрНакопления.ДолгиПоОтгрузкам.Остатки(,Организация = &Организация И Партнер = &Партнер)
//	|УПОРЯДОЧИТЬ ПО ПериодДолга;
//	
//	//|ВЫБРАТЬ ПЕРВЫЕ 1 Рег.Документ ДокДолга, Рег.Период ПериодДолга
...
//	//|	ГДЕ	ФормаОтношений 	= &ФормаОтношений И
...
//	//|
//	//|ГДЕ
//	//|	ФормаОтношений 	= &ФормаОтношений И
//	//|	Организация 	= &Организация И
//	//|	Контрагент 		= &Контрагент И
...
//	|");
//	
//	Запрос.УстановитьПараметр("ФормаОтношений", 	?(ФормаОтношений = Неопределено, Перечисления.ФормаОтношений.Клиент, ФормаОтношений));
//	Запрос.УстановитьПараметр("Организация", 		Организация);
//	Запрос.УстановитьПараметр("Контрагент", 		Контрагент);
//	Запрос.УстановитьПараметр("Партнер", 			Контрагент.Владелец);
...
//														"Открыть", "РегистрСведений.ДолгиКонтрагентов.Форма.ДолгКонтрагента", Новый Структура("Отбор", Новый Структура("Контрагент", Контрагент)))), Символы.ПС, ""));
//														
//		// Если есть записи в регистр значит ктото комуто должен
//		
//		массСтрокДок 	= Новый Массив;
//		ВыборкаНеДолг 	= Пакет[1].Выбрать();
//		
//		Если ВыборкаНеДолг.Следующий() Тогда Док = ВыборкаНеДолг.ДокДолга;
//			массСтрокДок.Добавить(Новый ФорматированнаяСтрока("нет опл:"));
//			массСтрокДок.Добавить(Новый ФорматированнаяСтрока(Строка(Док),,,,ПолучитьНавигационнуюСсылку(Док)));
//			ОсталосьДней 	= ((НачалоДня(ВыборкаНеДолг.ПериодДолга) + ДнейОтсрочки * 86400) - НачалоДня(ТекущаяДата())) / 86400; 
//			
//		Иначе Док = Выборка.Документ;
//			ОсталосьДней 	= ДнейОтсрочки; 
//			массСтрокДок.Добавить(Новый ФорматированнаяСтрока("задолжность дней макс: "));
//			массСтрокДок.Добавить(Новый ФорматированнаяСтрока(Строка(ОсталосьДней),,WebЦвета.ТемноСиний));
//		КонецЕсли;
//		
//			
//		Возврат Новый Структура("Инф1, Инф2, Инф3, Документ, Дата, Сумма",
//								?(Выборка.Сумма = 0, Новый ФорматированнаяСтрока("никто никому не должен",,,,СтрОткрытьОтчетПоКонтрагенту), 
//									?(Выборка.Сумма > 0, 
//											Новый ФорматированнаяСтрока("Долг: " + Выборка.Сумма,,WebЦвета.КрасноФиолетовый,,СтрОткрытьОтчетПоКонтрагенту), 
//											Новый ФорматированнаяСтрока("Аванс: " + Строка(-Выборка.Сумма),,WebЦвета.ТемноЗеленый,,СтрОткрытьОтчетПоКонтрагенту))),
//								Новый ФорматированнаяСтрока(массСтрокДок),
//								?(ОсталосьДней > 0 Или Выборка.Сумма < 0, Новый ФорматированнаяСтрока("отгрузка разрешена",,WebЦвета.Зеленый), ТекстОтгрузкаЗапрещена),
//								Док, Выборка.Дата, Выборка.Сумма);
//	Иначе
//		
//		// Если нет записи то он новенький
//								
//		стрДнейОтсрочки = КонвертацияТипов.ПолучитьЧислительноеЧислоПрописью(ДнейОтсрочки) + " " + КонвертацияТипов.ПолучитьКоличествоДнейПрописью(ДнейОтсрочки);
//		Возврат Новый Структура("Инф1, Инф2, Инф3, Документ, Дата, Сумма",
//								"нет информации в базе",
//								?(ДнейОтсрочки, "отсрочка в течении " + стрДнейОтсрочки, "клиент по предоплате"),
//								?(ДнейОтсрочки, Новый ФорматированнаяСтрока("отгрузка разрешена",,WebЦвета.Зеленый), ТекстОтгрузкаЗапрещена),
//								,,'00010101', 0); КонецЕсли;
//КонецФункции
...
Функция ИнформацияОДолгеПартнера(Контрагент,ДатаКонтроля=Неопределено)
	...
	//|Выбрать 
	//|ДокументОтгрузки,
	//|РАЗНОСТЬДАТ(ДОБАВИТЬКДАТЕ(ДокументОтгрузки.Дата, ДЕНЬ, Партнер.ДнейОтсрочки), &ДатаКонтроля, ДЕНЬ) КАК ДнейПросрочено,
	//|СуммаОстаток
	//|из РегистрНакопления.ДолгиПоОтгрузкам.Остатки(&ДатаКонтроля,Партнер = &Партнер)
	//|");
	//
	//
	...
	//			
	//			Долг=Долг+Выборка.СуммаОстаток;
	//			Если Выборка.ДнейПросрочено>0 Тогда
	//				ПросроченныйДолг=ПросроченныйДолг+Выборка.СуммаОстаток;
	//			КонецЕсли;	
	//		Иначе
	//			Аванс=Аванс-Выборка.СуммаОстаток;
	//		КонецЕсли;
	//		ОбщаяЗадолженность=ОбщаяЗадолженность+Выборка.СуммаОстаток;
	//	КонецЦикла;	
	//	
	//	РазвернутаяЗадолженность = Новый Массив;
	//	Если Аванс>0 Тогда
	//		РазвернутаяЗадолженность.Добавить(Новый ФорматированнаяСтрока("в т.ч.не разн.оплата: " + Аванс,,WebЦвета.ТемноЗеленый,,));
	//	КонецЕсли;
	//	Если Долг>0 и (Долг<>ОбщаяЗадолженность) Тогда
	//		РазвернутаяЗадолженность.Добавить(Новый ФорматированнаяСтрока("в т.ч.долг: " + Долг,,WebЦвета.КрасноФиолетовый,,));
	//	КонецЕсли;
	//	Если ПросроченныйДолг>0 Тогда
	//		РазвернутаяЗадолженность.Добавить(Новый ФорматированнаяСтрока(" в т.ч.просроч.: " + ПросроченныйДолг,,WebЦвета.КрасноФиолетовый,,));
	//	КонецЕсли;
	//	
	//	Если ОбщаяЗадолженность>0 тогда
	//		Инф1 = Новый ФорматированнаяСтрока("общий долг: "+ОбщаяЗадолженность,,WebЦвета.ТемноЗеленый,,СтрОткрытьОтчетПоКонтрагенту);
	//	Иначе
	//		Инф1 = Новый ФорматированнаяСтрока("наш долг: "+(-ОбщаяЗадолженность),,WebЦвета.ТемноЗеленый,,СтрОткрытьОтчетПоКонтрагенту);
	//	КонецЕсли;	
	//	Инф2 = Новый ФорматированнаяСтрока(РазвернутаяЗадолженность);
	//	Инф3 = ?(ПросроченныйДолг=0,Новый ФорматированнаяСтрока("отгрузка разрешена",,WebЦвета.Зеленый), ТекстОтгрузкаЗапрещена); 
	//	
	//Иначе
	//	Инф1 = Новый ФорматированнаяСтрока("никто никому не должен",,,,);
	//	Инф2="";
	...
	//КонецЕсли;	
	//
	//Возврат Новый Структура("Инф1, Инф2, Инф3, Документ, Дата, Сумма",Инф1, Инф2, Инф3,,,);
...
//Функция СобратьИнформациюОДолгеВФоне(Партнер, УникИдентификаторФормы, ФормаОтношений = Неопределено) Экспорт
//	
//	Возврат ЗапуститьПоток(
//					"Результат = ИнформацияОДолгеПартнера(Параметры.Партнер)", 
