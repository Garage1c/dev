...
Функция ПолучитьПередачиТовараPOST(ЗапросHTTP = Неопределено, стрJSONСтруктураДляТестов = Неопределено) Экспорт
	...
	//Если НЕ СтруктураЗапроса.Свойство("data_begin") Тогда Возврат ВернутьОшибку(Ответ, "Не указана дата начала для отбора данных, свойство data_begin") КонецЕсли;
	//Если НЕ СтруктураЗапроса.Свойство("data_end") Тогда Возврат ВернутьОшибку(Ответ,"Не указана дата окончания для отбора данных, свойство data_end") КонецЕсли;
