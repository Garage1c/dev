...
Процедура УстановитьСвязиГрузополучателя(Объект,Элементы,СтруктураСвязей) Экспорт
	...
	Если Объект.Свойство("Грузоотправитель") Тогда Если НЕ ЗначениеЗаполнено(Объект.Грузоотправитель) Тогда Объект.Грузоотправитель = ПустойКонтрагент; ИначеЕсли ТипЗнч(Объект.Грузоотправитель)<>Тип("СправочникСсылка.Грузополучатели") Тогда Элементы.Грузоотправитель.ВыбиратьТип = Истина; КонецЕсли; КонецЕсли;
	Если Объект.Свойство("Грузополучатель")  Тогда Если НЕ ЗначениеЗаполнено(Объект.Грузополучатель)  Тогда Объект.Грузополучатель  = ПустойКонтрагент; ИначеЕсли ТипЗнч(Объект.Грузополучатель) <>Тип("СправочникСсылка.Грузополучатели") Тогда Элементы.Грузополучатель.ВыбиратьТип  = Истина; КонецЕсли; КонецЕсли;
