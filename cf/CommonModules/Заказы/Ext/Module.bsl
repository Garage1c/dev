...
Функция ПолучитьРеквизитыЗаказаДляЗаполненияШапкиДокумента(Заказ) Экспорт
	...
	|ИЗ
	|	(
	|		ВЫБРАТЬ СуммаВключаетНДС, УчитыватьНДС, Ссылка, Склад, Контрагент, Валюта, ТипЦен, БанковскийСчетОрганизации, БанковскийСчетПартнера, Грузополучатель, Грузоотправитель, Организация, ДисконтнаяКарта, БанковскийСчетГрузоотправителя, БанковскийСчетГрузополучателя, Смена, Плательщик, Отдел, Подразделение, РасчетСкидкиОтЦены
	|		ИЗ 		Документ.ЗаказПокупателя
	|		ГДЕ		Ссылка = &Заказ
	|
	|		ОБЪЕДИНИТЬ ВСЕ
	|
	|		ВЫБРАТЬ СуммаВключаетНДС, УчитыватьНДС, Док.Ссылка, Док.Склад, Док.Контрагент, Док.Валюта, Док.ТипЦен, Док.БанковскийСчетОрганизации, Док.БанковскийСчетПартнера, Грузополучатель, Грузоотправитель, Организация, ДисконтнаяКарта, &ПустойБанковскийСчет, &ПустойБанковскийСчет, &ПустаяСмена, Плательщик, Отдел, Подразделение, РасчетСкидкиОтЦены
...
// Изменения - структура: Ключ - имя реквизита, Заначение - значение реквизита для записи
Функция СохранитьИзмененияРеквизитовЗаказа(ЗаказСсылка, Изменения, ИзмененияТЧ = Неопределено) Экспорт
	...
	Если Изменения.Свойство("Организация") ИЛИ Изменения.Свойство("Контрагент") ИЛИ Изменения.Свойство("ОтветственноеЛицо") ИЛИ Изменения.Свойство("СуммаВключаетНДС") Тогда
...
Функция ПолучитьТаблицуРазмещения(Заказ, Параметры = Неопределено) Экспорт
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ	Номенклатура, ЗаказПоставщику, КоличествоОстаток, 0, 0, 0 , 0
...
Процедура ЗаполнитьТаблицыРазмещений(Заказ, ТаблицаТоваров, ТаблицаРазмещения, Параметры = Неопределено) Экспорт
	...
	//+Андриянов 19.06.2017
	//Закомментировали, так как когда возвращаем от менеджера идёт автоматическое резервирование - БЕДА БЕДА!
...
Функция ПолучитьТаблицуТоваровСтарымСпособом(СсылкаПроцесс, СсылкаЗадача = Неопределено, ЕстьСуммы = Истина) Экспорт
	...
	Если ТипЗнч(Заказ) = Тип("ДокументСсылка.ВнутреннийЗаказ") Тогда
		ТипЦен = Неопределено; Валюта = Неопределено; Контрагент = Неопределено; ЕстьАкция = Ложь; ЕстьЦенаПоАкции = Ложь;
	Иначе
		ТипЦен = Заказ.ТипЦен; Валюта = Заказ.Валюта; Контрагент = Заказ.Контрагент; ЕстьАкция = Истина; ЕстьЦенаПоАкции = Истина;
	КонецЕсли;
	
	Если ЕстьСуммы Тогда
		СтруктураКолонокТовары = Новый Структура("ЕстьПроцентРучнойСкидки, ЕстьПроцентАвтоматическойСкидки, ТипЦен, Валюта, Контрагент, ЕстьАкция, ЕстьЦенаПоАкции, ЕстьУпаковка, ЕстьКоличество, ЕстьЦена, ЕстьСуммаБезСкидки, ЕстьСумма, ЕстьСуммаАвтоматическойСкидки, ЕстьСуммаРучнойСкидки, ЕстьСтавкаНДС, ЕстьСуммаНДС, СуммаВключаетНДС, ЕстьВсего, ЕстьДоставка, ЕстьАкция",
												Истина, Истина, ТипЦен, Валюта, Контрагент, ЕстьАкция, ЕстьЦенаПоАкции, Истина, Истина, Истина, Истина, Истина, Истина, Истина, Истина, Истина, СсылкаПроцесс.Заказ.СуммаВключаетНДС, Ложь, ЕстьДоставка, Истина);
	Иначе
		СтруктураКолонокТовары = Новый Структура("ЕстьПроцентРучнойСкидки, ЕстьПроцентАвтоматическойСкидки, ТипЦен, Валюта,Контрагент, ЕстьАкция, ЕстьЦенаПоАкции, ЕстьУпаковка, ЕстьКоличество, ЕстьЦена, ЕстьСуммаБезСкидки, ЕстьСумма, ЕстьСуммаАвтоматическойСкидки, ЕстьСуммаРучнойСкидки, ЕстьСтавкаНДС, ЕстьСуммаНДС, СуммаВключаетНДС, ЕстьВсего, ЕстьДоставка, ЕстьАкция",
												Ложь, Ложь, ТипЦен, Валюта, Контрагент, ЕстьАкция, ЕстьЦенаПоАкции, Истина, Истина, Ложь, Ложь, Ложь, Ложь, Ложь, Ложь, Ложь, Ложь, Ложь, Ложь, Ложь);
	КонецЕсли;
...
#Область Запрос
	...
	|	Запр.НомерСтроки,
	|	Запр.Номенклатура,
	|	Запр.Номенклатура.БольшеНеПоставляется КАК БольшеНеПоставляется,
...
#КонецОбласти
	...
	Если ЕстьСуммы Тогда
		...
		Иначе	
			ТипЦен = Заказ.ТипЦен; Валюта = Заказ.Валюта; Контрагент = Заказ.Контрагент;  ЕстьАкция = Истина; ЕстьЦенаПоАкции = Истина; КонецЕсли;
		
		СтруктураКолонокТовары = 
			Новый Структура("ЕстьПроцентРучнойСкидки, ЕстьПроцентАвтоматическойСкидки, ТипЦен, Валюта, Контрагент, ЕстьАкция, ЕстьЦенаПоАкции, ЕстьУпаковка, ЕстьКоличество, ЕстьЦена, ЕстьСуммаБезСкидки, ЕстьСумма, ЕстьСуммаАвтоматическойСкидки, ЕстьСуммаРучнойСкидки, ЕстьСтавкаНДС, ЕстьСуммаНДС, СуммаВключаетНДС, ЕстьВсего, ЕстьДоставка, ЕстьАкция, ЕстьЦенаДо, Касса, РасчетСкидкиОтЦены",
					Истина, Истина, ТипЦен, Валюта, Контрагент, ЕстьАкция, ЕстьЦенаПоАкции, Истина, Истина, Истина, Истина, Истина, Истина, Истина, Истина, Истина, Заказ.СуммаВключаетНДС, Истина, ЕстьДоставка, Истина, ЕстьДоставка, Касса, Заказ.РасчетСкидкиОтЦены);
...
Функция СформироватьТаблицуВозвратаРезультат(СтруктураКолонокТовары, СтрокаСверткиГруппы, СтрокаСверткиПоказатели, Заказ = Неопределено, РасчетСумм = Истина, ЕстьДоставка = Ложь)
	...
	Если РасчетСумм И ЗначениеЗаполнено(Заказ) Тогда
		ТипЦен = Заказ.ТипЦен; Валюта = Заказ.Валюта; Контрагент = Заказ.Контрагент; 
	Иначе
		ТипЦен = Неопределено; Валюта = Неопределено; Контрагент = Неопределено;  
	КонецЕсли;
	
	СтруктураКолонокТовары = Новый Структура("ЕстьПроцентРучнойСкидки, ЕстьПроцентАвтоматическойСкидки, ТипЦен, Валюта, Контрагент, ЕстьАкция, ЕстьЦенаПоАкции, ЕстьУпаковка, ЕстьКоличество, ЕстьЦена, ЕстьСуммаБезСкидки, ЕстьСумма, ЕстьСуммаАвтоматическойСкидки, ЕстьСуммаРучнойСкидки, ЕстьСтавкаНДС, ЕстьСуммаНДС, СуммаВключаетНДС, ЕстьВсего, ЕстьДоставка, ЕстьАкция",
												РасчетСумм, РасчетСумм, ТипЦен, Валюта, Контрагент, РасчетСумм, РасчетСумм, Истина, Истина, РасчетСумм, РасчетСумм, РасчетСумм, РасчетСумм, РасчетСумм, РасчетСумм, РасчетСумм, ?(Заказ = Неопределено, РасчетСумм, Заказ.СуммаВключаетНДС), РасчетСумм, ЕстьДоставка, РасчетСумм);
...
Функция ПолучитьПараметрыДляТабличногоДокумента(Ссылки, СтрНомера) Экспорт
	...
	Для Каждого Ссылка Из Ссылки Цикл 
		...
		Структура = Новый Структура("Заказ, Контрагент", СсылкаДок, СсылкаДок.Контрагент); 
...
Процедура ПриЗаписиЗаказаПриЗаписи(Источник, Отказ) Экспорт
	...
	Если Не Источник.ОбменДанными.Загрузка Тогда
		...
		Если Источник.ДополнительныеСвойства.Свойство("ИзмененныеРеквизиты") Тогда
			...
			Если Изменения.Свойство("Контрагент") ИЛИ Изменения.Свойство("Орагнизация") Тогда
		...
		Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.КорректировкаЗаказаПокупателя") ИЛИ ТипЗнч(Ссылка) = Тип("ДокументСсылка.КорректировкаИнтернетЗаказа")  Тогда
				Склад = Ссылка.Заказ.Склад; МВЗ = Ссылка.Заказ.МВЗ;  Контрагент = ссылка.Заказ.Контрагент
		Иначе   Склад = Ссылка.Склад;       МВЗ = Ссылка.МВЗ;		 Контрагент =ссылка.Контрагент
		КонецЕсли;
...
Процедура ОбработкаПроведенияЗаказаОбработкаПроведения(Источник, Отказ, РежимПроведения) Экспорт
	...
	Если ТипЗнч(Заказ) = Тип("ДокументСсылка.ИнтернетЗаказПокупателя") и Заказ.Склад.ПередачаТовараМВЗ Тогда
		//Если Заказ.Склад.ПередачаТовараМВЗ Тогда
			стрОшибки = "";
		// проверми не привешен ли лимит для контрагента
		ОтказПоЛимиту = Ложь;
...
// Получить таблицу соответствия документ - статус
//
// Возвращаемое значение:
//   ТаблицаЗначений   - Таблица соответствий документ - статус
//
Функция ПолучитьТаблицуСоответствий() Экспорт 
	...
	ТипСтатус = Новый ОписаниеТипов("ПеречислениеСсылка.СтатусыЗаказаПоставщику");
	
	ТаблицаСоответствиеДокументСтатус = Новый ТаблицаЗначений;
	ТаблицаСоответствиеДокументСтатус.Колонки.Добавить("Документ", ТипыДокументов);
	ТаблицаСоответствиеДокументСтатус.Колонки.Добавить("Статус", ТипСтатус);
	
	Возврат ТаблицаСоответствиеДокументСтатус;
...
//
// Параметры:
//  Документ  - ДокументСсылка.ЗаказПоставщику, ДокументСсылка.ПоступлениеТоваров - документ, по которому получаем статус.
// Возвращаемое значение:
//   Перечисление.СтатусыЗаказовПоставщикам - текущий статус заказа поставщикам
//
Функция ТекущийСтатусДокумента(Документ) Экспорт 
	
	ВозвращаемоеЗначение = Перечисления.СтатусыЗаказаПоставщику.ПустаяСсылка();
	...
	|ИЗ
	|	РегистрСведений.СтатусыДокументов.СрезПоследних(, Документ = &Документ) КАК СтатусыДокументовСрезПоследних";
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		...
		ВозвращаемоеЗначение = ВыборкаДетальныеЗаписи.Статус;
		
	КонецЕсли;
	
	Возврат ВозвращаемоеЗначение;
	
КонецФункции // ТекущийСтатусДокумента()

// Устанавливает новый статус документа, если он изменился.
//
// Параметры:
//		ТаблицаДокументов - ТаблицаЗначений
//  		Документ  - ДокументСсылка.ЗаказПоставщику, ДокументСсылка.ПоступлениеТоваров - документ, для которого выставляем статус           
//  		Статус  - Перечисление.СтатусыЗаказовПоставщикам - устанавливаемы статус документа
//		ДатаИзменения - Дата - Момент изменения статуса
//
Процедура НовыеСтатусыДокументов(ТаблицаДокументов, ДатаИзменения) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	СоответствиеДокументСтатус.Документ КАК Документ,
		|	СоответствиеДокументСтатус.Статус КАК Статус
		|ПОМЕСТИТЬ СтатусыДокументов
		|ИЗ
		|	&СоответствиеДокументСтатус КАК СоответствиеДокументСтатус
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ЕСТЬNULL(СтатусыДокументовСрезПоследних.Статус, ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказаПоставщику.ПустаяСсылка)) КАК СтатусТекущий
		|ПОМЕСТИТЬ ТекущиеСтатусыДокументов
		|ИЗ
		|	СтатусыДокументов КАК СтатусыДокументов
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыДокументов.СрезПоследних КАК СтатусыДокументовСрезПоследних
		|		ПО СтатусыДокументов.Документ = СтатусыДокументовСрезПоследних.Документ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТекущиеСтатусыДокументов.Документ,
		|	ТекущиеСтатусыДокументов.Статус
		|ИЗ
		|	ТекущиеСтатусыДокументов КАК ТекущиеСтатусыДокументов
		|ГДЕ
		|	ТекущиеСтатусыДокументов.Статус <> ТекущиеСтатусыДокументов.СтатусТекущий";
	
	Запрос.УстановитьПараметр("СоответствиеДокументСтатус", ТаблицаДокументов);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Набор = РегистрыСведений.СтатусыДокументов.СоздатьНаборЗаписей();
		Набор.Отбор.Период.Установить(ДатаИзменения);
		Набор.Отбор.Документ.Установить(ВыборкаДетальныеЗаписи.Документ);
...
КонецПроцедуры // НовыйСтатусДокумента()

// Добавляет документ в таблицу соответствий документ - статус
//
// Параметры:
//  ТаблицаДокументов - ТаблицаЗначений
//  		Документ  - ДокументСсылка.ЗаказПоставщику, ДокументСсылка.ПоступлениеТоваров - документ, для которого выставляем статус           
//  		Статус  - Перечисление.СтатусыЗаказовПоставщикам - устанавливаемы статус документа
//  Документ  - ДокументСсылка.ЗаказПоставщику, ДокументСсылка.ПоступлениеТоваров - документ, добавляемый в таблицу
//  Статус  - Перечисление.СтатусыЗаказовПоставщикам - устанавливаемы статус документа
//
Процедура ДокументВТаблицуСоответствий(ТаблицаДокументов, Документ, Статус) Экспорт 
	
	НоваяЗапись = ТаблицаДокументов.Добавить();
	НоваяЗапись.Документ = Документ;
	НоваяЗапись.Статус  = Статус;

КонецПроцедуры // ДокументВТаблицуСоответствий()
