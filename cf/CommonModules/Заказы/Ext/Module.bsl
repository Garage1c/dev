...
Функция ПроставитьРазмещениеВТаблицеТоваровПоСкладам(ТоварыТЗ, СкладИлиМассивОчередностиСкладов = Неопределено, Дата = Неопределено, ДокументРезерва = Неопределено, ИмяКолонкиРазмещение = "Размещение", СвойРезервВПриоритете = Истина)
	...
	ОтданныеТовары.Колонки.Добавить("Склад",);
...
Процедура ПроставитьРазмещениеВТаблицеТоваров(
	...
	Если ЭтоРазмещениеБезЗаказа Тогда
		...
		Если СпособРазмещения.ОчиститьРазмещение Тогда
			...
			Для Ном = 1 По КолСтрок Цикл Инд = Инд - 1; 
				...
				Если ТоварыТЗ.НайтиСтроки(Новый Структура("Номенклатура", Строка.Номенклатура)).Количество() Тогда
					Если Строка.КоличествоМинимум Тогда
							Строка.Количество = Строка.КоличествоМинимум;
					Иначе	РазмещениеТоваров.Удалить(Строка); КонецЕсли; КонецЕсли; КонецЦикла;
		
			// Затем добавим пустые строки
			Для Каждого Строка Из ТоварыТЗ Цикл 
				НужноДобавить = Строка.Количество - КонвертацияТипов.ПолучитьСуммуКолонкиПоУсловию(РазмещениеТоваров, "Количество", Новый Структура("Номенклатура", Строка.Номенклатура));
				Если НужноДобавить > 0 Тогда
					НовСтрока = РазмещениеТоваров.Добавить(); 
					ЗаполнитьЗначенияСвойств(НовСтрока, Строка); 
					НовСтрока[ИмяКолонкиРазмещение] = ""; КонецЕсли; КонецЦикла;
				
			//	НовСтрока = РазмещениеТоваров.Добавить(); 
			//	ЗаполнитьЗначенияСвойств(НовСтрока, Строка); 
				
				//КоличествоМинимум = Строка.КоличествоОтгружено + Строка.КоличествоСобрано;
				//Если КоличествоМинимум Тогда НовСтрока.КоличествоМинимум = КоличествоМинимум КонецЕсли;
				//
				//Очистить = Строка.Количество - КоличествоМинимум;
				//Если Очистить Тогда
				//	НовСтрока = РазмещениеТоваров.Добавить(); 
				//	ЗаполнитьЗначенияСвойств(НовСтрока, Строка); 
				//	НовСтрока.Количество = Очистить;
				//	НовСтрока.КоличествоМинимум = 0;
				//	НовСтрока[ИмяКолонкиРазмещение] = "" КонецЕсли; КонецЦикла;
				
			//Для Каждого Строка Из ТоварыТЗ Цикл НовСтрока = РазмещениеТоваров.Добавить(); ЗаполнитьЗначенияСвойств(НовСтрока, Строка); НовСтрока[ИмяКолонкиРазмещение] = Неопределено; НовСтрока.КоличествоМинимум = НовСтрока.КоличествоМинимум - Строка.КоличествоОтгружено - Строка.КоличествоСобрано; КонецЦикла;
	...
	Иначе
		...
		Если 	СпособРазмещения <> Неопределено И
			...
			ТоварыТЗ.ЗаполнитьЗначения("", ИмяКолонкиРазмещение); КонецЕсли; 
	...
	Если 	СпособРазмещения <> Неопределено И
		...
		ПроставитьРазмещениеВТаблицеТоваровПоЗаказамПоставщику(?(ЭтоРазмещениеБезЗаказа, Таблица ,ТоварыТЗ), Дата, ДокументРезерва, ИмяКолонкиРазмещение); КонецЕсли;
	...
	Если ЭтоРазмещениеБезЗаказа Тогда
		...
		РазмещениеТоваров.Загрузить(Таблица); КонецЕсли;
...
Функция МожноПрименитьОленя(Заказ) Экспорт
	...
	Запрос = Новый Запрос("
	|ВЫБРАТЬ Ссылка ПОМЕСТИТЬ Ссылки ИЗ Документ.СборочныйЛист.Товары ГДЕ &Заказ = Заказ;
	
	|ВЫБРАТь ПЕРВЫЕ 1 Ячейка 	ИЗ РегистрНакопления.РезервыВЯчейках.Остатки(,СборочныйЛист В (ВЫБРАТЬ Ссылка ИЗ Ссылки)) ОБЪЕДИНИТЬ ВСЕ 
	|ВЫБРАТЬ ПЕРВЫЕ 1 Склад 	ИЗ РегистрНакопления.ТоварыСобранные.Остатки(,СборочныйЛист В (ВЫБРАТЬ Ссылка ИЗ Ссылки))");
...
Функция ПолучитьСостояниеТоваров(СсылкаПроцессаИлиЗаказИлиМассивЗаказов, ДатаСостояния = Неопределено, УчитыватьБПРодитель = Истина, ПоказатьОжидающие = Истина) Экспорт
	
	// При передачи массивов заказа, ВАЖНО чтобы склад получатель был у них одинаковый!
	// т.к. склад береться у первого заказа в массиве (см. строку где определяется СкладПолучатель)
	// если нет такой возможности передать с одинаковм складом, тогда не передавайте ничего сюда, и вобще забудте дорогу в эту функцию
	
	Если ТипЗнч(СсылкаПроцессаИлиЗаказИлиМассивЗаказов) = Тип("Массив") Тогда
		текЗаказы = СсылкаПроцессаИлиЗаказИлиМассивЗаказов;
	Иначе
		текЗаказы = Новый Массив;
		
		Если ТипЗнч(СсылкаПроцессаИлиЗаказИлиМассивЗаказов) = Тип("БизнесПроцессСсылка.ПеремещениеТоваров") Тогда 	
			текЗаказы.Добавить(СсылкаПроцессаИлиЗаказИлиМассивЗаказов.Заказчик.Заказ);
		ИначеЕсли Метаданные.Документы.Содержит(СсылкаПроцессаИлиЗаказИлиМассивЗаказов.Метаданные()) Тогда			
			текЗаказы.Добавить(СсылкаПроцессаИлиЗаказИлиМассивЗаказов);
		Иначе																						
			текЗаказы.Добавить(СсылкаПроцессаИлиЗаказИлиМассивЗаказов.Заказ); КонецЕсли; КонецЕсли;
	...
	|	Поместить Состояния
	|ИЗ
	|		РегистрНакопления.ЗаказыНаПеремещения.Остатки(,Заказ В(&Заказы)) Зак 
	|
	|	Внутреннее соединение
	|		РегистрНакопления.ТоварыСобранные.Остатки(,Заказ В(&Заказы)) КАК Собрано
	|	По Зак.Номенклатура = Собрано.Номенклатура  и Зак.СкладОтправитель = Собрано.Склад И Зак.Заказ = Собрано.Заказ
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ИЗ
	|		РегистрНакопления.ТоварыВПути.Остатки(,Заказчик В(выбрать ссылка из бизнесПроцесс.СборкаЗаказа ГДЕ Заказ В(&Заказы)))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ИЗ
	//|		РегистрНакопления.РезервыВЯчейках.Остатки(,СборочныйЛист в (выбрать ссылка из Документ.СборочныйЛист ГДЕ Заказ В(&Заказы)))
	|		РегистрНакопления.РезервыВЯчейках.Остатки(,Заказ В (&Заказы))
	|	Сгруппировать по Заказ, Номенклатура, Ячейка.Владелец
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ИЗ
	|		РегистрНакопления.ТоварыВСборке.Остатки(,Заказ В(&Заказы)) Сбор
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		(	ВЫБРАТЬ Заказ, Номенклатура, Ячейка.Владелец Склад, Сумма(КоличествоОстаток) Количество ИЗ 
	//|			РегистрНакопления.РезервыВЯчейках.Остатки(,СборочныйЛист в (выбрать ссылка из Документ.СборочныйЛист ГДЕ Заказ В(&Заказы)))
	|			РегистрНакопления.РезервыВЯчейках.Остатки(,Заказ В (&Заказы))
	|			Сгруппировать по Заказ, Номенклатура, Ячейка.Владелец) Рез
	|	ПО
	|		Сбор.Заказ 			= Рез.Заказ И
	...
	|	ИЗ
	|		РегистрНакопления.ТоварыСобранные.Остатки(,Заказ В(&Заказы) И Склад=&СкладПолучатель)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ИЗ
	|		РегистрНакопления.ОтменаСборкиЗаказа.Остатки(,Заказ В(&Заказы))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ГДЕ Заказ В(&Заказы)
	|	Сгруппировать по Продажи.Заказ, Номенклатура, Упаковка, ЕСТЬNULL(СостоянияДокументовОтгрузки.СостояниеСборкиЗаказа,Значение(Перечисление.СостояниесборкиЗаказа.Отгружен))
	|	Имеющие Сумма(Количество)>0		
	//-Андриянов
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ГДЕ ВнутреннийЗаказ В(&Заказы)
	|	и ВидДвижения = Значение(ВидДвиженияНакопления.Расход)
	|	Сгруппировать по ВнутреннийЗаказ, Номенклатура, Упаковка
	|	Имеющие Сумма(Количество)>0		
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ИЗ
	|		РегистрНакопления.РазмещениеЗаказов.Остатки(,Заказ В(&Заказы))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ГДЕ Заказ В(&Заказы)
	| 	и Количество<0
	| Сгруппировать по Заказ, Номенклатура, Упаковка, Склад
	|
	|;
	...
	|   ВЫБРАТЬ Заказ, Номенклатура, Размещение, Сумма(КоличествоВычитаемоеИзРезерва) 
	|		Поместить СостоянияВычитаемыеИзРезерва
	|	из Состояния ГДЕ КоличествоВычитаемоеИзРезерва>0
	|		Сгруппировать по Заказ, Номенклатура, Размещение
	|;	
	...
	|	ИЗ
	|		РегистрНакопления.ТоварыВРезерве.Остатки(,ДокументРезерва В(&Заказы)) Резервы
	|  левое соединение
	|         СостоянияВычитаемыеИзРезерва КАК Состояния
	|		по Резервы.ДокументРезерва = Состояния.Заказ И Резервы.Номенклатура=Состояния.Номенклатура и Резервы.Размещение=Состояния.Размещение
	...
	|           Выбрать Заказ, Номенклатура,Упаковка,Сумма(Количество)
	|			Поместить ТабДок
	|			из
	|			(
	|			ВЫБРАТЬ
	|				Ссылка Заказ, Номенклатура,Упаковка,Количество 
	|			ИЗ
	...
	|			ГДЕ Ссылка.Заказ В(&Заказы) и Ссылка.Проведен
	//костылёг для внутренних заказов
	|			) Доки
	|
	| 			Сгруппировать по Заказ, Номенклатура, Упаковка
	|;
	...
	|   Левое соединение
	|	(
	|		Выбрать Заказ, Номенклатура, Сумма(Количество)
	|			из Состояния
	|		Сгруппировать по Заказ, Номенклатура
	|	) КАК СостоянияГрп
	|
	|по ТабДок.Заказ = СостоянияГрп.Заказ И ТабДок.Номенклатура = СостоянияГрп.Номенклатура 
	|
	|Левое соединение
	|    (
	|	Выбрать Заказ, Номенклатура, Сумма(Количество)
	...
	|
	|По ТабДок.Заказ = РезервыБезСостояния.Заказ И ТабДок.Номенклатура = РезервыБезСостояния.Номенклатура
	|
	...
	|) КАК ВТ
	|
	|Упорядочить по артикул, Размещение, Заказ
	|
	|", 
	"Документ.ЗаказПокупателя", 			// это заменить 
	текЗаказы[0].Метаданные().ПолноеИмя())); // на ето
	
	Запрос.УстановитьПараметр("Заказы", 		текЗаказы);
	Запрос.УстановитьПараметр("СкладПолучатель",?(ТипЗнч(текЗаказы[0]) = Тип("ДокументСсылка.ВнутреннийЗаказ"), текЗаказы[0].Заказчик, текЗаказы[0].Склад));
	
	Возврат Запрос.Выполнить().Выгрузить();
	
КонецФункции
Функция ПолучитьСостояниеТоваров_ДоТогоКакЗаказыСталиМассивом(СсылкаПроцессаИлиЗаказ, ДатаСостояния = Неопределено, УчитыватьБПРодитель = Истина, ПоказатьОжидающие = Истина) Экспорт
