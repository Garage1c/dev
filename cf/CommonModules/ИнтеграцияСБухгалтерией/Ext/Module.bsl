...
Функция ЗаполнитьОбъектПоСтрокеЗапроса(Connector, ОбъектБух, СтруктураОбъект, ПеренесенныеОбъекты, Изменился)
	...
	стрИсключения = "Ссылка, Контрагент, Касса, ПометкаУдаления, Проведен, ИмяДокумента";
...
Функция ПолучитьДоговор81ИзОрганизации(Connector, ПартнерИлиКонтрагент, КонтрагентКакДоговор = Неопределено, Организация81, ПеренесенныеОбъекты, СоздаватьНовыйЕслиНеНайден = Истина, ПараметрыДоговора = Неопределено, ДопПараметрыДоговора = Неопределено)
	...
	// ели наоборот то контрагент становится контрагентом а контрагент договором
	...
	Иначе //если договор жестко привязан к контрагенту а контрагент к контрагенту
...
Функция ПеренестиКонтактнуюИнформациюКонтрагента(Connector, СсылкаБух, КонтрагентСсылка, ПеренесенныеОбъекты)
	...
	|", Новый Структура("Контрагент", КонтрагентСсылка));
...
Функция Получить_СправочникКонтрагенты(Connector, СсылкаНаОбъект, ПеренесенныеОбъекты, ЭтоГруппа = Ложь)
	...
	Если НЕ ПеренестиКонтактнуюИнформациюКонтрагента(Connector, СсылкаБух, СсылкаНаОбъект, ПеренесенныеОбъекты) Тогда
		Возврат Неопределено; КонецЕсли;
...
Функция Получить_ДокументПлатежноеПоручениеВходящее(Connector, СсылкаНаОбъект, ПеренесенныеОбъекты, Версия = Неопределено)
	...
	|	Организация,
	|	Контрагент,
	|	ВЫБОР КОГДА Контрагент.ЭтоКонтрагентБухгалтерии ТОГДА Контрагент ИНАЧЕ НЕОПРЕДЕЛЕНО 	КОНЕЦ ДоговорКонтрагента,
