...
Функция ПолучитьТекстовыеДанныеПоЮрЛицу(СсылкаОрганизация, Полный)
	...
	Если Телефон = Неопределено И ТипЗнч(СсылкаОрганизация) = Тип("СправочникСсылка.Контрагенты") Тогда
		Телефон = ПерваяИнфаКонтактнойИнформации(СсылкаОрганизация, Справочники.ВидыКонтактнойИнформации.Телефон) КонецЕсли;
	Если Адрес = Неопределено И ТипЗнч(СсылкаОрганизация) = Тип("СправочникСсылка.Контрагенты") Тогда
		Адрес = ПерваяИнфаКонтактнойИнформации(СсылкаОрганизация.Контрагент, Справочники.ВидыКонтактнойИнформации.АдресФактический) КонецЕсли;
...
Функция ПолучитьДанныеДляЗаказаВЗаказе(СсылкаИлиМассивЗаказ) Экспорт
	...
	|ВЫБРАТЬ 	Номер, Дата, Организация, Грузоотправитель, Контрагент, Ответственный, СуммаВключаетНДС, Валюта, ЕСТЬNULL(Контрагент.НеВыводьКолонкуСкидкаВПечатнойФорме, ЛОЖЬ) НеВыводьКолонкуСкидкаВПечатнойФорме
...
Функция ПолучитьДанныеДляНемецкогоЗаказаВЗаказе(СсылкаЗаказ) Экспорт
	...
	|		ОБЪЕДИНИТЬ
	|		ВЫБРАТЬ Контрагент ИЗ Документ.ЗаказПокупателя ГДЕ Ссылка = &Ссылка) Запрос;
	...
	|	Контрагент,
	|	Контрагент.НомерКлиента 		НомерКлиента,
	|	Контрагент.ИНН 				ИНН,
	|	КонтактноеЛицо,
...
Функция ПолучитьДанныеДляНемецкогоЗаказаВЧеке(СсылкаЧек) Экспорт
	...
	|ВЫБРАТЬ 	Контрагент Грузоотправитель 
	|ПОМЕСТИТЬ 	ТаблицаДоставки 
...
Процедура Печатать_СчетЗаказНаНемецком(ТабДок, ДанныеДляПечати, СвоиОбласти, Префикс = "") Экспорт
	...
	СтруктураШапки.Вставить("Компания", 			СтруктураШапки.Контрагент);
...
Функция ПолучитьДанныеДляНемецкойНакладнойИзРеализации(СсылкаРеализация) Экспорт
	...
	|		ОБЪЕДИНИТЬ
	|		ВЫБРАТЬ Контрагент ИЗ Документ.РеализацияТоваров ГДЕ Ссылка = &Ссылка) Запрос;
...
Функция ПолучитьСборочныйЛист(БизнесПроцесс,ВыводитьПомеченные = Ложь, Сортировка = "Ячейка", VIPКонтрагент = Ложь) Экспорт
	...
	ОбластьШапка.Параметры.ВариантДоставки 	= БизнесПроцесс.Заказ.ВариантДоставкиНов;
	ОбластьШапка.Параметры.Грузоперевозчик 	= БизнесПроцесс.Заказ.Грузоперевозчик.НаименованиеПолное;	
	//neti Леконцев 28.06.2017 -
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ТоварыСобранныеОстатки.СборочныйЛист КАК Ссылка
	|ПОМЕСТИТЬ СборочныеЛисты
	|ИЗ
	|	РегистрНакопления.ТоварыСобранные.Остатки(
	|			,
	|			Заказ = &Заказ
	|				И Склад = &Склад) КАК ТоварыСобранныеОстатки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(СборочныйЛист.ЯчейкаСобранногоТовара) КАК ЯчейкиХранения
	|ИЗ
	|	СборочныеЛисты КАК СборочныеЛисты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СборочныйЛист КАК СборочныйЛист
	|		ПО СборочныеЛисты.Ссылка = СборочныйЛист.Ссылка";
	
	Запрос.УстановитьПараметр("Заказ", БизнесПроцесс.Заказ);
	Запрос.УстановитьПараметр("Склад", Склад);
	
	РезультатЗапроса = Запрос.Выполнить();
	
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ОбластьСобранныйЗапас = Макет.ПолучитьОбласть("СобранныйЗаказ");
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			Ячейки  = Ячейки + ?(ЗначениеЗаполнено(Ячейки), "; ", "") + ВыборкаДетальныеЗаписи.ЯчейкиХранения;	
		КонецЦикла;
		
		ОбластьСобранныйЗапас.Параметры.Ячейки = Ячейки;
		
		ТабДокумент.Вывести(ОбластьСобранныйЗапас);		
	КонецЕсли;
	//neti Леконцев 28.06.2017 -
	...
	// silber { Сколько же их... заказов.
	
	ТабКолПоЗаказам = ТаблВывода.Скопировать(,"Заказ, Номенклатура, Количество"); 
	ТабКолПоЗаказам.Свернуть("Заказ, Номенклатура", "Количество");
	
	ТЗЗаказы = ТабКолПоЗаказам.Скопировать(,"Заказ"); 
	ТЗЗаказы.Свернуть("Заказ");
	МногоЗаказов = ТЗЗаказы.Количество() > 1;
	
	Если МногоЗаказов Тогда
		ОбластьЗаголовокТаблицы.Параметры.Количество = "з1-з2-..зN";
		ТаблВывода.Колонки.Удалить("Заказ");
		КонвертацияТипов.СвернутьАЧислаСуммировать(ТаблВывода); КонецЕсли;
	...
	Для каждого Строка Из СтрокиВывода Цикл Ном = Ном + 1;
		...
		//neti Леконцев 28.06.2017 +
		ОбластьСтрока.Параметры.Объем = Строка.Номенклатура.Объем;
		//neti Леконцев 28.06.2017 -
		Если Строка.Ячейка.Пустая() Тогда 
			ОбластьСтрока.Параметры.Ячейка = Склад; КонецЕсли;
			
		// silber { Добавим расшифровку количества
		
		Если МногоЗаказов Тогда
			КоличестваПоЗаказам = КонвертацияТипов.ПолучитьМассивИзНайденныхЗначенийСтрокТаблицыЗначений(ТабКолПоЗаказам, Новый Структура("Номенклатура", Строка.Номенклатура), "Количество");
			Если КоличестваПоЗаказам.Количество() > 1 Тогда
				ОбластьСтрока.Параметры.Количество = СтрШаблон("%1%2%3", Формат(Строка.Количество, "ЧДЦ=3"), Символы.ПС, СтрСоединить(КоличестваПоЗаказам, "-")) КонецЕсли; КонецЕсли;
