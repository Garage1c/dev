...
КонецПроцедуры
Функция ПолучитьПочтуПартнера(ПартнерСсылка) Экспорт
	...
	// Контрагент
	ДобавитьЗапросНаПоискПочты(ТекстЗапроса, "Объект.Партнер = &Объект И НЕ Объект.ПометкаУдаления И Вид В ИЕРАРХИИ(&ВидыПочты)");
	// Контактное лицо
	...
	Запрос.УстановитьПараметр("Объект", 	ПартнерСсылка);
