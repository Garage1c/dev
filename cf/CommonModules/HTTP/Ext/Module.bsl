...
//Функция ПолучитьGUIDПользователяИнтернетИзПартнера(СсылкаПартнер) Экспорт
//	
//	Запрос = Новый Запрос("ВЫБРАТЬ  РАЗРЕШЕННЫЕ ПользовательИнтернет ИЗ РегистрСведений.СвязьПартнераСПользователямиИнтернет ГДЕ Партнер = &Партнер");
...
Функция СформироватьОбновитьЗаказ(user = Неопределено, order, number = "", type_order = "", status = "", error = "", products = "", ЗаказОбъект = Неопределено) Экспорт
	...
	Если Не ЕстьЗаказ Тогда
		...
		Если Не СсылкаИнтернетПользователь.Партнер.Пустая() Тогда
