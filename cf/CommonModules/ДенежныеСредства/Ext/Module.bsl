...
Процедура ВытащитьПодразделениеОтделРекурсивно(Объект, ДокОбъект, Отработанные, СтруктураДокумента)
	...
	Если ОтделПодразделенияНеЗаполнен(ДокОбъект, СтруктураДокумента) Тогда
		...
		// Укажим какие реквизиты проверять впоисках подразделения и Контрагента
		...
		Если стрНайти(стрОбъект, "Документ") = 1 Тогда 	
				...
				Иначе	
					ПоследПросмотра = "Контрагент, Склад, Касса, Продавец"; // Это попытки просмотра у всех документов
				КонецЕсли;
		ИначеЕсли 	стрОбъект = "Справочник.Контрагенты" Тогда 	ПоследПросмотра = "ОсновнойМенеджер";
		КонецЕсли;
