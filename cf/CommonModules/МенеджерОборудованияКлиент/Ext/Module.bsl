...
//
Функция БПО_ПередНачаломРаботыСистемы() Экспорт
...
// Начать формирование отчета отчет без нашения  на фискальном устройстве.
// УникальныйИдентификатор - идентификатор формы.
// ИдентификаторУстройства - идентификатор устройства, если неопределенно - будет предложен выбор.
// ОповещениеПриЗавершении - оповещение при завершении завершении.
//
Процедура НачатьФормированиеОтчетаБезГашения(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено, ТипОборудования = Неопределено) Экспорт
...
Процедура НачатьЗакрытиеСменыНаФискальномУстройстве(ОповещениеПриЗавершении, УникальныйИдентификатор, ПараметрыОперации, ИдентификаторУстройства = Неопределено, ТипОборудования = Неопределено, КассоваяСмена = Неопределено) Экспорт
	...
	Если КассоваяСмена = Неопределено Тогда
		...
		Иначе
			...
			ОписаниеПоследнейСмены = КассовыеСменыВызовСервера.ОписаниеПоследнейКассовойСмены(ИдентификаторУстройства);
			Если ОписаниеПоследнейСмены = Неопределено Тогда
				
				Если ОповещениеПриЗавершении <> Неопределено Тогда
					ТекстСообщения = НСтр("ru = 'Нет кассовых смен для устройства '") + Строка(ИдентификаторУстройства);
...
Функция ПолучитьШаблонАтрибутовЧекаДляОтсылки(РегистрационныйНомерККТ, ЗаводскойНомерФН, ФискальныйПризнак, СуммаЧека, АдресСайтаПроверки, ДатаСеанса) Экспорт 
	...
	ТекстСообщения  = НСтр("ru='ККТ№:'") + РегистрационныйНомерККТ + ";" + Символы.НПП + 
		НСтр("ru='СУММА:'") + Формат(СуммаЧека, "ЧРД=,;ЧЦ=10;ЧДЦ=2;ЧН=0,00;ЧГ=0") + ";" + Символы.НПП + 
