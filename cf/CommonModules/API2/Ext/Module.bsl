...
Функция СформироватьОбновитьЗаказ(order, error = "", ЗаказОбъект = Неопределено, НеСоздаватьЗаказ = Ложь, Сайт = Неопределено)
	...
	Если Не ЕстьЗаказ Тогда
		...
		Если НЕ МВЗ = Неопределено Тогда // Заказ Северстали
			//Если НЕ СсылкаИнтернетПользователь = Неопределено И Не СсылкаИнтернетПользователь.Партнер.Пустая() Тогда
			//	Заказ.Партнер = СсылкаИнтернетПользователь.Партнер;
			//КонецЕсли;
			//Заказ.Партнер = МВЗ.Владелец.Партнер;
			...
			Запрос = Новый Запрос;
			//Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1 Табл.Организация КАК Организация ИЗ Справочник.Контрагенты.Организации КАК Табл ГДЕ Табл.ЗначениеПоУмолчанию И Табл.Ссылка.Партнер = &Партнер";
		...
		Иначе // Обычный заказ
			...
			// Попробуем поискать партнера по ИНН
			
			Если ЗаказJson.Свойство("inn") Тогда
				Если ЗначениеЗаполнено(ЗаказJson.inn) Тогда
					Заказ.ИНН = ЗаказJson.inn;
					//Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1 Партнер ИЗ Справочник.Контрагенты ГДЕ ИНН = """ + ЗаказJson.inn + """");
					Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1 Ссылка Контрагент, Партнер ИЗ Справочник.Контрагенты ГДЕ ИНН = """ + ЗаказJson.inn + """");
					Выборка = Запрос.Выполнить().Выбрать();
					Если Выборка.Следующий() Тогда 
						Заказ.Контрагент = Выборка.Контрагент;
						Заказ.Партнер = Выборка.Партнер;
						ФункцииФормДокументов.КонтрагентПриИзменении(Заказ);
						// Раз есть такой партнер то проверем его связь с интрнет пользователем и если связи не будет, тогда свяжем их
	...
	Если ЗаказJson.Свойство("line_items", СтрокиТоваровJson) Тогда
		...
		Для Каждого СтруктураСтроки Из СтрокиТоваровJson Цикл Ном = Ном + 1;
			...
			СтруктураКолонокТовары = Новый Структура("ЕстьПроцентРучнойСкидки, ЕстьПроцентАвтоматическойСкидки, ТипЦен, Валюта, Партнер, 
													...
													Истина, Истина, Заказ.ТипЦен, Заказ.Валюта, Заказ.Партнер,
...
Функция ДопКатегории()
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ Различные Контрагент.ИдДляСайта КАК ид, Контрагент.Наименование КАК Наименование ИЗ Документ.СогласованиеАссортимента.Партнеры КАК СогласованиеАссортиментаПартнеры СГРУППИРОВАТЬ ПО СогласованиеАссортиментаПартнеры.Контрагент";
	Выборка = Запрос.Выполнить().Выбрать();
...
#Область Текст_запроса
	...
	|	ЕСТЬNULL(Зак.Номенклатура, 		Инв.Номенклатура) 					Номенклатура,
	|	ЕСТЬNULL(Зак.ЗаказПоставщику, 	Инв.ЗаказПоставщику) 				ЗаказПоставщику,
	|	ЕСТЬNULL(Зак.ЗаказПоставщику.Партнер, Инв.ЗаказПоставщику.Партнер)	Партнер,
	...
	// выбираем данные о сроках производста и доставки по всем поставщикам для товаров из Инвойсов/ЗаказовПоставщику, доступных для размещения (нужно для сопоставления с Партнерам из Инвойсов/Заказов поставщику)
	...
	|	ЕСТЬNULL(Осн.Партнер, Люб.Партнер) Партнер,
	...
	Для Инд = 0 По ПослИнд Цикл
		...
		Если структураОбъекта.Свойство("ДисконтнаяКарта") Тогда
			...
			ИначеЕсли структураОбъекта.Свойство("Контрагент") И структураОбъекта.Контрагент.Пустая() И НЕ ПустаяСтрока(структураОбъекта.email) Тогда
				...
				|			КОГДА спр.Объект Ссылка Справочник.Партнеры ТОГДА спр.Объект = Карты.ВладелецКарты
...
Функция СоздатьЗапросНаСогласованиеТовара(reconcile, стрОшибки = "") Экспорт
	...
	Если  Структура.Свойство("id", id) Тогда
		...
		Запись.Партнер = Пользователь.ФизЛицо.Партнер;
