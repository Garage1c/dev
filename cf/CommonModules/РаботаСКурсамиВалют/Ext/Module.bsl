...
Функция ПолучитьМассивЗагружаемыхВалют() Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ Ссылка ИЗ Справочник.Валюты
	              | ГДЕ ВалютаЗагружается";
	
	ВыборкаВалют = Запрос.Выполнить().Выбрать();
	
	Пока ВыборкаВалют.Следующий() Цикл
		Результат.Добавить(ВыборкаВалют.Ссылка);
	КонецЦикла;
...
Функция ЗаполнитьДанныеКурсаДляВалюты(ВыбраннаяВалюта) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ РегКурсы.Период, РегКурсы.Курс, РегКурсы.Кратность
	              | ИЗ РегистрСведений.КурсыВалют.СрезПоследних(&ОкончаниеПериодаЗагрузки) КАК РегКурсы
	              | ГДЕ РегКурсы.Валюта = &ВыбраннаяВалюта";
...
Функция ЗагрузитьКурсыВалют() Экспорт
	
	Если ПолучитьФункциональнуюОпцию("НемецкийУчет") Тогда
		
		Обработки.КурсыВалютЕЦБ.Создать().ЗагрузитьКурсыДолараКЕвро();
		
	Иначе
		
		Запрос = Новый Запрос("ВЫБРАТЬ Ссылка Валюта, Код КодВалюты ИЗ Справочник.Валюты ГДЕ ВалютаЗагружается = ИСТИНА");	
		Валюты = Запрос.Выполнить().Выгрузить();

		НачалоПериодаЗагрузки 		= ТекущаяДата();
		ОкончаниеПериодаЗагрузки 	= ТекущаяДата();
			
		РаботаСКурсамиВалютКлиентСервер.ЗагрузитьКурсыВалютПоПараметрам(Валюты, НачалоПериодаЗагрузки, ОкончаниеПериодаЗагрузки);КонецЕсли;
...
Функция ПолучитьТекстЗапросаКурсыВалют(ИмяПараметра = "Валюта", ИмяПакета = "КурсыВалют", ПриставкаКИмени = "ВалЦен", ПриставкаКИмениУпр = "ВалУпр") Экспорт
	
	Возврат "ВЫБРАТЬ
	|	ВалЦен.Курс			" + ПриставкаКИмени + "Курс,
	|	ВалЦен.Кратность 	" + ПриставкаКИмени + "Кратность,
	|	ВалУпр.Курс			" + ПриставкаКИмениУпр + "Курс,
	|	ВалУпр.Кратность 	" + ПриставкаКИмениУпр + "Кратность
	|ПОМЕСТИТЬ " + ИмяПакета + "
	|ИЗ
	|РегистрСведений.КурсыВалют.СрезПоследних(&Период, Валюта = &" + ИмяПараметра + ") КАК ВалЦен 
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	|	РегистрСведений.КурсыВалют.СрезПоследних(&Период, Валюта В (ВЫБРАТЬ Значение ИЗ Константа.ВалютаУправленческогоУчета)) КАК ВалУпр
	|ПО ИСТИНА" ;
	
КонецФункции


Функция ПолучитьКурсыВалютВЗапросе(ИсточникСвязи, ИмяТаблицы = "ВалЦен", ИмяТаблицыУпр = "ВалУпр") Экспорт
	
	Возврат "РегистрСведений.КурсыВалют.СрезПоследних(, ) КАК " + ИмяТаблицы + " 
		|	ПО 
		|" + ИсточникСвязи + " =  " + ИмяТаблицы + ".Валюта
		|	ЛЕВОЕ СОЕДИНЕНИЕ 
		|		РегистрСведений.КурсыВалют.СрезПоследних(, Валюта В (ВЫБРАТЬ Значение ИЗ Константа.ВалютаУправленческогоУчета)) КАК " + ИмяТаблицыУпр + "
		|	ПО 
		|		ИСТИНА";
	
	
КонецФункции
Функция ПолучитьСуммуУпрВЗапросе(ПолеСуммы,  ИмяТаблицы = "КурсыВалют", ПриставкаКИмени = "ВалЦен", ПриставкаКИмениУпр = "ВалУпр") Экспорт
	
	Если НЕ ПустаяСтрока(ИмяТаблицы) Тогда ИмяТаблицы = ИмяТаблицы + "."; КонецЕсли; 
	
	Возврат  ПолеСуммы + " * (ЕСТЬNULL(" + ИмяТаблицы + ПриставкаКИмени + "Курс, 1) * ЕСТЬNULL(" + ИмяТаблицы + ПриставкаКИмениУпр + "Кратность, 1)) / (ЕСТЬNULL(" + ИмяТаблицы + ПриставкаКИмениУпр + "Курс, 1) * ЕСТЬNULL(" + ИмяТаблицы + ПриставкаКИмени + "Кратность, 1))";  
