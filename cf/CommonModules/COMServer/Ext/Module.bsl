...
Функция ВыполнитьСинхронизациюКонтактнойИнформации_ст(МетаданныеПолноеИмя, Ссылка, стрОшибки) Экспорт
    ...
    Если Connector <> Неопределено Тогда
		...
		ТекстЗапроса = "ВЫБРАТЬ
								...
								|ЗначениеПоУмолчанию,
								|Представление,
								|ВЫБОР КОГДА Объект ССЫЛКА Справочник.Контрагенты ТОГДА ""Контрагенты""
								|ИНАЧЕ
	...
	Если Connector <> Неопределено Тогда
		...
		Если РезультатЗапроса81 <> Неопределено Тогда
			...
			Пока Выборка.Следующий() Цикл
					...
					ГУИД81 = ПолучитьГУИД(Connector, Выборка.Контрагент); 
					Если ГУИД81 = Неопределено Тогда
						Продолжить;
					КонецЕсли;
						 
					СсылкаКонтрагент	= Справочники.Контрагент.ПолучитьСсылку(Новый УникальныйИдентификатор(ГУИД81));
					
					ОбъектКонтрагент = СсылкаКонтрагент.ПолучитьОбъект();
					Если ОбъектКонтрагент  = Неопределено Тогда
						Сообщить("Контрагент отсутствует в 8.2 - " + Выборка.ПредставлениеОбъекта);
						Продолжить;
					КонецЕсли;
					...
					Если ГУИД81 <> "00000000-0000-0000-0000-000000000000" Тогда
						...
						ОбъектКонтрагент.ТипЦен = СсылкаТипЦен; 
		...
		Если РезультатЗапроса81 <> Неопределено Тогда
			...
			Пока Выборка.Следующий() Цикл
					...
					СсылкаКонтрагент	= Справочники.Контрагенты.ПолучитьСсылку(Новый УникальныйИдентификатор(ГУИД81));
