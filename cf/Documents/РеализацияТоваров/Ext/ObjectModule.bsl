	...
	Если Не ОбменДанными.Загрузка И Не ПараметрыСеанса.КонтрольОстатковВСеансеОтключен И  ЗначениеЗаполнено(Заказ) И Проведен Тогда
		...
		События.ЗарегистрироватьСобытие("ОтгруженЗаказКлиенту",
			Новый Структура("Ссылка, Инициатор, Место, Название, КраткоеОписание, Параметры", 
					...
					Строка(Склад),
					"Отгрузка заказа",
					"Отгружен заказ " + Заказ.Номер + " (" + Партнер + ") на сумму " + Сумма + " " + Валюта),
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если 	РежимПроведения = РежимПроведенияДокумента.Оперативный И
			...
			Не CRM.ПартнеруРазрешенаОтгрузка(Партнер) Тогда
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипДанных = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		...
		Запрос = Новый Запрос("
		|ВЫБРАТЬ 
		|	Ссылка Заказ, Партнер, Контрагент, Склад, ТипЦен, Валюта, СуммаВключаетНДС,
		|   Товары.(
		|		Номенклатура,
		|		Упаковка,
		|		Количество,
	...
	ИначеЕсли ТипДанных = Тип("ДокументСсылка.ЗаказНаряд") Тогда
		...
		СтруктураКолонокТовары = Новый Структура("ЕстьПроцентРучнойСкидки, ЕстьПроцентАвтоматическойСкидки, ТипЦен, Валюта, Партнер, ЕстьАкция, ЕстьЦенаПоАкции, ЕстьУпаковка, ЕстьКоличество, ЕстьЦена, ЕстьСуммаБезСкидки, ЕстьСумма, ЕстьСуммаАвтоматическойСкидки, ЕстьСуммаРучнойСкидки, ЕстьСтавкаНДС, ЕстьСуммаНДС, СуммаВключаетНДС, ЕстьВсего",
												 Истина, Истина, ТипЦен, Валюта, Партнер, Истина, Ложь, Истина, Истина, Истина, Истина, Истина, Истина, Истина, Истина, Истина, Истина, Ложь);
...
Процедура ЗаполнитьДокументыОплаты(СсылкаРеализации = Неопределено) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|ГДЕ
		|	ДолгиПоОтгрузкам.Партнер = &Партнер
...
Процедура ЗаполнитьДокументыОплаты_Ст(СсылкаРеализации = Неопределено) Экспорт
	...
	|ГДЕ
	|	СуммаРасход > 0 И
	|	НЕ Регистратор ССЫЛКА Документ.РеализацияТоваров И
...
//Функция ПолучитьНомерИзБухгатерии()
//	
//	// Создадим запрос
//	
//	Сервер 			= "1cgerm2.garagetools.ru";
//	База 			= "Intrade"; 
...
//	Структура.Дата = Дата;
//	Структура.Организация 	= Новый Структура("Ссылка, Наименование", Организация, 	Организация.Наименование);

//	ЗапросHTTP.УстановитьТелоИзСтроки(w1_Json.JSON(Структура, Истина));
//	
//	// Отправим на сервак
//	
...
//Функция ПеренестиРеализациюВБухгалтерию()
//	
//	// Создадим запрос
//	
...
//	
//	Структура.Организация 	= Новый Структура("Ссылка, Наименование", Организация, 	Организация.Наименование);
//	Структура.Контрагент 	= Новый Структура("Ссылка, Наименование", Контрагент, 	Контрагент.Наименование);
////	Структура.Валюта 		= Новый Структура("Ссылка, Наименование", Валюта, 		Валюта.Наименование);
////	Структура.СуммаДокумента = Сумма;

//	
//	// Вложим в него документ
//	
//	ЗапросHTTP.УстановитьТелоИзСтроки(w1_Json.JSON(Структура, Истина));
//	
//	// Отправим на сервак
...
//	
//	Если Организация.ЗаимствоватьНумерациюИзБухгалтерии Тогда
//		Номер = ПолучитьНомерИзБухгатерии();
//		Если 	НЕ ПустаяСтрока(Номер) Тогда  СтандартнаяОбработка = Ложь;
//		Иначе	Комментарий = "Автоматическая выдача номера произошла с ошибкой. Необходимо назначить его самостоятельно в полу-автоматическом режиме";
//		КонецЕсли;
//	КонецЕсли;
...
////	Если ЭтоНовый() И 
//	Если Организация.ЗаимствоватьНумерациюИзБухгалтерии Тогда
//		 ПеренестиРеализациюВБухгалтерию();
//	КонецЕсли;
