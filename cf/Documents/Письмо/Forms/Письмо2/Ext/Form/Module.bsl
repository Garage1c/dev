...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда  // Заполним по умолчанию
		...
		Если Параметры.Кому <> Неопределено И Параметры.Кому.Количество() Тогда
			Параметры.Кому[0].Свойство("Контрагент", Объект.Контрагент); 
			ДобавитьКому(Параметры.Кому); 
		//Копия
	ОсМен = Объект.Контрагент.ОсновнойМенеджер;
	Если Не ОсМен = ПараметрыСеанса.ТекущийПользователь и Не ПустаяСтрока(ОсМен.Почта) Тогда
	ДобавитьКопия(Объект.Контрагент);  КонецЕсли;КонецЕсли;
...
Процедура ДобавитьКому(текКому)
	...
	Для Каждого Кто Из текКому Цикл 
		...
		НовСтрока.Кому 	= ?(Кто.Свойство("Почта"), Кто.Почта,Почта2Сервер.ПолучитьПочтуПартнера(Кто.Контрагент)); КонецЦикла;
	
КонецПроцедуры

&НаСервере
Процедура ДобавитьКопия(текКонтрагент)
	  		
		НовСтрока = Объект.КопияСписок.Добавить();
		НовСтрока.Партнер = текКонтрагент.Партнер;
		НовСтрока.Кому 	= текКонтрагент.ОсновнойМенеджер.Почта; 
...
Процедура ОбновитьПредставлениеПолучателей(ИмяТаблицы)
	...
	Для Каждого Строка Из Объект[ИмяТаблицы] Цикл 
		...
		стрКонтрагент = ?(Строка.Контрагент.Пустая(), "", "(" + Строка.Контрагент + ")");
		Стр = Стр +?(Стр = "","","; ") + Строка.Кому + " " + стрКонтрагент; КонецЦикла;
...
Процедура стрКомуНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Документ.Письмо.Форма.ВыборПолучателей", Новый Структура("стрТаблица,знКонтрагент", ПолучитьСтрТаблицу("КомуСписок"),ПолучитьзнПартнер()),,,,,Новый ОписаниеОповещения("ВыборПолучателей", ЭтаФорма, "КомуСписок"));
...
Процедура стрКомуПриИзменении(Элемент)
	...
	// Запомним Контрагента
	
	стКонтрагент = ?(Объект.КомуСписок.Количество(), Объект.КомуСписок[0].Контрагент, Неопределено);
	
	// Очистим всю таблицу и будем считать что Контрагент 1	
	
	Объект.КомуСписок.Очистить();
	НовСтрока = Объект.КомуСписок.Добавить();
	
	НовСтрока.Кому 		= стрКомуСписок;
	НовСтрока.Контрагент 	= стКонтрагент;
