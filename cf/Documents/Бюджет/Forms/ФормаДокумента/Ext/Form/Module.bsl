...
Процедура ЗаполнитьПоМоделиНаСервере()
	...
	Запрос = Новый Запрос;
	Запрос.Текст=
	"Выбрать СтатьяБюджета,Аналитика1,Аналитика2,Аналитика3,ТипАналитики1,ТипАналитики2,ТипАналитики3 
	|
...
Процедура УправлениеВидимостью()
	КолАналитик=Объект.МодельБюджета.КоличествоАналитик;
	Для Н=1 По 3 Цикл
...
&НаСервере
Функция ПоказателиНаСервере(ВыбранноеЗначение)
	//РезультатЗапроса= НастройкиДоступаБюджетирование();	
	//
	//Если РезультатЗапроса.Найти(ВыбранноеЗначение)=Неопределено Тогда
	//	Возврат Истина;
	//КонецЕСли;
	//
	//Возврат Ложь;
...
&НаКлиенте
Процедура ПоказателиСтатьяБюджетаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	//Отказ= ПоказателиНаСервере(ВыбранноеЗначение);
	//Если Отказ Тогда
	//    СтандартнаяОбработка = Ложь;
	//    ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Выбранная Статья недоступна к редактированию");
	//КонецЕсли; 
КонецПроцедуры
...
Процедура ПоказателиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	//	Если  ПоказателиНаСервере(Элемент.ТекущиеДанные.СтатьяБюджета) Тогда
	//		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Выбранная Статья недоступна к редактированию");
	//		Объект.Показатели.Удалить(Элемент.ТекущиеДанные.НомерСтроки-1) ;
	//	Иначе
	//		Для Н=1 По 12 Цикл
	//			Если Элементы.Показатели.ПодчиненныеЭлементы["ПоказателиЗначение"+Н].Доступность=Ложь Тогда
	//				Элемент.ТекущиеДанные["Значение"+Н]=0;
	//			КонецЕсли;
	//		КонецЦикла;
	//	КонецЕсли;
	//КонецЕСли;
