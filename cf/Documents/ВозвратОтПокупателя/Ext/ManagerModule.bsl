...
Процедура ИницилизироватьДополнительныеДанныеДокумента(Ссылка, ДополнительныеСвойства) Экспорт
	...
	//|
	//|ПРАВОЕ СОЕДИНЕНИЕ
	//|	(ВЫБРАТЬ Дата, Организация, Контрагент, Ссылка, Сумма ИЗ Документ.ВозвратОтПокупателя ГДЕ Ссылка = &Ссылка) Док
	//|ПО ИСТИНА
	//|
	//|ЛЕВОЕ СОЕДИНЕНИЕ 
	...
	// Спишем долги по отгрузкам
	//
	//Если Ссылка.ОтражатьВБУ Тогда
	//
	//	Списываем 	= Ссылка.Сумма;
	//	Долги 		= Пакет[7].Выгрузить();
	//	ДолгиТЗ 	= Долги.СкопироватьКолонки();
	//	
	//	Для Каждого СтрокаДолга Из Долги Цикл
	//		
	//		Берем = Мин(Списываем, СтрокаДолга.Сумма);
	//		Если Берем Тогда
	//			
	//			НовСтрока = ДолгиТЗ.Добавить();
	//			ЗаполнитьЗначенияСвойств(НовСтрока, СтрокаДолга);
	//			СтрокаДолга.Сумма = Берем;
	//			
	//			Списываем = Списываем - Берем;
	//			
	//		КонецЕсли;
	//	КонецЦикла;
	//	
	//	Если Списываем Тогда // Аванс на освободившиюся сумму
	//		
	//		НовСтрока = ДолгиТЗ.Добавить();
	//		НовСтрока.Период 		= Ссылка.Дата;
	//		НовСтрока.ВидДвижения	= ВидДвиженияНакопления.Расход;
	//		НовСтрока.Организация	= Ссылка.ДокументОтгрузки.Организация;
	//		НовСтрока.Партнер		= Ссылка.ДокументОтгрузки.Партнер;
	//		НовСтрока.Контрагент	= Ссылка.ДокументОтгрузки.Контрагент;
	//		НовСтрока.Заказ			= Ссылка.ДокументОтгрузки.Заказ;
	//		НовСтрока.ДокументОтгрузки = Ссылка.ДокументОтгрузки;
	//		НовСтрока.Сумма 		= Списываем;
	//		
	//	КонецЕсли;
	//	
	//	ДополнительныеСвойства.Вставить("ДолгиПоОтгрузкам",	ДолгиТЗ);
	//	
	//КонецЕсли;
