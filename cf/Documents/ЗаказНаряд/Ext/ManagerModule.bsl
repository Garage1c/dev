...
Функция ПолучитьПараметрыПечати(Ссылка)
	...
	//|	ЛЕВОЕ СОЕДИНЕНИЕ 
	//|		(ВЫБРАТЬ ПЕРВЫЕ 1 Ссылка, Партнер ИЗ Справочник.Контрагенты ГДЕ Партнер = &Партнер УПОРЯДОЧИТЬ ПО ЗначениеПоУмолчанию Убыв) Контр
	//|	ПО
	//|		Контр.Партнер = Док.Партнер	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|	
	...
	Запрос.Параметры.Вставить("Партнер", Ссылка.Партнер);
