Процедура ИницилизироватьДополнительныеДанныеДокумента(Ссылка, ДополнительныеСвойства) Экспорт
	...
	////|	Документ.КорректировкаИнтернетЗаказа
	////|ГДЕ
	////|	Ссылка = &Ссылка И Сумма <> 0 И Заказ.Партнер <> &ПустойПартнер
	////|;
	//
	//
	//// Долги по заказам
	...
	//|										&ПериодОчереди,
	//|										Размещение = &Заказ И		
	//|										Номенклатура В(ВЫБРАТЬ РАЗЛИЧНЫЕ Номенклатура ИЗ Документ.КорректировкаИнтернетЗаказа.РазмещениеТоваров ГДЕ Ссылка = &Ссылка И Размещение ССЫЛКА Документ.ЗаказПоставщику))
	...
	//|	СУММА(Количество)	Количество
	//|ИЗ
	//|	Документ.КорректировкаИнтернетЗаказа.РазмещениеТоваров Док
	//|
