Процедура ИницилизироватьДополнительныеДанныеДокумента(Ссылка, ДополнительныеСвойства) Экспорт
	...
	|	СогласованиеАссортиментаПартнеры.Партнер,
...
Функция ПисьмоОРезультатеОбработка(Ссылка, Товары, Контрагенты) Экспорт
	...
	|;
	|ВЫБРАТЬ Пользователь, Номенклатура, Номенклатура.Артикул Артикул, Партнер, ID, Период, Оповещение, Письмо 
	|ИЗ РегистрСведений.ЗапросНаСогласованиеТовара 
	|ГДЕ Номенклатура В (ВЫБРАТЬ Номенклатура ИЗ Товары ГДЕ ДатаНачала <> ДАТАВРЕМЯ(1,1,1)) И Партнер В (&Контрагенты) И (Статус = 1 ИЛИ Статус = 2) 
