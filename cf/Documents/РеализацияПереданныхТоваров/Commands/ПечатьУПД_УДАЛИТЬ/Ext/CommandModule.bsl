...
Процедура Печать(ТабДок, ПараметрКоманды, ДополнительныеПараметры, стрНомера)
	...
	Для Каждого Ссылка Из ПараметрКоманды Цикл 
		...
		Если ТипЗнч(Ссылка) = Тип("БизнесПроцессСсылка.СборкаЗаказа") Тогда
			СсылкаДок = Ссылка.РеализацияПереданныхТоваров;
...
&НаСервере
 Процедура Печать_УПД(ТабличныйДокумент, Ссылка, Номер = "", Дата = Неопределено) Экспорт
	Основание = "Заказ";
	Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.РеализацияПереданныхТоваров") Тогда
		ДанныеДляПечати = Документы.РеализацияПереданныхТоваров.ПолучитьДанныеДляУПД_ИзРеализации(Ссылка);
	//ИначеЕсли
	...
	ИначеЕсли	
		...
		ДанныеДляПечати = Документы.РеализацияПереданныхТоваров.ПолучитьДанныеДляУПД_ИзРеализации(Ссылка.РеализацияПереданныхТоваров);
	ИначеЕсли	
 ...
 Процедура ЗаполнитьТабличныйДокументУПД(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, Номер, Дата, ЭтоСчетФактураНаАванс = Ложь, ПечатьВВалюте = Ложь,Основание = "") Экспорт
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Иначе
			...
			//Документы.РеализацияПереданныхТоваров.УдалитьЛишниеКопейкиВТабличнойЧасти(ДанныеПечати.Ссылка, ТаблицаТовары); 
