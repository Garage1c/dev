...
//				НовыйКонтрагент, 
//				Объект.Организация, 
//				КонтрагентРаботаетСОрганизацией(НовыйКонтрагент, Объект.Организация)) Тогда
//				
...
Процедура ПоискВБухгалтерии(Команда)
	ВыбранноеЗначение = ОткрытьФорму("ОбщаяФорма.ПоискПартнераВБухгалтерии",Новый Структура("Партнер", Объект.Партнер), ЭтаФорма,,,,Новый ОписаниеОповещения("ОбработкаПоискВБухгалтерии",ЭтаФорма,));
КонецПроцедуры
...
Процедура НомерДоговораНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Контрагент81.ПолучитьОбъект() = Неопределено Тогда
		Контрагент81 	= Connector.Справочники.Контрагенты.ПолучитьСсылку(Connector.NewObject("UUID", Строка(Объект.Партнер.УникальныйИдентификатор()))); КонецЕсли;	
