...
Функция ПроверкаУникальностиНомера(ТекущийОбъект)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ ИСТИНА ИЗ Документ.Инвойс ГДЕ НЕ ПометкаУдаления И ВходящийНомер = &Номер И НАЧАЛОПЕРИОДА(Дата, ДЕНЬ) = &Дата И Контрагент = &Контрагент И Ссылка <> &Ссылка");
