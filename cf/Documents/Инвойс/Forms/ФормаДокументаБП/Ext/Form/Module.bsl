...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Если Параметры.Страница = 1 Тогда
		...
		Элементы.ГруппаТовары.ТекущаяСтраница = Элементы.ГруппаИнвойс;
		Элементы.ГруппаПакинг.Видимость = Ложь;
		ИначеЕсли 
			Параметры.Страница = 2 Тогда
			
			ЗагрузитьТаблицуТовары();				
			Элементы.ГруппаТовары.ТекущаяСтраница = Элементы.ГруппаПакинг;
			Элементы.ГруппаИнвойс.Видимость = Ложь;
		КонецЕсли;
	КонецЕсли;		
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	//Если НЕ ТекущийОбъект.Процесс.Пустая() Тогда
	//	Процесс = ТекущийОбъект.Процесс.ПолучитьОбъект();
	//	Процесс.Инвойс = ТекущийОбъект.Ссылка;
	//	Задача = ФункцииБизнесПроцессов.ТекущаяЗадача(ТекущийОбъект.Процесс);
	//	ЗадачаОбъект = Задача.ПолучитьОбъект();
	//	ЗадачаОбъек.ВыполнитьЗадачу();
	//КонецЕсли;
КонецПроцедуры
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ПроцессОбъект.Инвойс = ТекущийОбъект.Ссылка;
	ПроцессОбъект.Записать();
	
	Задача = ФункцииБизнесПроцессов.ТекущаяЗадача(Процесс); //, БизнесПроцессы.ЗакупкаИмпорт.ТочкиМаршрута.ЗаполнитьИнвойс);
	ЗадачаОбъект = Задача.ПолучитьОбъект();
...
Процедура Готово(Команда)
	Записать(Новый Структура("РежимЗаписи, ВыполнитьЗадачу", РежимЗаписиДокумента.Проведение, Истина));
