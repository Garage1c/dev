...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДолгПартнера = ?(Объект.Ссылка.Пустая(), 0, ДенежныеСредства.ПолучитьДолгКонтрагента(Объект.Контрагент));
...
Функция УведомитьОПоступленииТовараЗамены()
	...
	|ВЫБРАТЬ
	|	Тов.Заказ 									Заказ,
	|	Тов.Заказ.Контрагент						Контрагент,
	|	Тов.Заказ.Контрагент.ОсновнойМенеджер.Почта   	Почта,
	|	Тов.Номенклатура							Номенклатура,
	|	Тов.Номенклатура.Артикул					Артикул,
	|	&ДеньПоявления								ДеньПоявления
	|ИЗ
	|	РегистрНакопления.РазмещениеЗаказов.Остатки(, Номенклатура В (ВЫБРАТЬ Номенклатура ИЗ ТоварыДляКоторыхПришлаЗамена) И Заказ.Контрагент В (ВЫБРАТЬ Контрагент ИЗ ТоварыДляКоторыхПришлаЗамена)) Тов
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	...
	|ИТОГИ МАКСИМУМ(Контрагент) ПО Почта, Заказ
	|");
	...
	Пока ВыборкаПочты.Следующий() Цикл
			...
			Пока ВыборкаЗаказов.Следующий() Цикл
				...
				ТекстПисьма = ТекстПисьма + "<p>
								|" + ВыборкаЗаказов.Заказ + " (" + ВыборкаЗаказов.Контрагент + ")<UL>";
...
Функция СообщитьМенеджерамОРезерве()
	...
	Для Каждого Строка Из Объект.Резервы Цикл
		...
		Если 	(	ТипЗнч(Строка.Размещение) = ТипЗаказ Или
				...
				Не Строка.Размещение.Контрагент.Пустая() И
				Не Строка.Размещение.Контрагент.ОсновнойМенеджер.Пустая() И
				Не ПустаяСтрока(Строка.Размещение.Контрагент.ОсновнойМенеджер.Почта) Тогда
				
			НовСтрока = Товары.Добавить();
			ЗаполнитьЗначенияСвойств(НовСтрока, Строка);
			НовСтрока.Почта 	= Строка.Размещение.Контрагент.ОсновнойМенеджер.Почта;
		...
		События.ЗарегистрироватьСобытие("РаздачаРезерва",
			Новый Структура("Ссылка, Инициатор, Место, Название, КраткоеОписание, Параметры",
				...
				?(ТипЗнч(текЗаказ) = ТипВнЗаказ,
					...
					"Роздан резерв при поступлении по заказу " + текЗаказ.Номер + " (" + текЗаказ.Контрагент + ") на сумму " + Сумма + " " + текЗаказ.Валюта)),
				Объект); КонецЦикла;
	...
	Если Товары.Количество() Тогда
		...
		|	Тов.Почта,
		|	Тов.Заказ,
		|	Тов.Заказ.Контрагент,
		|	Тов.Артикул,
		|	Тов.Номенклатура,
		|	Тов.ДеньПоявления	
		|
		|Итоги МАКСИМУМ(Контрагент) ПО
		|	Тов.Почта, Тов.Заказ
		|";
...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	Если Копирование Тогда
		Элемент.ТекущиеДанные.КоличествоПоДокументам = 0;	                	
	КонецЕсли;
КонецПроцедуры
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Документы.ПоступлениеТоваров.УстановитьСтатусыДокументов(ТекущийОбъект.Ссылка, ЭтаФорма, ТекущийОбъект);
КонецПроцедуры
// - neti Леконцев 19.06.2017
