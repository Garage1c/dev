...
Процедура ИницилизироватьДополнительныеДанныеДокумента(Ссылка, ДополнительныеСвойства) Экспорт
	...
	Если ИменаРегистров.Найти("Взаиморасчеты")<>Неопределено ИЛИ ПоВсем Тогда
		...
		//|	СУММА(Сумма)		КАК Сумма,
		//|   СУММА(Сумма) * (ЕСТЬNULL(МАКСИМУМ(ВалЦен.Курс), 1) * ЕСТЬNULL(МАКСИМУМ(ВалУпр.Кратность), 1)) / (ЕСТЬNULL(МАКСИМУМ(ВалУпр.Курс), 1) * ЕСТЬNULL(МАКСИМУМ(ВалЦен.Кратность), 1)) КАК СуммаУпр
		//|ИЗ
		//|	Документ.ПоступлениеТоваров.Товары
		//|	ЛЕВОЕ СОЕДИНЕНИЕ 
...
Функция ЕстьРасхожденияПоКоличествуВПоступленииИИнвойсе(Ссылка) Экспорт
	...
	|	ЕСТЬNULL(ИнвойсТовары.Количество, 0) КАК КоличествоПоИнвойсу
	|ИЗ
	|	Документ.ПоступлениеТоваров.Товары КАК ПоступлениеТоваровТовары
	...
	|ГДЕ
	|	ПоступлениеТоваровТовары.Ссылка = &Ссылка
	|	И ПоступлениеТоваровТовары.Количество <> ЕСТЬNULL(ИнвойсТовары.Количество, 0)";
...
Функция ЕстьРасхожденияПоКоличествуВДокументе(ПоступлениеСсылка) Экспорт
	...
	|ГДЕ
	|	ПоступлениеТоваровТовары.Ссылка = &Ссылка
	|	И ПоступлениеТоваровТовары.Количество <> ПоступлениеТоваровТовары.КоличествоПоДокументам";
