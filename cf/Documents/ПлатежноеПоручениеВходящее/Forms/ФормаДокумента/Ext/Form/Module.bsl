...
&НаСервере
//Процедура ПартнерПриИзмененииНаСервере(ЗаполнитьОрганизацию = Истина)
//	
//	ФункцииФормДокументов.ПартнерПриИзменении(Объект,, ЗаполнитьОрганизацию);
//	
//КонецПроцедуры
//&НаСервере
//Функция ПолучитьОсновногоКонтрагентаПартнера()
//	Возврат Справочники.Партнеры.ПолучитьОсновногоКонтрагентаПартнера(Объект.Партнер);
//КонецФункции

//&НаКлиенте
//Процедура ПартнерПриИзменении(Элемент)
//	
//	НовыйКонтрагент = ПолучитьОсновногоКонтрагентаПартнера();
//	
//	Если НЕ ДиалогиСПользователем.ПроверитьНаСоответствиеОсновнойОрганизации(
//				НовыйКонтрагент, 
//				Объект.Организация, 
//				КонтраСоргой(НовыйКонтрагент, Объект.Организация)) Тогда
//				
//		Объект.Организация = ОрганизацияПоДокументу;
//		ПартнерПриИзмененииНаСервере(Ложь);
//	Иначе
...
Процедура УправлениеВидимостью()
	...
	//Элементы.ПодтвердитьОплатуКартой.Видимость	= ЭтоОплатаПоБанковскойКарте;
	//Элементы.РасшифровкаСуммыЗаказ.Видимость	= НЕ ЭтоОплатаПоБанковскойКарте;
	...
	//Элементы.Партнер.Доступность = ?(РольДоступна("ПолныеПрава") ИЛИ РольДоступна("Бухгалтер"),Истина, Ложь); 
...
Процедура ПоискВБухгалтерии(Команда)
	ВыбранноеЗначение = ОткрытьФорму("ОбщаяФорма.ПоискПартнераВБухгалтерии",Новый Структура("Партнер", Объект.Партнер), ЭтаФорма,,,,Новый ОписаниеОповещения("ОбработкаПоискаВБухгалтерии",ЭтаФорма,));
КонецПроцедуры
&НаКлиенте
...
Процедура НомерДоговораНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Контрагент81 	= Connector.Справочники.Контрагенты.ПолучитьСсылку(Connector.NewObject("UUID", Строка(Объект.Партнер.УникальныйИдентификатор())));	
	Если Контрагент81.ПолучитьОбъект() = Неопределено Тогда
		Контрагент81 	= Connector.Справочники.Контрагенты.ПолучитьСсылку(Connector.NewObject("UUID", Строка(Объект.Контрагент.УникальныйИдентификатор()))); КонецЕсли;	
