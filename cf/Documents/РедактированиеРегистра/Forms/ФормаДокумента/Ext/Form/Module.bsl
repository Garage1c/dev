...
Процедура ЗаполнитьРазницуИз81ВзаиморасчетыНаСервере()
	...
	ТаблицаДанных81.Колонки.Добавить("Партнер", 		Новый ОписаниеТипов("СправочникСсылка.Партнеры"));
	...
	Пока Выборка81.Следующий() Цикл
		...
		НовСтрока.Партнер           = Справочники.Партнеры.ПолучитьСсылку(Новый УникальныйИдентификатор(Connector.String(Выборка81.Контрагент.УникальныйИдентификатор())));
	...
	|	ЕСТЬNULL(Ост.Партнер, Таблица81.Партнер)	 		КАК Партнер,	
	|	ВЫБОР 
...
//	ТаблицаДанных81.Колонки.Добавить("ВладелецКарты", 		Новый ОписаниеТипов("СправочникСсылка.Партнеры"));
//	ТаблицаДанных81.Колонки.Добавить("Сумма", 		Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
//	
//	Выборка81 = РезультатЗапроса81.Выбрать();
//	Пока Выборка81.Следующий() Цикл
...
//		НовСтрока.ВладелецКарты     = Справочники.Партнеры.ПолучитьСсылку(Новый УникальныйИдентификатор(Connector.String(Выборка81.ВладелецКарты.УникальныйИдентификатор())));
//		НовСтрока.Сумма				= Выборка81.Сумма;
//		
//	КонецЦикла;
//	
