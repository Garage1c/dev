...
Процедура ИницилизироватьДополнительныеДанныеДокумента(Ссылка, ДополнительныеСвойства) Экспорт
	...
	Если ИменаРегистров.Найти("ТоварыВРезерве")<>Неопределено ИЛИ ПоВсем Тогда
		...
		// ТОВАРЫ В РЕЗЕРВЕ
		//1. списание резервов с отправителя
		//2. приход резервов на получателя
		//3. приход на спецзаказ при перемещении в ячейку "не найдено"
		//4. снятие со спецзаказа при перемещении с ячейки "не найдено"
		...
		|ГДЕ
		|	Ссылка = &Ссылка 
		|	И ЯчейкаПолучатель = &ЯчейкаПолучательНеНайдено и ЯчейкаПолучатель <> Значение(Справочник.Ячейки.ПустаяСсылка) 
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|
		...
		|ГДЕ
		|	Ссылка = &Ссылка 
		|	И ЯчейкаОтправитель = &ЯчейкаОтправительНеНайдено и ЯчейкаОтправитель <> Значение(Справочник.Ячейки.ПустаяСсылка) 
		|
		|	"
	...
	Запрос.УстановитьПараметр("ЯчейкаОтправительНеНайдено",		Ссылка.СкладОтправитель.ЯчейкаНеНайдено);
	Запрос.УстановитьПараметр("ЯчейкаПолучательНеНайдено",		Ссылка.СкладПолучатель.ЯчейкаНеНайдено);
