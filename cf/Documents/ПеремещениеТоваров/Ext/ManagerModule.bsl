...
Процедура ИницилизироватьДополнительныеДанныеДокумента(Ссылка, ДополнительныеСвойства) Экспорт
	...
	Если ИменаРегистров.Найти("ТоварыВРезерве")<>Неопределено ИЛИ ПоВсем Тогда
		...
		|ГДЕ
		|	Ссылка = &Ссылка 
		|	И ЯчейкаОтправитель = &ЯчейкаОтправителНеНайдено и ЯчейкаОтправитель <> Значение(Справочник.Ячейки.ПустаяСсылка) 
		|
		|	"
	...
	Запрос.УстановитьПараметр("ЯчейкаОтправителНеНайдено",		Ссылка.СкладОтправитель.ЯчейкаНеНайдено);
