...
Процедура ИницилизироватьДополнительныеДанныеДокумента(Ссылка, ДополнительныеСвойства) Экспорт
	...
	//|	СУММА(Сумма) Сумма
	//|ИЗ
	//|	Документ.ПриходныйКассовыйОрдер.РасшифровкаСуммы
	//|ГДЕ
	...
	//|
	//|ПРАВОЕ СОЕДИНЕНИЕ
	//|	(	ВЫБРАТЬ Дата, Организация, Контрагент, Ссылка, Сумма ИЗ Документ.ПриходныйКассовыйОрдер ГДЕ Ссылка = &Ссылка
	...
	//Если НЕ Ссылка.Партнер.Пустая() Тогда
		ДополнительныеСвойства.Вставить("Взаиморасчеты", 	Пакет[3].Выгрузить());
	//КонецЕсли;
	//Авансы = Пакет[7].Выгрузить();
	//Если Не Ссылка.РасшифровкаСуммы.Количество() И НЕ Ссылка.Партнер.Пустая() И Ссылка.ВидОперации = Перечисления.ВидыОперацийПКО.ОплатаПокупателя Тогда
