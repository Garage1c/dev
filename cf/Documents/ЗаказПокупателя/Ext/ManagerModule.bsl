...
Функция СформироватьПечатнуюФорму(ТабличныйДокумент, Ссылка, Тип, Дата=Неопределено, ИмяДопИмениОбластей = "", ПолеСортировки = "НомерСтроки",  ИмяДокумента = "ЗаказПокупателя")	
	...
	ИначеЕсли
		...
		ИмяРеквизитаЗаказ = "ЗаказПокупателя";
	ИначеЕсли
		ИмяДокумента = "РеализацияПереданныхТоваров" Тогда
		ИмяРегистра = "ЗаказыПокупателей";
		ИмяРеквизитаЗаказ = "ЗаказПокупателя";
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|" + ?(ИмяДокумента = "РеализацияПереданныхТоваров" или ИмяДокумента = "РеализацияТоваров"  ИЛИ Ссылка.СпособРазмещенияБезЗаказа ИЛИ НЕ Ссылка.Проведен,"
		|
		|ВЫБРАТЬ ""фиктивный запрос для равенства количества запросов в пакете""
		|;
		|ВЫБРАТЬ
		|	Док.НомерСтроки                         					КАК НомерСтроки,"+
		?(ИмяДокумента = "РеализацияПереданныхТоваров" ,"
		|Док.Номенклатура 	КАК Номенклатура, "
		,"
		...
		?(ИмяДокумента = "РеализацияПереданныхТоваров" ,"
		|	(Док.Цена * Док.Количество - ЕСТЬNULL(Таб.Доставка, 0)) * 0.01 + (Док.Цена * Док.Количество -  ЕСТЬNULL(Таб.Доставка, 0))* 0.01  КАК СуммаСкидки, "
		,"
	    ...
	    |	Док.Цена * Док.КоличествоПриход     		КАК СуммаБезСкидки,"+
		?(ИмяДокумента = "РеализацияПереданныхТоваров" ,"
		|	(Док.Цена * Док.КоличествоПриход - ЕСТЬNULL(Таб.Доставка, 0)) * 0.01 + (Док.Цена * Док.КоличествоПриход - ЕСТЬNULL(Таб.Доставка, 0)) * 0.01  КАК СуммаСкидки,"
		,"
		|	(Док.Цена * Док.Количество - ЕСТЬNULL(Таб.Доставка, 0)) * Док.ПроцентРучнойСкидки * 0.01 + (Док.Цена * Док.Количество -  ЕСТЬNULL(Таб.Доставка, 0)) * Док.ПроцентАвтоматическойСкидки * 0.01  КАК СуммаСкидки,")
