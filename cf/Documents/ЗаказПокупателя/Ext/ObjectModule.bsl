...
Процедура ПриЗаписи(Отказ)
	...
	//Если Склад.ПередачаТовараМВЗ Тогда
	//	
	//	Запрос = Новый Запрос("ВЫБРАТЬ Док.Номенклатура ИЗ Документ.ЗаказПокупателя.Товары Док
	//	|ЛЕВОЕ СОЕДИНЕНИЕ
	//	|	РегистрСведений.ТоварыДляПередачи Рег
	//	|ПО Док.Ссылка.Партнер = Рег.Партнер И 
	//	|	Док.Номенклатура = Рег.Номенклатура"
	//	|ГДЕ Док.Ссылка = &Ссылка И НЕ Рег.Номенклатура ЕСТЬ NULL");
	//	
	...
	//		Отказ = Истина;
	//		ОбщиеФункции.СообщитьТекст("Товар """ + Выборка.Номенклатура + " не входит в список доступных товаров для заказа");
	//	КонецЕсли;
	//КонецЕсли;
КонецПроцедуры
