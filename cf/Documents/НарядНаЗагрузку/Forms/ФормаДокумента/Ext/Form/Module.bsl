...
Процедура ОтправитьПослеЗакрытияВопросаНаСервере(ЭтоОтправка)
	Для Каждого ТекСтрока Из Объект.ТабЗаказов Цикл
		...
		Если НЕ ЗначениеЗаполнено(ТекСтрока.ДокументОтгрузки) Или НЕ ЗначениеЗаполнено(ТекСтрока.Заказ) Тогда
			Продолжить;
		КонецЕсли;
		//Задачи стартуем только для Реализации и Передачи
		Если ТипЗнч(ТекСтрока.ДокументОтгрузки)<>Тип("ДокументСсылка.РеализацияТоваров") И ТипЗнч(ТекСтрока.ДокументОтгрузки)<>Тип("ДокументСсылка.ПередачаТовара") Тогда
			Продолжить;
		КонецЕсли;
		Если НЕ НайтиВыполнитьЗадачу(ТекСтрока.Заказ,ТекСтрока.ДокументОтгрузки,ЭтоОтправка) Тогда
			Сообщить("Ошибка при поиске и выполнении задачи "+ТекСтрока.Заказ+" "+ТекСтрока.ДокументОтгрузки);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура ТабЗаказовДокументОтгрузкиПриИзмененииНаСервере()
	...
	Если ЗначениеЗаполнено(ТекСтрока.ДокументОтгрузки) И ТипЗнч(ТекСтрока.ДокументОтгрузки) = Тип("ДокументСсылка.ОтгрузкаТоваров") И ЗначениеЗаполнено(ТекСтрока.ДокументОтгрузки.СкладПолучатель) Тогда
		ЗагрузкаВАвтомобильЗаказов.ЗаполнитьПоСкладу(ТекСтрока.ДокументОтгрузки.СкладПолучатель,ТекСтрока);
	КонецЕсли;
КонецПроцедуры
