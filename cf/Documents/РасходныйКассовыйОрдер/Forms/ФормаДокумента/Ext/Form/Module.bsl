...
Функция КонтрольАвтозаполненияПартнер()
	...
	Возврат Объект.ВидОперации <> Перечисления.ВидыОперацийРКО.ПрочийРасход И Объект.Контрагент.Пустая();
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение И ОбщегоНазначения.НужноПробитиеПоКассе(Объект.Касса) И НЕ Объект.ПроведеноПоККТ И Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРКО.ВозвратПокупателю") Тогда
