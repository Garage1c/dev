...
&НаСервере
Функция НоменклатураСоздать(Артикул, Наименование, Производитель, ГруппаТовара, Код, Код2, Комментарий, КатегорияТовара,Поставщик,АртикулПоставщика,СрокПроизводства,СрокДоставки)
	...
	НовыйСпр.Поставщик = Поставщик;
	НовыйСпр.АртикулПоставщика = АртикулПоставщика;
	НовыйСпр.СрокПроизводства = СрокПроизводства;
	НовыйСпр.СрокДоставки = СрокДоставки;
...
Процедура СоздатьНаСервере(ИндСтроки)
	...
	Если Строка.Пометка Тогда
		...
		Если 	Спр.Пустая() Тогда
				Строка.ЭтоНовый = Истина;
				Строка.Номенклатура = НоменклатураСоздать(Строка.Артикул, Строка.Наименование, Строка.Производитель, Строка.ГруппаТовара, Строка.Код, Строка.Код2, Строка.Комментарий, Строка.КатегорияТовара,Строка.Поставщик, Строка.АртикулПоставщика, Строка.СрокПроизводства,Строка.СрокДоставки);
...
Процедура ЗаполнитьСсылочныеДанные()
	...
	Для Ном = 2 По КолВоСтрок Цикл ОбработкаПрерыванияПользователя();
...
Процедура ЗаполнитьСсылочныеДанныеСтроки(Ид)
	...
	УстановитьСсылку("Поставщик", 	"КодПоставщика", 	"Контрагенты",  "Контрагенты",	Строка);
...
Функция ПолучитьСправочникПоКоду(Код, ИмяСпр)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1 Ссылка ИЗ Справочник.Контрагенты ГДЕ Код = """ + Код + """");
	//Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1 Ссылка ИЗ Справочник.Контрагенты ГДЕ Код = """ + Формат(Число(Код), "ЧЦ=9; ЧВН=; ЧГ=0") + """");
