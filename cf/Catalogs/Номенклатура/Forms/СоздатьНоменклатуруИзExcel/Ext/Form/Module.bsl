...
&НаСервере
Функция НоменклатураСоздать(Артикул, Наименование, Производитель, ГруппаТовара, Код, Код2, Комментарий, КатегорияТовара,Поставщик)
...
Процедура СоздатьНаСервере(ИндСтроки)
	...
	Если Строка.Пометка Тогда
		...
		Если 	Спр.Пустая() Тогда
				Строка.ЭтоНовый = Истина;
				Строка.Номенклатура = НоменклатураСоздать(Строка.Артикул, Строка.Наименование, Строка.Производитель, Строка.ГруппаТовара, Строка.Код, Строка.Код2, Строка.Комментарий, Строка.КатегорияТовара,Строка.Поставщик); 
