...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Метаданные.ОбщиеМодули.Найти("УправлениеЭлектроннойПочтой") = Неопределено Тогда
		Элементы.ОтветственныйЗаОбработкуПисем.Видимость = Ложь;
...
Процедура ДополнительныеНастройкиВыполнить()
	...
	УчетнаяЗаписьСтруктура.Вставить("ИспользоватьSSLPOP3",              Объект.ИспользоватьSSLPOP3);
	УчетнаяЗаписьСтруктура.Вставить("ИспользоватьSSLSMTP",              Объект.ИспользоватьSSLSMTP);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "УстановкаДополнительныхПараметровУчетнойЗаписи" И Источник = Объект.Ссылка Тогда
		...
		Объект.ИспользоватьSSLPOP3             	= Параметр.ИспользоватьSSLPOP3;
		Объект.ИспользоватьSSLSMTP              = Параметр.ИспользоватьSSLSMTP;
