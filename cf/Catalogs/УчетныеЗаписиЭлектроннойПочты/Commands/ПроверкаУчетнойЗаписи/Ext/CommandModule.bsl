...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	Если ТипЗнч(УчетнаяЗапись) = Тип("СправочникСсылка.УчетныеЗаписиЭлектроннойПочты") Тогда
		...
		Иначе
			...
			//ПарольПараметр = ОткрытьФорму("ОбщаяФорма.ПодтверждениеПароляУчетнойЗаписи", ПараметрУчетнаяЗапись);
			//Если ТипЗнч(ПарольПараметр) <> Тип("Строка") Тогда
			//	Возврат
			//КонецЕсли;
			ПарольПараметр = ОткрытьФорму("ОбщаяФорма.ПодтверждениеПароляУчетнойЗаписи", ПараметрУчетнаяЗапись,,,,,Новый ОписаниеОповещения("ОбработкаВводаПароля",,Новый Структура("Флаг", Флаг)));
			Если Флаг <> "Строка" Тогда
				Возврат;
			КонецЕсли;
		КонецЕсли;              
...
КонецПроцедуры
&НаКлиенте
Процедура ОбработкаВводаПароля(Результат, Параметры) Экспорт
	Если ТипЗнч(Результат) <> Тип("Строка") Тогда
		Параметры.Флаг = "";
	Иначе
		Параметры.Флаг = "Строка";
	КонецЕсли;
КонецПроцедуры
&НаКлиенте
