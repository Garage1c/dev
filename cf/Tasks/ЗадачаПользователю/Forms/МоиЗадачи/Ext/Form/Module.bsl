&НаКлиенте
Перем ТочкиМаршрутаРазрешающиеОбъединение, стВыбранныеСклады, стВыбранныеТочкиМаршрута;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокТекущих.Параметры.УстановитьЗначениеПараметра("ПомечатьСборочники", 	Ложь);
	СписокТекущих.Параметры.УстановитьЗначениеПараметра("ПустаяДата", 			'00010101');
	СписокТекущих.Параметры.УстановитьЗначениеПараметра("ПустойСборочныйЛист", 	Документы.СборочныйЛист.ПустаяСсылка());
...
&НаКлиенте
Процедура ИницилизироватьОбъединение()
	
	ТочкиМаршрутаРазрешающиеОбъединение = новый Массив;
	ТочкиМаршрутаРазрешающиеОбъединение.Добавить(ПредопределенноеЗначение("БизнесПроцесс.СборкаТовара.ТочкаМаршрута.СобратьТовар"));
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ЗадачиМогутБытьВместе(Выбзадачи)
	
	// Возвращает ИСТИНА если задачми можно объединить
	
	Запрос = Новый Запрос("
	|ВЫБРАТЬ ИСТИНА ИЗ (
	|	ВЫБРАТЬ КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Склад) + КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТочкаМаршрута) Полей
	|	ИЗ Задача.ЗадачаПользователю
	|	ГДЕ Ссылка В(&Задачи)) Запрос
	|ГДЕ Полей <> 2
	|");
...
КонецФункции
&НаСервереБезКонтекста
Функция ПолучитьСписокЗаказов(выбЗадачи)
	
	Список = Новый СписокЗначений;
	Список.ЗагрузитьЗначения(ЗаказыКоллекцияСервер.ПолучитьВсеЗаказыЗадач(выбЗадачи));
	Возврат Список;
	
КонецФункции
	
&НаКлиенте
Процедура ЗакрытьАнимациюОбъединения(РезЗакрытия, Доппараметры) Экспорт
	
	Элементы.АнимацияКвадратики.Видимость = Ложь;
	
КонецПроцедуры
&НаКлиенте
Процедура ОткрытьИОбъединитьВыбранные(Команда)
	
	ВыбЗадачи = Новый Массив;
	Для Каждого вбЗадача Из Элементы.СписокТекущих.ВыделенныеСтроки Цикл ВыбЗадачи.Добавить(вбЗадача); КонецЦикла;
	
	СборочныеЛисты = ПолучитьСборочныеЛистыПоЗадачам(ВыбЗадачи);
	
	Если СборочныеЛисты.Количество() > 1 Тогда
		ПоказатьПредупреждение(, "Запрещено выбирать задачи с разными сборочными листами. Задачи со сборочными листами выделены цветом.
								|Если у вас не отображаются цвета, нажмите на кнопку ""Порядок""",,"Защитник от дураков");
	Иначе
		ОткрытьФорму("Задача.ЗадачаПользователю.ФормаОбъекта", 
			Новый Структура("Ключ, ЗаказыДляОбъединения", 
				?(СборочныеЛисты.Количество(), СборочныеЛисты[0].Задача, Элементы.СписокТекущих.ТекущаяСтрока),
				ПолучитьСписокЗаказов(ВыбЗадачи)),, Новый УникальныйИдентификатор,,,Новый ОписаниеОповещения("ЗакрытьАнимациюОбъединения", ЭтаФорма)); КонецЕсли;
КонецПроцедуры
&НаКлиенте
Процедура Объединить(Команда)
	
	текЗадача = Элементы.СписокТекущих.ТекущаяСтрока;
	
	Если текЗадача <> Неопределено Тогда
		
		Кнопка = Элементы.ОбъединитьКнопка;
		
		Если Кнопка.Пометка Тогда Кнопка.Пометка = Ложь;
			
			СписокТекущих.Параметры.УстановитьЗначениеПараметра("ПомечатьСборочники", Ложь);
			Элементы.ОткрытьИОбъединить.Видимость 	= Ложь;
			
			ВыбранныеСклады 		= стВыбранныеСклады;
			ВыбранныеТочкиМаршрута 	= стВыбранныеТочкиМаршрута;
			
			ОбновитьСписокЗадач();
			
		Иначе Кнопка.Пометка = Истина;
			
			СписокТекущих.Параметры.УстановитьЗначениеПараметра("ПомечатьСборочники", Истина);
			Элементы.ОткрытьИОбъединить.Видимость 	= Истина;
			
			стВыбранныеСклады 			= ВыбранныеСклады.Скопировать();
			стВыбранныеТочкиМаршрута 	= ВыбранныеТочкиМаршрута.Скопировать();
			
			УстановитьФильтрПоЗаказуОбъединителю(текЗадача); КонецЕсли; КонецЕсли;
	
КонецПроцедуры
&НаСервере
Процедура УстановитьФильтрПоЗаказуОбъединителю(текЗадача)
	
	ВыбранныеСклады.Очистить(); 		ВыбранныеСклады.Добавить(текЗадача.Склад);
	ВыбранныеТочкиМаршрута.Очистить(); 	ВыбранныеТочкиМаршрута.Добавить(текЗадача.Точкамаршрута);
...
&НаСервере
Функция ПолучитьСборочныеЛистыПоЗадачам(ВыбЗадачи)
	
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ БизнесПроцесс.СборочныйЛист, Ссылка задача ИЗ Задача.ЗадачаПользователю ГДЕ Ссылка В(&задачи) И БизнесПроцесс.СборочныйЛист <> &ПустойСборочныйЛист");
	Запрос.УстановитьПараметр("Задачи", 				ВыбЗадачи);
...
&НаКлиенте
Процедура СписокТекущихПриАктивизацииСтроки(Элемент)
	
	текВидимость 	= Элементы.ГруппаОбъединить.Видимость;
	ТекДанные 		= Элемент.ТекущиеДанные;
	Показывать		= ТекДанные <> Неопределено И ТочкиМаршрутаРазрешающиеОбъединение.Найти(ТекДанные.Точкамаршрута) <> Неопределено;
	
	Если Не текВидимость И Показывать Тогда
		
		Элементы.ГруппаОбъединить.Видимость = Истина;
		
	ИначеЕсли текВидимость И Не Показывать Тогда
		
		Элементы.ГруппаОбъединить.Видимость 	= Ложь;
		Элементы.ОбъединитьКнопка.Пометка 		= Ложь;
		Элементы.ОткрытьИОбъединить.Видимость 	= Ложь; КонецЕсли;
...
Функция ПолучитьТочкиМаршрутаВыбранныхРолей() Экспорт
	...
	//Точки.Добавить(БизнесПроцессы.ОтменаСборки.ТочкиМаршрута.ОтменитьСборку);
