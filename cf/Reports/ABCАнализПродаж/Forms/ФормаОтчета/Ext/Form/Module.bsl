...
Процедура ЗаполнитьПарамерты()
	...
	ПараметрПартнеры = Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("Партнеры"));
	
	ПараметрПартнеры.Значение = Партнеры;
	
	Если Партнеры.Количество() Тогда
