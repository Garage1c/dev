...
&НаСервере
Функция ПолучитьТаблицуОстатковНаДень(НачалоПериода = '00010101', КонецПериода= '00010101', НоменклатурнаяГруппа = Неопределено, ФильтрСклады = Неопределено, ФильтрАкции = Неопределено, Отбор = Неопределено, МассивЭлементовОтбора = Неопределено, ПустаяТаблица = Ложь)
	...
	|ИЗ
	|    РегистрНакопления.ТоварыНаСкладах.ОстаткиИОбороты(&НачалоПериода, &КонецПериода, День, ДвиженияИГраницыПериода, {Номенклатура.* КАК Номенклатура, Склад КАК СкладыОстатков}" +
	?(НоменклатурнаяГруппа = Неопределено, "",  "Номенклатура В (ВЫБРАТЬ Номенклатура Из Справочник.НоменклатурныеГруппы.Состав ГДЕ Ссылка В (&НоменклатурнаяГруппа))") +
	?(ФильтрАкции = Неопределено, "",  ?(НоменклатурнаяГруппа = Неопределено, "", " И ") + "Номенклатура В (ВЫБРАТЬ Номенклатура ИЗ Документ.Акция.Товары ГДЕ Ссылка В (&ФильтрАкции))") + "
	|	)
	|   ИТОГИ
	...
	Если НоменклатурнаяГруппа <> Неопределено Тогда
		...
		Если ТипЗнч(НоменклатурнаяГруппа) = Тип("СправочникСсылка.НоменклатурныеГруппы") Тогда
			СписокНГ = Новый Массив;
			СписокНГ.Добавить(НоменклатурнаяГруппа);
		Иначе
			СписокНГ = НоменклатурнаяГруппа.ВыгрузитьЗначения(); 
		КонецЕсли;
		
		Построитель.Параметры.Вставить("НоменклатурнаяГруппа", СписокНГ); КонецЕсли;

	Если ФильтрАкции <> Неопределено Тогда
		
		Если ТипЗнч(ФильтрАкции) = Тип("ДокументСсылка.Акция") Тогда
			СписокАкций = Новый Массив;
			СписокАкций.Добавить(ФильтрАкции);
		Иначе
			СписокАкций = ФильтрАкции.ВыгрузитьЗначения();
		КонецЕсли;
		
		Построитель.Параметры.Вставить("ФильтрАкции", СписокАкций); КонецЕсли;

	
	Если ФильтрСклады <> Неопределено Тогда

		Если ТипЗнч(ФильтрСклады) = Тип("СправочникСсылка.Склады") Тогда
			СписокСкладов = Новый Массив;
			СписокСкладов.Добавить(ФильтрСклады);
		Иначе
			СписокСкладов = ФильтрСклады.ВыгрузитьЗначения();
		КонецЕсли;
		
		
		Построитель.Отбор.Добавить("СкладыОстатков", "СкладыОстатков");
		Построитель.Отбор["СкладыОстатков"].ВидСравнения = ВидСравнения.ВСписке; 
		Построитель.Отбор["СкладыОстатков"].Значение.ЗагрузитьЗначения(СписокСкладов);
		Построитель.Отбор["СкладыОстатков"].Использование = Истина; КонецЕсли;
...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ЗначениеФильтрАкции = Неопределено;
	ЗначениеФильтрПродажиТолькоПоАкции = Ложь;
	элементотбораФильтрПродажиТолькоПоАкции = Неопределено;
	...
	Для Каждого Элемент ИЗ Параметры.Элементы Цикл
		...
		Если ТипЗнч(Элемент) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") И Элемент.Параметр = Новый ПараметрКомпоновкиДанных("СравнительныйПериод")  И Элемент.Использование Тогда

		     ЗначениеНачалоПериода2 = Элемент.Значение.ДатаНачала;
			 ЗначениеКонецПериода2 = Элемент.Значение.ДатаОкончания;
		КонецЕсли;
		
		//Если ТипЗнч(Элемент) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") И Элемент.Параметр = Новый ПараметрКомпоновкиДанных("НачалоСравнительногоПериода")  И Элемент.Использование Тогда

		//	ЗначениеНачалоПериода2 = Элемент.Значение; 
		//КонецЕсли;
		//
		//Если ТипЗнч(Элемент) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") И Элемент.Параметр = Новый ПараметрКомпоновкиДанных("КонецСравнительногоПериода")  И Элемент.Использование Тогда
		//	ЗначениеКонецПериода2 = Элемент.Значение; 
		//КонецЕсли;		
		...
		Если ТипЗнч(Элемент) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") И Элемент.Параметр = Новый ПараметрКомпоновкиДанных("СкладыПродаж") И Элемент.Использование Тогда
	    	ЗначениеСкладыОстатков = Элемент.Значение;
		КонецЕсли;
		
		Если ТипЗнч(Элемент) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") И Элемент.Параметр = Новый ПараметрКомпоновкиДанных("ФильтрАкции") И Элемент.Использование Тогда
	    	ЗначениеФильтрАкции = Элемент.Значение;
		КонецЕсли;
			
		Если ТипЗнч(Элемент) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			
		//	Если Элемент.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ФильтрПродажиТолькоПоАкции") Тогда  // слева почему то возвращает Неопределено
		// поэтому придется во так делать
			Если ТипЗнч(Элемент.ПравоеЗначение) = Тип("Булево") Тогда
				ЗначениеФильтрПродажиТолькоПоАкции = Элемент.Использование;
				
				Если Элемент.Использование Тогда
					Элемент.Использование = Ложь; // нужен был для интерфейса - отключаем
				КонецЕсли;
				
				элементотбораФильтрПродажиТолькоПоАкции  = Элемент;
	...
	//	
	//			Если ЭлементГруппы.Поле = ПолеКоличествоСреднее И Элемент.Использование Тогда ПоказатьОстаткиНаКаждыйДень = Истина; Прервать; КонецЕсли; 
	//			
	//		КонецЦикла;
	//	Иначе
	//		
	//		Если Элемент.Поле = ПолеКоличествоСреднее И Элемент.Использование Тогда ПоказатьОстаткиНаКаждыйДень = Истина; Прервать; КонецЕсли;	
	//	КонецЕсли;	
	//		
	//КонецЦикла; 
	
	//ПустаяТаблицаОстатков =  ПолучитьТаблицуОстатковНаДень(, , , , , , , Истина);
	
	//Если //ПоказатьОстаткиНаКаждыйДень И 
	//	
	//	ЗначениеЗаполнено(ЗначениеНачалоПериода) И ЗначениеЗаполнено(ЗначениеКонецПериода) Тогда
	//	ВнешниеНаборыДанных.Вставить("ОстаткиНаКаждыйДень", ПолучитьТаблицуОстатковНаДень(ЗначениеНачалоПериода, ЗначениеКонецПериода, ЗначениеНоменклатурнаяГруппа, ЗначениеСкладыОстатков, ЗначениеФильтрАкции, Отбор, МассивЭлементовОтбора));
	//Иначе
	//	ВнешниеНаборыДанных.Вставить("ОстаткиНаКаждыйДень", ПустаяТаблицаОстатков);
	//	
	//КонецЕсли; 		
	//
	Параметры = КомпоновщикНастроек.Настройки.ПараметрыДанных;
	
	Если  ЗначениеЗаполнено(ЗначениеНачалоПериода2) И ЗначениеЗаполнено(ЗначениеКонецПериода2) Тогда
		//ТЗ = ПолучитьТаблицуОстатковНаДень(ЗначениеНачалоПериода2, ЗначениеКонецПериода2, ЗначениеНоменклатурнаяГруппа, ЗначениеСкладыОстатков, ЗначениеФильтрАкции, Отбор, МассивЭлементовОтбора);
		//ТЗ.Колонки.КоличествоСреднее.Имя = "КоличествоСреднее2";
		//ВнешниеНаборыДанных.Вставить("ОстаткиНаКаждыйДеньСравПериод", ТЗ);
		
		// включаем использование таблицы сравнительного периода
		ЗначениеПараметра = Параметры.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("ПоказыватьСравнительныйПериод"));
		Параметры.УстановитьЗначениеПараметра("ПоказыватьСравнительныйПериод", Истина);
		
	Иначе
		//ПустаяТаблицаОстатков.Колонки.КоличествоСреднее.Имя = "КоличествоСреднее2";
		//ВнешниеНаборыДанных.Вставить("ОстаткиНаКаждыйДеньСравПериод", ПустаяТаблицаОстатков);
		
		// выключаем использование таблицы сравнительного периода
		ЗначениеПараметра = Параметры.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("ПоказыватьСравнительныйПериод"));
		Параметры.УстановитьЗначениеПараметра("ПоказыватьСравнительныйПериод", Ложь);
	КонецЕсли;
	
	ЗначениеПараметра = Параметры.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("ТипЦенРозница"));
	Если НЕ ЗначениеЗаполнено(ЗначениеПараметра.Значение) Тогда
   		Параметры.УстановитьЗначениеПараметра("ТипЦенРозница", Константы.ТипЦенРозница.Получить());
	КонецЕсли;
	ЗначениеПараметра = Параметры.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("ТипЦенСЦ"));
	Если НЕ ЗначениеЗаполнено(ЗначениеПараметра.Значение) Тогда
   		Параметры.УстановитьЗначениеПараметра("ТипЦенСЦ", Справочники.ТипыЦен.НайтиПоКоду("000000016"));
	КонецЕсли;
	...
	параметрПродажиТолькоПоАкции = Параметры.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("ПродажиТолькоПоАкции"));
	
	параметрПродажиТолькоПоАкции.Значение = ЗначениеФильтрПродажиТолькоПоАкции;  // если галка не стоит - значит ЛОЖЬ, если стоит, то значение - ИСТИНА
	параметрПродажиТолькоПоАкции.Использование = ЗначениеЗаполнено(ЗначениеФильтрАкции);  // используем всегда, если заполнен фильтр по акциям
	...
	Если ЗначениеФильтрПродажиТолькоПоАкции Тогда  //заметаем следы
		элементотбораФильтрПродажиТолькоПоАкции.Использование = Истина; 
	КонецЕсли;
КонецПроцедуры
