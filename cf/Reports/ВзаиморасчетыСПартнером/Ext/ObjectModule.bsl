...
Функция ДанныеИзВнешнегоИсточника()
	...
	Организация = ""; Контрагент = ""; НачалоПериода = ""; КонецПериода = "";
	...
	|" + ?(ЕстьКонтрагент ИЛИ ЕстьНачалоПериода ИЛИ ЕстьКонецПериода ИЛИ ЕстьОрганизация, "ГДЕ", "") +"
	|
	|" + ?(ЕстьОрганизация, " Организация В (&Организация) " + ?(ЕстьКонтрагент ИЛИ ЕстьНачалоПериода ИЛИ ЕстьКонецПериода, "И", ""), "") +"
	|
	|" + ?(ЕстьКонтрагент, " Контрагент = &Контрагент " + ?(ЕстьНачалоПериода ИЛИ ЕстьКонецПериода, "И", ""), "") + "
	|
	...
	|УПОРЯДОЧИТЬ ПО Организация, Контрагент, Период Возр
	|");
	...
	Если ЕстьКонтрагент Тогда
		Запрос.УстановитьПараметр("Контрагент", Контрагент.УникальныйИдентификатор());
	КонецЕсли;
	...
	Пока Результат.СледующийПоЗначениюПоля("Организация") Цикл
		...
		Пока Результат.СледующийПоЗначениюПоля("Контрагент") Цикл
			...
			НачОстаток = 0;
			текКонтрагент	= Справочники.Контрагенты.ПолучитьСсылку(Результат.Контрагент);	
